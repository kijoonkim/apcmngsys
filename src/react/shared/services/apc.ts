...
export type ApcItem = {
    APC_NM: string; APC_CD: string; NH: "Y"|"N";
    XCRD: string; YCRD: string;
    WGH_AGT_INSTL_YN?: "Y"|"N"; SORT_AGT_INSTL_YN?: "Y"|"N";
    WRHS_ACTVTN_YN?: "Y"|"N"; SORT_ACTVTN_YN?: "Y"|"N";
    SPMT_ACTVTN_YN?: "Y"|"N"; INVNTR_ACTVTN_YN?: "Y"|"N";
    AGRC_ACTVTN_YN?: "Y"|"N"; CLCLN_ACTVTN_YN?: "Y"|"N";
    DEL_YN?: "Y"|"N";
    WGH_LAST_USE_DT?: string; WRHS_LAST_USE_DT?: string; SORT_LAST_USE_DT?: string;
    PCKG_LAST_USE_DT?: string; SPMT_LAST_USE_DT?: string; ORDR_LAST_USE_DT?: string;
    CLCLN_LAST_USE_DT?: string; AGRC_LAST_USE_DT?: string; INVNTR_LAST_USE_DT?: string;
};
...
export type OneYearStatsResult = {
    result: {
        SEARCH_YMD_FROM: string; SEARCH_YMD_TO: string;
...
export async function getApcAgtStatus() {
    ...
    const apcLists = dto.result.resultLists.map((item) => {
        const kinds = [];
        if (item.WGH_AGT_INSTL_YN != null) kinds.push({ name: "계량", value: item.WGH_AGT_INSTL_YN === "Y" ? "active" : "" });
        if (item.SORT_AGT_INSTL_YN != null) kinds.push({ name: "선별", value: item.SORT_AGT_INSTL_YN === "Y" ? "active" : "" });

        return {
            name: item.APC_NM,
            value: item.APC_CD,
            nh: item.NH === "Y",
            cx: item.XCRD, cy: item.YCRD,
            kinds,
            status: [
            ...
            delYn: item.DEL_YN,
            rcntData: latestDate([
                item.WGH_LAST_USE_DT, item.WRHS_LAST_USE_DT, item.SORT_LAST_USE_DT,
                item.PCKG_LAST_USE_DT, item.SPMT_LAST_USE_DT, item.ORDR_LAST_USE_DT,
                item.CLCLN_LAST_USE_DT, item.AGRC_LAST_USE_DT, item.INVNTR_LAST_USE_DT,
            ]),
            useData: [
    ...
    const oprtngFcltCount = apcLists.reduce((sum, i)=> sum + i.kinds.filter(k=>k.value==='active').length, 0);
...
export async function getApcLinkList(apcCd: string) {
    const dto = await postJSON<{ resultList: any[] }>("/co/user/selectApcLinkTrsmMatSttsList.do", { apcCd });
