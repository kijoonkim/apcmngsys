		                        	...
		                        	<table id="dataArea1" class="table table-bordered tbl_fixed">
					                    ...
					                    <tbody>
			                                ...
			                                <tr>
			                                    ...
			                                    <td colspan="2" class="td_input">
						                           	<div style="display:flex;float:left;vertical-align:middle;width:100%">
					  									<sbux-input style="width:80px" id="FM_SUBSIDIES_ACCOUNT" uitype="text" class="form-control input-sm"></sbux-input>
					  									<font style="width:5px"></font>
			                                ...
			                                <tr>
			                                    ...
			                                    <td colspan="2" class="td_input">
						                           	<div style="display:flex;float:left;vertical-align:middle;width:100%">
					  									<sbux-input style="width:80px" id="FM_SUBSIDIES_DEPR_ACC" uitype="text" class="form-control input-sm"></sbux-input>
					  									<font style="width:5px"></font>
			                                ...
			                                <tr>
			                                    ...
			                                    <td colspan="2" class="td_input">
						                           	<div style="display:flex;float:left;vertical-align:middle;width:100%">
					  									<sbux-input style="width:80px" id="FM_SUBSIDIES_ACCUM_DEPR_ACC" uitype="text" class="form-control input-sm"></sbux-input>
					  									<font style="width:5px"></font>
...
<script type="text/javascript">
	...
	const fn_initSBSelect = async function() {
		let rst = await Promise.all([
            ...
            // 사업장
            gfnma_setComSelect(['SCH_SITE_CODE','FM_SITE_CODE'],				jsonSiteCode, 		'L_ORG001', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SITE_CODE', 'SITE_NAME', 'Y', ''),
            // 실적구분
            ...
            // 자산구분
            gfnma_setComSelect(['SCH_ASSET_CATEGORY'],							jsonActualFlagP,	'L_FIA001', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SUB_CODE', 'CODE_NAME', 'Y', ''),
            // 중분류
            ...
            // 자산구분
            gfnma_setComSelect(['FM_ASSET_CATEGORY'],							jsonFmAssetCategory,	'P_ASSET_CATEGORY', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'ASSET_CATEGORY', 'ASSET_CATEGORY_NAME', 'Y', ''),
            // 통화
            gfnma_setComSelect(['FM_CURRENCY_CODE'],							jsonCurrencyCode,	'L_COM001', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'CURRENCY_CODE', 'CURRENCY_NAME', 'Y', ''),
            // 회계기준
            ...
            // 감가상각기준
            gfnma_setComSelect(['SCH_DEPRECIATION_TYPE'],						jsonDepreciationType,'L_FIA018', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SUB_CODE', 'CODE_NAME', 'Y', ''),
            
            // 감가상각방법
            gfnma_setComSelect(['FM_DEPRECIATION_METHOD_GAAP','FM_DEPRECIATION_METHOD_TAX','FM_DEPRECIATION_METHOD_IFRS'],	jsonDepreMethodCode,'L_FIA003', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SUB_CODE', 'CODE_NAME', 'Y', ''),
            // 감가상각주기
            gfnma_setComSelect(['FM_DEPRECIATION_PERIOD_GAAP','FM_DEPRECIATION_PERIOD_TAX','FM_DEPRECIATION_PERIOD_IFRS'],	jsonDeprePeriodCode,'L_FIA004', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SUB_CODE', 'CODE_NAME', 'Y', ''),
            
            // 단위
            gfnma_setComSelect([],	jsonUnitCode,	'L_LGS001', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'UNIT_CODE', 'UNIT_NAME', 'Y', ''),
            // 단위
            gfnma_setComSelect([],	jsonAreaUom,	'L_FIM402', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SUB_CODE', 'CODE_NAME', 'Y', ''),
		]);
	}	
...
//     	}
    ...
    function fn_validationMain(){
    	...
    	var p_fm_capital_expenditure	= gfnma_nvl(SBUxMethod.get('FM_CAPITAL_EXPENDITURE')); 
    ...
    function fn_viewFia3100GridMastEvent() {
		...
		if(rowData['DEPRECIATE_YN']=='Y'){
			...
			SBUxMethod.attr('FM_SUBSIDIES_DEPR_ACC', 			'disabled', false);
			$('#BTN_FM_SUBSIDIES_DEPR_ACC').attr('disabled', 	false);
			SBUxMethod.attr('FM_SUBSIDIES_DEPR_ACC_NAME',	'	disabled', false);
			
			SBUxMethod.attr('FM_SUBSIDIES_ACCUM_DEPR_ACC', 		'disabled', false);
			...
			SBUxMethod.attr('FM_SUBSIDIES_DEPR_ACC', 			'disabled', true);
			$('#BTN_FM_SUBSIDIES_DEPR_ACC').attr('disabled', 	true);
			SBUxMethod.attr('FM_SUBSIDIES_DEPR_ACC_NAME',	'	disabled', true);
			
			SBUxMethod.attr('FM_SUBSIDIES_ACCUM_DEPR_ACC', 		'disabled', true);
				...
				if(wtype=='LIST'){
	  	        	...
	  	        	data.cv_1.forEach((item, index) => {
	  					const msg = {
							...
							DEPR_EXP_ACC_NAME				: gfnma_nvl(item.DEPR_EXP_ACC_NAME),	
							DEPRECIATION_PERIOD_GAAP		: gfnma_nvl(item.DEPRECIATION_PERIOD_GAAP),	
							OUT_ACCUMULATED_DEPRECIATION	: gfnma_nvl(item.OUT_ACCUMULATED_DEPRECIATION),	
							SALVAGE_RATE_IFRS				: gfnma_nvl(item.SALVAGE_RATE_IFRS),	
							ASSET_CATEGORY_NAME				: gfnma_nvl(item.ASSET_CATEGORY_NAME),	
							BOOK_VALUE						: gfnma_nvl(item.BOOK_VALUE),	
							ASSET_ACC_NAME					: gfnma_nvl(item.ASSET_ACC_NAME),	
							QTY								: gfnma_nvl(item.QTY),	
							CATEGORY2						: gfnma_nvl(item.CATEGORY2),	
							FI_ORG_CODE						: gfnma_nvl(item.FI_ORG_CODE),	
							CATEGORY3						: gfnma_nvl(item.CATEGORY3),	
							CATEGORY4						: gfnma_nvl(item.CATEGORY4),	
							CATEGORY5						: gfnma_nvl(item.CATEGORY5),	
							OUT_SUBSIDIES_AMOUNT			: gfnma_nvl(item.OUT_SUBSIDIES_AMOUNT),	
							MEMO							: gfnma_nvl(item.MEMO),	
							COMP_CODE						: gfnma_nvl(item.COMP_CODE),	
							SALVAGE_RATE_TAX				: gfnma_nvl(item.SALVAGE_RATE_TAX),	
							DEPRECIATION_METHOD_TAX			: gfnma_nvl(item.DEPRECIATION_METHOD_TAX),	
							SUBSIDIES_ACC_NAME				: gfnma_nvl(item.SUBSIDIES_ACC_NAME),	
							CATEGORY1						: gfnma_nvl(item.CATEGORY1),	
							...
							OUT_ACQUISITION_AMOUNT			: gfnma_nvl(item.OUT_ACQUISITION_AMOUNT),	
							USEFUL_LIFE_MM_TAX				: gfnma_nvl(item.USEFUL_LIFE_MM_TAX),	
							DEPT_CODE						: gfnma_nvl(item.DEPT_CODE),	
							SALVAGE_VALUE_GAAP				: gfnma_nvl(item.SALVAGE_VALUE_GAAP),	
							PROJECT_CODE					: gfnma_nvl(item.PROJECT_CODE),	
							DEPRECIATE_YN					: gfnma_nvl(item.DEPRECIATE_YN),	
							CAPITAL_EXPENDITURE				: gfnma_nvl(item.CAPITAL_EXPENDITURE),	
							DEPR_EXP_ACC					: gfnma_nvl(item.DEPR_EXP_ACC),	
							COST_CENTER_NAME				: gfnma_nvl(item.COST_CENTER_NAME),	
							CS_NAME							: gfnma_nvl(item.CS_NAME),	
							SUBSIDIES_AMOUNT				: gfnma_nvl(item.SUBSIDIES_AMOUNT),	
							DEPRECIATE_START_YYYYMM			: gfnma_nvl(item.DEPRECIATE_START_YYYYMM),	
							DEPRECIATION_PERIOD_IFRS		: gfnma_nvl(item.DEPRECIATION_PERIOD_IFRS),	
							ASSET_ACQUIRE_NO				: gfnma_nvl(item.ASSET_ACQUIRE_NO),	
							ACCUM_DEPR_ACC					: gfnma_nvl(item.ACCUM_DEPR_ACC),	
							LOCATION_NAME					: gfnma_nvl(item.LOCATION_NAME),	
							SALVAGE_RATE_GAAP				: gfnma_nvl(item.SALVAGE_RATE_GAAP),	
							EQUIPMENT_NO					: gfnma_nvl(item.EQUIPMENT_NO),	
							DEPT_NAME						: gfnma_nvl(item.DEPT_NAME),	
							OUT_SUBSIDIES_ACC_DEPRECIATION	: gfnma_nvl(item.OUT_SUBSIDIES_ACC_DEPRECIATION),	
							ASSET_LEVEL2_NAME				: gfnma_nvl(item.ASSET_LEVEL2_NAME),	
							ACCUM_DEPR_ACC_NAME				: gfnma_nvl(item.ACCUM_DEPR_ACC_NAME),	
							COST_CENTER_CODE				: gfnma_nvl(item.ACCUM_DEPR_ACC_NAME),	
							USEFUL_LIFE_MM_IFRS				: gfnma_nvl(item.USEFUL_LIFE_MM_IFRS),	
							...
							SALVAGE_VALUE_IFRS				: gfnma_nvl(item.SALVAGE_VALUE_IFRS),	
							DEPRECIATE_START_YYYYMM_IFRS	: gfnma_nvl(item.DEPRECIATE_START_YYYYMM_IFRS),	
							SUBSIDIES_ACCUM_DEPR_ACC_NAME	: gfnma_nvl(item.SUBSIDIES_ACCUM_DEPR_ACC_NAME),	
							LOCATION_CODE					: gfnma_nvl(item.LOCATION_CODE),	
							ACCUMULATED_DEPRECIATION		: gfnma_nvl(item.ACCUMULATED_DEPRECIATION),	
							SALVAGE_VALUE_TAX				: gfnma_nvl(item.SALVAGE_VALUE_TAX),	
							SUBSIDIES_ACCOUNT				: gfnma_nvl(item.SUBSIDIES_ACCOUNT),	
							CS_CODE							: gfnma_nvl(item.CS_CODE),	
							SUBSIDIES_ACCUM_DEPR_ACC		: gfnma_nvl(item.SUBSIDIES_ACCUM_DEPR_ACC),	
							ASSET_LEVEL3					: gfnma_nvl(item.ASSET_LEVEL3),	
							USEFUL_LIFE_TAX					: gfnma_nvl(item.USEFUL_LIFE_TAX),	
							ASSET_LEVEL2					: gfnma_nvl(item.ASSET_LEVEL2),	
							USEFUL_LIFE_GAAP				: gfnma_nvl(item.USEFUL_LIFE_GAAP),	
							USEFUL_LIFE_MM_GAAP				: gfnma_nvl(item.USEFUL_LIFE_MM_GAAP),	
							SUBSIDIES_DEPR_ACC				: gfnma_nvl(item.SUBSIDIES_DEPR_ACC),	
							ASSET_LEVEL3_NAME				: gfnma_nvl(item.ASSET_LEVEL3_NAME),	
							SITE_CODE						: gfnma_nvl(item.SITE_CODE),	
							SITE_NAME						: gfnma_nvl(item.SITE_NAME),	
							ACQUIRE_DATE					: gfnma_nvl(item.ACQUIRE_DATE),	
							ASSET_ACCOUNT					: gfnma_nvl(item.ASSET_ACCOUNT),	
							SUBSIDIES_DEPR_ACC_NAME			: gfnma_nvl(item.SUBSIDIES_DEPR_ACC_NAME),	
							ASSET_NO						: gfnma_nvl(item.ASSET_NO),	
							SUBSIDIES_ACC_DEPRECIATION		: gfnma_nvl(item.SUBSIDIES_ACC_DEPRECIATION),	
							USEFUL_LIFE_IFRS				: gfnma_nvl(item.USEFUL_LIFE_IFRS),	
							ACTUAL_FLAG						: gfnma_nvl(item.ACTUAL_FLAG),	
							DEPRECIATION_METHOD_GAAP		: gfnma_nvl(item.DEPRECIATION_METHOD_GAAP),	
							DEPRECIATION_PERIOD_TAX			: gfnma_nvl(item.DEPRECIATION_PERIOD_TAX),	
	  					}
	  					tlist.push(msg);
	  					totalRecordCount ++;
	  				});					
	 				...
	 				if(jsonFia3100Mast.length > 0){
	  	        	...
	  	        	data.cv_2.forEach((item, index) => {
	  					const msg = {
							...
							ACCT_RULE_CODE					: gfnma_nvl(item.ACCT_RULE_CODE),	
							ACCT_RULE_NAME					: gfnma_nvl(item.ACCT_RULE_NAME),
							ACQUIRE_QTY						: gfnma_nvl(item.ACQUIRE_QTY),
							ASSET_ACQUIRE_NO				: gfnma_nvl(item.ASSET_ACQUIRE_NO),
							ASSET_NAME						: gfnma_nvl(item.ASSET_NAME),	
							ASSET_SEQ						: gfnma_nvl(item.ASSET_SEQ),	
					...
					Fia3100GridDetail02.rebuild();
	  	        	...
	  	        	data.cv_3.forEach((item, index) => {
	  					const msg = {
							...
							ACCT_RULE_CODE					: gfnma_nvl(item.ACCT_RULE_CODE),	
							ACCT_RULE_NAME					: gfnma_nvl(item.ACCT_RULE_NAME),
							ACQ_DEPR_AMT					: gfnma_nvl(item.ACQ_DEPR_AMT),
							ASSET_CATEGORY					: gfnma_nvl(item.ASSET_CATEGORY),
							ASSET_LEVEL2					: gfnma_nvl(item.ASSET_LEVEL2),
							ASSET_LEVEL3					: gfnma_nvl(item.ASSET_LEVEL3),
							ASSET_NAME						: gfnma_nvl(item.ASSET_NAME),	
							ASSET_NO						: gfnma_nvl(item.ASSET_NO),	
							BEGIN_ACQUISITION_AMOUNT		: gfnma_nvl(item.BEGIN_ACQUISITION_AMOUNT),	
							BEGIN_SUBSIDIES_AMOUNT			: gfnma_nvl(item.BEGIN_SUBSIDIES_AMOUNT),	
							CONFIRM_FLAG					: gfnma_nvl(item.CONFIRM_FLAG),	
							DEPRECIATION_METHOD				: gfnma_nvl(item.DEPRECIATION_METHOD),	
							DEPRECIATION_PERIOD				: gfnma_nvl(item.DEPRECIATION_PERIOD),	
							DEPRECIATION_RATE				: gfnma_nvl(item.DEPRECIATION_RATE),	
							DEPRECIATION_TYPE				: gfnma_nvl(item.DEPRECIATION_TYPE),	
							DEPRECIATION_YYYYMM				: gfnma_nvl(item.DEPRECIATION_YYYYMM),	
							DOC_ID							: gfnma_nvl(item.DOC_ID),	
							END_ACCUM_DEPR					: gfnma_nvl(item.END_ACCUM_DEPR),	
							END_ACQUISITION_AMOUNT			: gfnma_nvl(item.END_ACQUISITION_AMOUNT),	
							END_SUBSIDES_ACCUM_DEPR			: gfnma_nvl(item.END_SUBSIDES_ACCUM_DEPR),	
							END_SUBSIDIES_AMOUNT			: gfnma_nvl(item.END_SUBSIDIES_AMOUNT),	
							IN_ACQUISITION_AMOUNT			: gfnma_nvl(item.IN_ACQUISITION_AMOUNT),	
							IN_SUBSIDIES_AMOUNT				: gfnma_nvl(item.IN_SUBSIDIES_AMOUNT),	
							OUT_ACCUM_DEPR					: gfnma_nvl(item.OUT_ACCUM_DEPR),	
							OUT_ACQUISITION_AMOUNT			: gfnma_nvl(item.OUT_ACQUISITION_AMOUNT),	
							OUT_ACQ_DEPR					: gfnma_nvl(item.OUT_ACQ_DEPR),	
							OUT_SUBSIDIES_ACCUM_DEPR		: gfnma_nvl(item.OUT_SUBSIDIES_ACCUM_DEPR),	
							OUT_SUBSIDIES_AMOUNT			: gfnma_nvl(item.OUT_SUBSIDIES_AMOUNT),	
							OUT_SUBSIDIES_DEPR				: gfnma_nvl(item.OUT_SUBSIDIES_DEPR),	
							SUBSIDIES_DEPR_AMT				: gfnma_nvl(item.SUBSIDIES_DEPR_AMT),	
							USEFUL_LIFE						: gfnma_nvl(item.USEFUL_LIFE),	
							USEFUL_LIFE_MM					: gfnma_nvl(item.USEFUL_LIFE_MM),	
	  					}
	  					tlist.push(msg);
	  					totalRecordCount ++;
	  				});		
					...
					Fia3100GridDetail03.rebuild();
	  	        	...
	  	        	data.cv_4.forEach((item, index) => {
	  					const msg = {
	  						BASE_YYYYMM				: gfnma_nvl(item.BASE_YYYYMM),	
	  						UNIT_PRICE				: gfnma_nvl(item.UNIT_PRICE),
	  						AREA					: gfnma_nvl(item.AREA),
	  						AREA_UOM				: gfnma_nvl(item.AREA_UOM),
...
//         };
        SBGridProperties.columns = [
            ...
            {caption: ["금액(통화)"],		ref: 'ORIGINAL_AMOUNT', 		type:'input',  		width:'170px',  	style:'text-align:right', format : {type:'number', rule:'#,###'}},
            {caption: ["금액(전표)"],		ref: 'FUNCTIONAL_AMOUNT', 		type:'input',  		width:'170px',  	style:'text-align:right', format : {type:'number', rule:'#,###'}},
...
//         };
        SBGridProperties.columns = [
            ...
            {caption: ["공시지가금액"],			ref: 'AMOUNT',				  	type:'input',  		width:'200px',  	style:'text-align:right', format : {type:'number', rule:'#,###'}}, 
    ...
    var fn_compopup4 = function() {
    	...
    	compopup1({
			...
			,itemSelectEvent		: function (data){
				console.log('callback data:', data);
				SBUxMethod.set('FM_ASSET_LEVEL2', 		data.ASSET_LEVEL2);
    ...
    function fn_compopup6() {
        ...
        compopup1({
            ...
            ,itemSelectEvent		: function (data){
                ...
                SBUxMethod.set('FM_ASSET_LEVEL3', 		data.ASSET_LEVEL3);
            },
        });
		SBUxMethod.openModal('modal-compopup1');
    }     
    ...
    var fn_gridPopup1Show = function(row, col, cellData1, cellData2) {
        ...
        compopup1({
            ...
            ,itemSelectEvent		: function (data){
                console.log('callback data:', data);
                Fia3100GridDetail01.setCellData(row, 14, 	data['DEPT_NAME'], true, true);
                Fia3100GridDetail01.setCellData(row, 16, 	data['DEPT_CODE'], true, true);
            },
        });
		SBUxMethod.openModal('modal-compopup1');
    }      
    ...
    var fn_gridPopup2Show = function(row, col, cellData1, cellData2) {
        ...
        compopup1({
            ...
            ,itemSelectEvent		: function (data){
                console.log('callback data:', data);
                Fia3100GridDetail01.setCellData(row, 17, 	data['COST_CENTER_NAME'], true, true);
                Fia3100GridDetail01.setCellData(row, 19, 	data['COST_CENTER_CODE'], true, true);
            },
        });
		SBUxMethod.openModal('modal-compopup1');
    }  
    ...
    const fn_processFia3100S = async function (wtype, callback){
		...
		let p_fm_location_code		= gfnma_nvl(SBUxMethod.get("FM_LOCATION_CODE"));
		...
		let p_fm_subsidies_account	= gfnma_nvl(SBUxMethod.get("FM_SUBSIDIES_ACCOUNT"));
		let p_fm_subsidies_depr_acc	= gfnma_nvl(SBUxMethod.get("FM_SUBSIDIES_DEPR_ACC"));
		let p_fm_subsidies_accum_depr_acc	= gfnma_nvl(SBUxMethod.get("FM_SUBSIDIES_ACCUM_DEPR_ACC"));
