...
<script type="text/javascript">
    ...
    //grid 초기화
    var Fia1200Grid; 			// 그리드를 담기위한 객체 선언
    var jsonFia1200Grid = []; 	// 그리드의 참조 데이터 주소 선언
     ...
     */
 	function cfn_search() {
 		p_sel_rowData = null;    	
    	fn_setFia1200Grid('LIST');
    }
    
    function fn_createFia1200Grid() {
        var SBGridProperties 				= {};
	    SBGridProperties.parentid 			= 'sb_area_grid1';
	    SBGridProperties.id 				= 'Fia1200Grid';
	    SBGridProperties.jsonref 			= 'jsonFia1200Grid';
        ...
        Fia1200Grid = _SBGrid.create(SBGridProperties);
        Fia1200Grid.bind('click', 			'fn_viewFia1200GridEvent');
    }
    //상세정보 보기
    function fn_viewFia1200GridEvent() {
    	
    	let nRow = Fia1200Grid.getRow();
    	let nCol = Fia1200Grid.getCol();
		if (nRow < 1) {
            return;
		}
        let rowData = Fia1200Grid.getRowData(nRow);
		...
		fn_setFia1200Detail('Q1', rowData);
    }  
     ...
     * 목록조회
     */
    const fn_setFia1200Grid = async function(wtype) {
		
  		Fia1200Grid.clearStatus();
    ...
    function cfn_save() {
    	...
    	fn_saveFia1200(strStatus, function(){
    ...
    const fn_saveFia1200 = async function(status, callbackFn) {
        ...
        const postJsonPromise = gfn_postJSON("/fi/ffa/com/insertFia1200.do", {
        	getType				: 'json',
        	workType			: status,
        	cv_count			: '0',
        	params				: gfnma_objectToString(paramObj)
