...
<script type="text/javascript">
	...
	const fn_initSBSelect = async function() {
		...
		let rst = await Promise.all([
			...
			// 차입금유형
			gfnma_setComSelect(['SCH_LOAN_TYPE', 'FM_LOAN_TYPE'],		jsonLoanType, 		'L_FIF003', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),

			// 사업장
			gfnma_setComSelect(['FM_SITE_CODE'],		jsonSiteCode, 		'L_ORG001', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SITE_CD', 'SITE_NM', 'Y', ''),
			// 차입금유형
			...
			// 년간이자일수
			gfnma_setComSelect(['FM_LOAN_STATUS'],		jsonLoanStatus,		'L_FIF004', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
			// 상환거래처계좌
			gfnma_setComSelect(['FM_REPAY_BANK_CODE'],	jsonRepayBankCode,	'L_BANK_CODE', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'BANK_CD', 'BANK_NM', 'Y', ''),
			// 원금상환조건
			gfnma_setComSelect(['FM_REPAY_TERM'],		jsonRepayTerm,		'L_FIF005', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
			// 원금상환일
			gfnma_setComSelect(['FM_REPAY_DD'],			jsonRepayDd,		'L_COM010', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
			// 상환기준
			gfnma_setComSelect(['FM_REPAY_BASE'],		jsonRepayBase,		'L_FIF017', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
			// 이자율구분
			gfnma_setComSelect(['FM_INTEREST_TYPE'],	jsonInterestType,	'L_FIF006', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),

			// 이자지급기준
			gfnma_setComSelect(['FM_INTEREST_REPAY_TYPE'],		jsonInterestRepayType,		'L_FIF007', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
			// 이자일수계산기준
			gfnma_setComSelect(['FM_INTEREST_CALC_DAYS_TYPE'],	jsonInterestCalcDaysType,	'L_FIF008', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
			// 년간이자일수
			gfnma_setComSelect(['FM_INTEREST_CALC_YEAR_TYPE'],	jsonInterestCalcYearType,	'L_FIF039', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
			// 이자기산일기준
			gfnma_setComSelect(['FM_INTEREST_CALC_BASE_DAY'],	jsonInterestCalcBaseDay,	'L_FIF043', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
			//금융기관
			gfnma_setComSelect(['Trl1010GridTop'],	jsonBankCsCode,	'L_CS_BANK', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'BANK_CNPT_CD', 'BANK_CS_NAME', 'Y', ''),
			...
			gfnma_multiSelectInit({
				...
				, callback : function(value) {
					...
					if (_.isEmpty(value) == false){
						...
						SBUxMethod.set('FM_REPAY_BANK_CODE', jsonBankAccountSeq.filter(data => data["BACNT_SEQ"] == value)[0]["BANK_CD"]);
						SBUxMethod.set('FM_REPAY_ACCOUNT_NUM', jsonBankAccountSeq.filter(data => data["BACNT_SEQ"] == value)[0]["BACNT_NO"]);
						SBUxMethod.set('FM_REPAY_ACCOUNT_OWNER', jsonBankAccountSeq.filter(data => data["BACNT_SEQ"] == value)[0]["DSCTN"]);
		...
		compopup1({
			...
			,itemSelectEvent		: function (data){
				if(type=='1'){
					...
					SBUxMethod.set('FM_HDG_CS_CODE', 	data.CNPT_CD);
					SBUxMethod.set('FM_HDG_CS_NAME', 	data.CNPT_NM);
				}
			},
		});
	}
	...
	function fn_compopup2() {
		...
		compopup1({
			...
			,itemSelectEvent		: function (data){
				SBUxMethod.set('FM_BANK_CODE', data.BANK_CD);
				SBUxMethod.set('FM_BANK_NAME', data.BANK_NM);
			},
		});
	}
	...
	function fn_compopup3(type) {
		...
		compopup1({
			...
			,itemSelectEvent		: function (data){
				if(type=='1'){
					SBUxMethod.set('FM_IN_DEPOSIT_CODE', data.DPMNY_NM);
					SBUxMethod.set('FM_IN_DEPOSIT_NAME', data.DPMNY_CD);
				} else if (type=='2'){
					SBUxMethod.set('FM_OUT_DEPOSIT_CODE', data.DPMNY_NM);
					SBUxMethod.set('FM_OUT_DEPOSIT_NAME', data.DPMNY_CD);
				}
			},
		});
	}
	...
	function fn_compopup4(type) {
		...
		compopup1({
			...
			,itemSelectEvent		: function (data){
				if(type=='1'){
					SBUxMethod.set('FM_DEPOSIT_NAME', data.DEPT_NM);
					SBUxMethod.set('FM_DEPOSIT_CODE', data.DEPT_CD);
				} else if(type=='2'){
				}
			},
		});
	...
	var fn_compopup7 = function(type) {
		...
		compopup1({
			...
			,itemSelectEvent		: function (data){
				if(type=='1'){
					SBUxMethod.set('FM_LOAN_ACCOUNT_CODE', 				data.ACNTL_CD);
					SBUxMethod.set('FM_LOAN_ACCOUNT_NAME', 				data.ACNT_NM);
				} else if(type=='2'){
					SBUxMethod.set('FM_INTEREST_EXPENSE_ACCOUNT_CODE', 	data.ACNTL_CD);
					SBUxMethod.set('FM_INTEREST_EXPENSE_ACCOUNT_NAME', 	data.ACNT_NM);
				} else if(type=='3'){
					SBUxMethod.set('FM_PREPAID_ACC_CODE', 				data.ACNTL_CD);
					SBUxMethod.set('FM_PREPAID_ACC_NAME', 				data.ACNT_NM);
				} else if(type=='4'){
					SBUxMethod.set('FM_LONG_PREPAID_ACC_CODE', 			data.ACNTL_CD);
					SBUxMethod.set('FM_LONG_PREPAID_ACC_NAME', 			data.ACNT_NM);
				} else if(type=='5'){
					SBUxMethod.set('FM_ACCRUED_EXPENSE_ACCOUNT_CODE',	data.ACNTL_CD);
					SBUxMethod.set('FM_ACCRUED_EXPENSE_ACCOUNT_NAME',	data.ACNT_NM);
				} else if(type=='6'){
					SBUxMethod.set('FM_PRESENT_VALUE_ACCOUNT_CODE',		data.ACNTL_CD);
					SBUxMethod.set('FM_PRESENT_VALUE_ACCOUNT_NAME',		data.ACNT_NM);
				} else if(type=='7'){
					SBUxMethod.set('FM_LOAN_LIQUID_ACCOUNT_CODE',		data.ACNTL_CD);
					SBUxMethod.set('FM_LOAN_LIQUID_ACCOUNT_NAME',		data.ACNT_NM);
				} else if(type=='8'){
					SBUxMethod.set('FM_PRESENT_VALUE_LIQUID_ACCT_CD',	data.ACNTL_CD);
					SBUxMethod.set('FM_PRESENT_VALUE_LIQUID_ACCT_NM',	data.ACNT_NM);
				} else if(type=='9'){
					SBUxMethod.set('FM_PRESENT_VALUE_INTEREST_ACCT_CD',	data.ACNTL_CD);
					SBUxMethod.set('FM_PRESENT_VALUE_INTEREST_ACCT_NM',	data.ACNT_NM);
				} else if(type=='10'){
					SBUxMethod.set('FM_HDG_GAIN_ACCOUNT_CODE',			data.ACNTL_CD);
					SBUxMethod.set('FM_HDG_GAIN_ACCOUNT_NAME',			data.ACNT_NM);
				} else if(type=='11'){
					SBUxMethod.set('FM_HDG_ACCOUNT_CODE',				data.ACNTL_CD);
					SBUxMethod.set('FM_HDG_ACCOUNT_NAME',				data.ACNT_NM);
				}
			},
		});
	}
	...
	var fn_compopup9 = function(cellData1, cellData2, cellData3) {
		...
		if(p_cellData2){
			strWhereClause += " AND '" + replaceText1 + "' BETWEEN A.EFCT_BGNG_YMD AND A.EFCT_END_YMD";
		}
		...
		compopup1({
			...
			,searchInputFields		: ["CNPT_CD", 			"PAY_PRNMNT_YMD"]
			...
			,itemSelectEvent		: function (data){
				SBUxMethod.set('FM_REPAY_BANK_CS_CODE', 	data.CNPT_CD);
				SBUxMethod.set('FM_REPAY_BANK_CODE', 		data.BANK_CD);
			},
		});
	...
	var fn_compopup10 = function(type) {
		...
		compopup1({
			...
			,itemSelectEvent		: function (data){
				if(type=='1'){
					SBUxMethod.set('FM_EXCHANGE_GAIN_ACC_CODE', 	data.ACNTL_CD);
					SBUxMethod.set('FM_EXCHANGE_GAIN_ACC_NAME', 	data.ACNT_NM);
				} else if(type=='2'){
					SBUxMethod.set('FM_EXCHANGE_LOSS_ACC_CODE', 	data.ACNTL_CD);
					SBUxMethod.set('FM_EXCHANGE_LOSS_ACC_NAME', 	data.ACNT_NM);
				} else if(type=='3'){
					SBUxMethod.set('FM_VAL_GAIN_ACC_CODE', 	data.ACNTL_CD);
					SBUxMethod.set('FM_VAL_GAIN_ACC_NAME', 	data.ACNT_NM);
				} else if(type=='4'){
					SBUxMethod.set('FM_VAL_LOSS_ACC_CODE', 	data.ACNTL_CD);
					SBUxMethod.set('FM_VAL_LOSS_ACC_NAME', 	data.ACNT_NM);
				} else if(type=='5'){
					SBUxMethod.set('FM_HDG_LOSS_ACCOUNT_CODE', 	data.ACNTL_CD);
					SBUxMethod.set('FM_HDG_LOSS_ACCOUNT_NAME', 	data.ACNT_NM);
				}
			},
		});
	}
	...
	async function fn_view() {
		...
		gfnma_multiSelectSet('#FM_REPAY_SEQ', 'SBSD_CD', 'CD_NM', tmpBankAcctSeq);
		...
		try {
			...
			if (_.isEqual("S", data.resultStatus)) {
				...
				data.cv_2.forEach((item, index) => {
					...
					SBUxMethod.set("FM_LOAN_NUM"      		, gfn_nvl(item.BRW_NO));
					SBUxMethod.set("FM_FI_ORG_CODE"      	, gfn_nvl(item.ACNTG_OGNZ_CD));
					SBUxMethod.set("FM_SITE_CODE"      		, gfn_nvl(item.SITE_CD));
					SBUxMethod.set("FM_LOAN_NAME"      		, gfn_nvl(item.BRW_NM));
					SBUxMethod.set("FM_LOAN_TYPE"      		, gfn_nvl(item.BRW_TYPE));
					SBUxMethod.set("FM_BANK_CS_CODE"       	, gfn_nvl(item.BANK_CNPT_CD));
					SBUxMethod.set("FM_BANK_CS_NAME"       	, gfn_nvl(item.BANK_CS_NAME));
					SBUxMethod.set("FM_BANK_CODE"      		, gfn_nvl(item.BANK_CD));
					SBUxMethod.set("FM_BANK_NAME"      		, gfn_nvl(item.BANK_NM));
					SBUxMethod.set("FM_LOAN_CATEGORY1"     	, gfn_nvl(item.LOAN_CTGRY1));
					SBUxMethod.set("FM_LOAN_CATEGORY2"     	, gfn_nvl(item.LOAN_CTGRY2));
					SBUxMethod.set("FM_LOAN_CATEGORY3"     	, gfn_nvl(item.LOAN_CTGRY3));
					SBUxMethod.set("FM_CURRENCY_CODE"      	, gfn_nvl(item.CRN_CD));
					SBUxMethod.set("FM_EXCHANGE_RATE"      	, gfn_nvl(item.EXCHRT));
					SBUxMethod.set("FM_LOAN_AMT"      		, gfn_nvl(item.LOAN_AMT));
					SBUxMethod.set("FM_LOAN_AMT_KRW"      	, gfn_nvl(item.LOAN_AMT_KRW));
					SBUxMethod.set("FM_IN_DEPOSIT_CODE"    	, gfn_nvl(item.DPST_SVG_CD));
					SBUxMethod.set("FM_IN_DEPOSIT_NAME"    	, gfn_nvl(item.IN_DEPOSIT_NAME));
					SBUxMethod.set("FM_IN_ACCOUNT_NUM"     	, gfn_nvl(item.RPMT_ACTNO));
					SBUxMethod.set("FM_OUT_DEPOSIT_CODE"   	, gfn_nvl(item.TKMNY_DPMNY_CD));
					...
					SBUxMethod.set("FM_LOAN_DATE"      		, gfn_nvl(item.BRW_YMD));
					SBUxMethod.set("FM_EXPIRE_DATE"      	, gfn_nvl(item.MTRY_YMD));
					SBUxMethod.set("FM_LOAN_STATUS"      	, gfn_nvl(item.BRW_STTS));
					SBUxMethod.set("FM_DESCR"      			, gfn_nvl(item.DSCTN));
					...
					SBUxMethod.set("FM_PREPAID_ACC_CODE"      				, gfn_nvl(item.PRPY_CST_ACNT_CD));
					SBUxMethod.set("FM_PREPAID_ACC_NAME"      				, gfn_nvl(item.PREPAID_ACC_NAME));
					SBUxMethod.set("FM_ACCRUED_EXPENSE_ACCOUNT_CODE"      	, gfn_nvl(item.ACCEXP_ACNT));
					SBUxMethod.set("FM_ACCRUED_EXPENSE_ACCOUNT_NAME"      	, gfn_nvl(item.ACCRUED_EXPENSE_ACCOUNT_NAME));
					SBUxMethod.set("FM_PRESENT_VALUE_ACCOUNT_CODE"      	, gfn_nvl(item.PRSTVL_DSCNT_ACNT_CD));
					SBUxMethod.set("FM_PRESENT_VALUE_ACCOUNT_NAME"      	, gfn_nvl(item.PRESENT_VALUE_ACCOUNT_NAME));
					SBUxMethod.set("FM_LOAN_LIQUID_ACCOUNT_CODE"      		, gfn_nvl(item.LOAN_LQRPLCMT_ACNT_CD));
					SBUxMethod.set("FM_LOAN_LIQUID_ACCOUNT_NAME"      		, gfn_nvl(item.LOAN_LIQUID_ACCOUNT_NAME));
					SBUxMethod.set("FM_LOAN_REPAY_TR_CODE"      			, gfn_nvl(item.BRW_INT_RPMT_FUND_TYPE)); //--차입금상환 (신규))
					SBUxMethod.set("FM_LOAN_REPAY_TR_NAME"      			, gfn_nvl(item.LOAN_REPAY_TR_NAME));
					SBUxMethod.set("FM_INTEREST_REPAY_TR_CODE"      		, gfn_nvl(item.INT_RPMT_FUND_TYPE)); // --이자상환 (신규))
					SBUxMethod.set("FM_INTEREST_REPAY_TR_NAME"      		, gfn_nvl(item.INTEREST_REPAY_TR_NAME));
					SBUxMethod.set("FM_LOAN_START_TR_CODE"      			, gfn_nvl(item.BRW_FUND_TYPE)); //--차입금차입 (신규))
					SBUxMethod.set("FM_LOAN_START_TR_NAME"      			, gfn_nvl(item.LOAN_START_TR_NAME));
					SBUxMethod.set("FM_EXCHANGE_GAIN_ACC_CODE"     			, gfn_nvl(item.EXCHRT_PRF_ACNT)); // -- ADD 20160804 외화환차익계)정
					SBUxMethod.set("FM_EXCHANGE_GAIN_ACC_NAME"      		, gfn_nvl(item.EXCHANGE_GAIN_ACC_NAME));
					SBUxMethod.set("FM_EXCHANGE_LOSS_ACC_CODE"      		, gfn_nvl(item.EXCHRT_LOSS_ACNT)); //-- ADD 20160804 외화환차손계)정
					SBUxMethod.set("FM_EXCHANGE_LOSS_ACC_NAME"      		, gfn_nvl(item.EXCHANGE_LOSS_ACC_NAME));
					SBUxMethod.set("FM_VAL_GAIN_ACC_CODE"      				, gfn_nvl(item.CNVRT_PRF_ACNT_CD)); // -- ADD 20160804 외화평가이익계)정
					SBUxMethod.set("FM_VAL_GAIN_ACC_NAME"      				, gfn_nvl(item.VAL_GAIN_ACC_NAME));
					SBUxMethod.set("FM_VAL_LOSS_ACC_CODE"      				, gfn_nvl(item.CNVRT_LOSS_ACNT_CD)); // -- ADD 20160804 외화평가손실계)정
					...
					/**************************** 상환 정보 **********************************/
					SBUxMethod.set("FM_REPAY_BANK_CS_CODE"      				, gfn_nvl(item.RPMT_BANK_CNPT_CD));
					SBUxMethod.set("FM_REPAY_BANK_CS_NAME"      				, gfn_nvl(item.REPAY_BANK_CS_NAME));
					SBUxMethod.set("FM_REPAY_BANK_CODE"      					, gfn_nvl(item.RPMT_BANK_CD));
					SBUxMethod.set("FM_REPAY_ACCOUNT_NUM"      					, gfn_nvl(item.RPMT_ACTNO));
					SBUxMethod.set("FM_REPAY_ACCOUNT_OWNER"      				, gfn_nvl(item.RPMT_DPSTR));
					SBUxMethod.set("FM_REPAY_AMT"      							, gfn_nvl(item.RPMT_AMT));
					SBUxMethod.set("FM_REMAIN_LOAN_AMT"      					, gfn_nvl(item.NRPY_AMT));
					SBUxMethod.set("FM_REPAY_TERM"      						, gfn_nvl(item.PRN_RPMT_TERM));
					SBUxMethod.set("FM_DEFERRED_MM"      						, gfn_nvl(item.DFRMNT_MM));
					SBUxMethod.set("FM_REPAY_CYCLE_MM"      					, gfn_nvl(item.RPMT_CYCL_MM));
					SBUxMethod.set("FM_REPAY_START_DATE"      					, gfn_nvl(item.RPMT_FRST_YMD));
					SBUxMethod.set("FM_REPAY_PER_AMT"      						, gfn_nvl(item.PONE_RPMT_AMT));
					SBUxMethod.set("FM_REPAY_FIRST_AMT"      					, gfn_nvl(item.PRN_RPMT_FRST_AMT));
					SBUxMethod.set("FM_REPAY_DD"      							, gfn_nvl(item.PRN_RPMT_YMD));
					SBUxMethod.set("FM_REPAY_BASE"      						, gfn_nvl(item.RPMT_BASE));
					SBUxMethod.set("FM_INTEREST_TYPE"      						, gfn_nvl(item.INT_RT_TYPE));
					SBUxMethod.set("FM_INTEREST_RATE"      						, gfn_nvl(item.INT_RT));
					SBUxMethod.set("FM_INTEREST_REPAY_TYPE"      				, gfn_nvl(item.INT_RPMT_TYPE));
					SBUxMethod.set("FM_INTEREST_CALC_DAYS_TYPE"      			, gfn_nvl(item.INT_CAL_DCNT_TYPE));
					SBUxMethod.set("FM_INTEREST_DEFERRED_MM"      				, gfn_nvl(item.INT_DFMT_MM));
					...
					SBUxMethod.set("FM_INSERT_USERID"      						, gfn_nvl(item.WRT_USER_ID));
					SBUxMethod.set("FM_INTEREST_CALC_YEAR_TYPE"      			, gfn_nvl(item.INT_CAL_YR_TYPE));
					SBUxMethod.set("FM_INTEREST_CALC_BASE_DAY"      			, gfn_nvl(item.INT_CAL_CRTR_DCNT));
					SBUxMethod.set("FM_AMORTIZE_BASE_DATE"      				, gfn_nvl(item.AMTZ_CRTR_YMD));
					...
					SBUxMethod.set("FM_HDG_GAIN_ACCOUNT_CODE"      				, gfn_nvl(item.DRSEC_GNLS_ACNT));
					SBUxMethod.set("FM_HDG_GAIN_ACCOUNT_NAME"      				, gfn_nvl(item.HDG_GAIN_ACCOUNT_NAME));
					SBUxMethod.set("FM_HDG_LOSS_ACCOUNT_CODE"      				, gfn_nvl(item.DRSEC_LOSS_ACNT));
					SBUxMethod.set("FM_HDG_LOSS_ACCOUNT_NAME"      				, gfn_nvl(item.HDG_LOSS_ACCOUNT_NAME));
					SBUxMethod.set("FM_HDG_CS_CODE"      						, gfn_nvl(item.DRSEC_CNPT_CD));
					SBUxMethod.set("FM_HDG_CS_NAME"      						, gfn_nvl(item.HDG_CS_NAME));
					SBUxMethod.set("FM_HDG_DESCR"      							, gfn_nvl(item.DRSEC_DSCTN));
					SBUxMethod.set("FM_HDG_ACCOUNT_CODE"      					, gfn_nvl(item.DRSEC_ACC));
