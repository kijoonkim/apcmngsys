...
<script type="text/javascript">
    ...
    const fn_initSBSelect = async function() {
        let rst = await Promise.all([
            // APC명
            gfnma_setComSelect(['SCH_FI_ORG_CODE', 'Trl2010GridTop'],		jsonFiOrgCode, 		'L_FIM022', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'ACNTG_OGNZ_CD', 'ACNTG_OGNZ_NM', 'Y', ''),
            // 차입금거래유형
            gfnma_setComSelect(['SCH_TXN_TYPE','Trl2010GridTop'], jsonTxnType, 		'L_FIM083', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
            //금융기관
            gfnma_setComSelect(['Trl2010GridTop'],	jsonBankCsCode,	'L_CS_BANK', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'BANK_CNPT_CD', 'BANK_CS_NAME', 'Y', ''),
            //지급방법
            gfnma_setComSelect(['Trl2010GridTop'],	jsonPayMethod,	'L_FIM073_LOAN', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
            //원가중심점
            gfnma_setComSelect(['Trl2010GridTop'],	jsonCostCenterCode,	'L_COST_CENTER', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
            //거래유형
            gfnma_setComSelect(['Trl2010GridTab1'],	jsonTxnGroupCode,	'L_FIM071', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
            //출금은행
            gfnma_setComSelect(['Trl2010GridTab1'],	jsonBankCode,	'L_BANK_CODE', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'BANK_CD', 'BANK_NM', 'Y', ''),
            //거래처
            gfnma_setComSelect(['Trl2010GridTab1'],	jsonPayerId,	'L_COMP_ID', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'CNPT_CD', 'CNPT_NM', 'Y', ''),
            //전표유형
            gfnma_setComSelect(['Trl2010GridTab1'],	jsonDocType,	'L_FIM051', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
            //지급방법
            gfnma_setComSelect(['Trl2010GridTab1'],	jsonPayMethodCode,	'L_FIM081', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
            //지급방식
            gfnma_setComSelect(['Trl2010GridTab1'],	jsonTrPayMethod,	'L_FIM073', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
            //상태
            gfnma_setComSelect(['Trl2010GridTab1'],	jsonStatusCode,	'L_FIM074', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
...
//     	}
    ...
    function fn_compopup1() {
        ...
        compopup1({
            ...
            ,itemSelectEvent		: function (data){
                SBUxMethod.set('SCH_OUT_ACCOUNT_NO', data.DPMNY_CD);
                SBUxMethod.set('SCH_OUT_ACCOUNT_NM', data.ACTNO);
            },
        });
    }
    ...
    function fn_compopup2(type) {
        ...
        compopup1({
            ...
            ,itemSelectEvent		: function (data){
                if(type=='1'){
                    SBUxMethod.set('SCH_BANK_CODE', 	data.CNPT_CD);
                    SBUxMethod.set('SCH_BANK_NAME', 	data.CNPT_NM);
                } else if(type=='2'){
                }
            },
        });
            ...
            if (_.isEqual("S", data.resultStatus)) {
                ...
                data.cv_1.forEach((item, index) => {
                    const msg = {
                        ...
                        PERIOD_CODE        	        : gfn_nvl(item.PRD_CD),
                        TXN_TYPE        	        : gfn_nvl(item.TRSC_TYPE),   //--3:이자상환
                        REPAY_PLAN_DATE             : gfn_nvl(item.RPMT_PLAN_YMD),
                        OUT_DEPOSIT_CODE            : gfn_nvl(item.TKMNY_DPMNY_CD),
                        OUT_ACCOUNT_NO              : gfn_nvl(item.OUT_ACCOUNT_NO),
                        BANK_CS_CODE                : gfn_nvl(item.BANK_CNPT_CD),
                        ACCOUNT_NUM        	        : gfn_nvl(item.ACTNO),
                        ...
                        REMAIN_LOAN_AMT_O        	: gfn_nvl(item.NRPY_AMT1),
                        REPAY_PLAN_AMT_BEFORE       : gfn_nvl(item.PRN_RPMT_AMT_BFR),
                        FOREIGN_EXCHANGE_PL        	: gfn_nvl(item.EXCHRT_GNLS),
                        ...
                        REPAY_PLAN_AMT_F        	: gfn_nvl(item.RPMT_PLAN_AMT_CNVS),
                        REMAIN_LOAN_AMT_F        	: gfn_nvl(item.NRPY_AMT_CNVS),
                        INTEREST_REPAY_PLAN_AMT_F   : gfn_nvl(item.INT_PRNMNT_AMT_ORGNL),
                ...
                if (_.isEqual(strWorkType, 'DETAIL')){
                    ...
                    data.cv_2.forEach((item, index) => {
                        const msg = {
                            ...
                            TREASURY_BATCH_NO   : gfn_nvl(item.FUND_BTCH_NO),  //--배치번호
                            TREASURY_LINE_NUM   : gfn_nvl(item.FUND_LINE_NO),  //--순번
                            TXN_GROUP_CODE      : gfn_nvl(item.TRSC_GROUP_CD),     //--거래유형
                            ACCOUNT_NUM        	: gfn_nvl(item.ACTNO),        //--출금계좌
                            BANK_CODE        	: gfn_nvl(item.BANK_CD),          //--은행코드
                            PAYER_ID        	: gfn_nvl(item.PMPL_ID),           // --거래처
                            DOC_TYPE        	: gfn_nvl(item.SLIP_TYPE),           // --전표유형
                            PLANNED_PAY_DATE    : gfn_nvl(item.PLAN_PAY_YMD),   // --출금예정일
                            PAY_DATE        	: gfn_nvl(item.PAY_YMD),           //--출금일
                            PAY_METHOD        	: gfn_nvl(item.PAY_MTHD),         //--지급방법
                            TR_PAY_METHOD       : gfn_nvl(item.TR_PAY_METHOD),      //--지급방법
                            CURRENCY_CODE       : gfn_nvl(item.CRN_CD),      // --통화
                            EXCHANGE_TYPE       : gfn_nvl(item.EXCHRT_TYPE),      //--환산유형
                            EXCHANGE_RATE       : gfn_nvl(item.EXCHRT),      //--환율
                            ORIGINAL_CR_AMT     : gfn_nvl(item.ORGNL_CRSD_AMT),    //---원래금액
                            FUNCTIONAL_CR_AMT   : gfn_nvl(item.CNVS_CRSD_AMT),  //--환산금액
                    ...
                    data.cv_1.forEach((item, index) => {
                        const msg = {
                            ...
                            VAT_NAME        	: gfn_nvl(item.VAT_NM),
                            DEPT_CODE        	: gfn_nvl(item.DEPT_CD),
                            COST_CENTER_CODE    : gfn_nvl(item.CSTCD_CD),
                            PROJECT_CODE        : gfn_nvl(item.PJT_CD),
                            ORIGINAL_CR_AMT     : gfn_nvl(item.ORGNL_CRSD_AMT),
                            ORIGINAL_DR_AMT     : gfn_nvl(item.ORGNL_DRSD_AMT),
                            FUNCTIONAL_CR_AMT   : gfn_nvl(item.CNVS_CRSD_AMT),
                            FUNCTIONAL_DR_AMT   : gfn_nvl(item.CNVS_DRSD_AMT),
                            TXN_QTY        	    : gfn_nvl(item.TRSC_QTY),
                            ACCOUNT_CODE        : gfn_nvl(item.ACNTL_CD),
                            ACCOUNT_NAME        : gfn_nvl(item.ACNT_NM),
                            ACC_ITEM_CODE1      : gfn_nvl(item.ACNT_MNG_ARTCL_CD1),
                            ACC_ITEM_CODE2      : gfn_nvl(item.ACNT_MNG_ARTCL_CD2),
                            ACC_ITEM_CODE3      : gfn_nvl(item.ACNT_MNG_ARTCL_CD3),
                            ACC_ITEM_CODE4      : gfn_nvl(item.ACNT_MNG_ARTCL_CD4),
                            ACC_ITEM_CODE5      : gfn_nvl(item.ACNT_MNG_ARTCL_CD5),
                            ACC_ITEM_CODE6      : gfn_nvl(item.ACNT_MNG_ARTCL_CD6),
                            ACC_ITEM_CODE7      : gfn_nvl(item.ACNT_MNG_ARTCL_CD7),
                            ACC_ITEM_CODE8      : gfn_nvl(item.ACNT_MNG_ARTCL_CD8),
                            ACC_ITEM_CODE9      : gfn_nvl(item.ACNT_MNG_ARTCL_CD9),
                            ACC_ITEM_CODE10     : gfn_nvl(item.ACNT_MNG_ARTCL_CD10),
                            ACC_ITEM_NAME1      : gfn_nvl(item.ACNT_MNG_ARTCL_NM1),
                            ACC_ITEM_NAME2      : gfn_nvl(item.ACNT_MNG_ARTCL_NM2),
                            ACC_ITEM_NAME3      : gfn_nvl(item.ACNT_MNG_ARTCL_NM3),
                            ACC_ITEM_NAME4      : gfn_nvl(item.ACNT_MNG_ARTCL_NM4),
                            ACC_ITEM_NAME5      : gfn_nvl(item.ACNT_MNG_ARTCL_NM5),
                            ACC_ITEM_NAME6      : gfn_nvl(item.ACNT_MNG_ARTCL_NM6),
                            ACC_ITEM_NAME7      : gfn_nvl(item.ACNT_MNG_ARTCL_NM7),
                            ACC_ITEM_NAME8      : gfn_nvl(item.ACNT_MNG_ARTCL_NM8),
                            ACC_ITEM_NAME9      : gfn_nvl(item.ACNT_MNG_ARTCL_NM9),
                            ACC_ITEM_NAME10     : gfn_nvl(item.ACNT_MNG_ARTCL_NM10),
                            ACC_ITEM_YN1        : gfn_nvl(item.ACNT_MNG_ARTCL_YN1),   //-- 필수여
                            ACC_ITEM_YN2        : gfn_nvl(item.ACNT_MNG_ARTCL_YN2),
                            ACC_ITEM_YN3        : gfn_nvl(item.ACNT_MNG_ARTCL_YN3),
                            ACC_ITEM_YN4        : gfn_nvl(item.ACNT_MNG_ARTCL_YN4),
                            ACC_ITEM_YN5        : gfn_nvl(item.ACNT_MNG_ARTCL_YN5),
                            ACC_ITEM_YN6        : gfn_nvl(item.ACNT_MNG_ARTCL_YN6),
                            ACC_ITEM_YN7        : gfn_nvl(item.ACNT_MNG_ARTCL_YN7),
                            ACC_ITEM_YN8        : gfn_nvl(item.ACNT_MNG_ARTCL_YN8),
                            ACC_ITEM_YN9        : gfn_nvl(item.ACNT_MNG_ARTCL_YN9),
                            ACC_ITEM_YN10       : gfn_nvl(item.ACNT_MNG_ARTCL_YN10),
                            ...
                            ACC_CHARACTER       : gfn_nvl(item.INCPL_TYPE),
                            ACC_ITEM_VALUE1     : gfn_nvl(item.ACNT_MNG_ARTCL_VL1),
                            ACC_ITEM_VALUE2     : gfn_nvl(item.ACNT_MNG_ARTCL_VL2),
                            ACC_ITEM_VALUE3     : gfn_nvl(item.ACNT_MNG_ARTCL_VL3),
                            ACC_ITEM_VALUE4     : gfn_nvl(item.ACNT_MNG_ARTCL_VL4),
                            ACC_ITEM_VALUE5     : gfn_nvl(item.ACNT_MNG_ARTCL_VL5),
                            ACC_ITEM_VALUE6     : gfn_nvl(item.ACNT_MNG_ARTCL_VL6),
                            ACC_ITEM_VALUE7     : gfn_nvl(item.ACNT_MNG_ARTCL_VL7),
                            ACC_ITEM_VALUE8     : gfn_nvl(item.ACNT_MNG_ARTCL_VL8),
                            ACC_ITEM_VALUE9     : gfn_nvl(item.ACNT_MNG_ARTCL_VL9),
                            ACC_ITEM_VALUE10    : gfn_nvl(item.ACNT_MNG_ARTCL_VL10),
                            ...
                            ITEM_CODE        	: gfn_nvl(item.ITEM_CD),
                            UOM        	        : gfn_nvl(item.UNIT),
                            TXN_QTY        	    : gfn_nvl(item.TRSC_QTY),
