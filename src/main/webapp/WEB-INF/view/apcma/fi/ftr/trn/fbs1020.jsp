        ...
        try {
            if (_.isEqual("S", data.resultStatus)) {
                ...
                data.cv_1.forEach((item, index) => {
                    const msg = {
                        SERVICE_TYPE : item.SRVC_TYPE,
                        FBS_SERVICE : item.FBS_SRVC,
                        FBS_TXN_TYPE : item.FBS_WORK_APLY_TYPE,
                        ELE_CODE : item.ELE_CODE,
                        ELE_NAME : item.ELE_NAME,
                        WORK_CODE : item.JOB_CD,
                        WORK_NAME : item.WORK_NAME,
                        FBS_TXN_NAME : item.FBS_TXN_NAME,
                        FBS_DATA_NO : item.FBS_DATA_NO,
                        DATA_CODE : item.DATA_CD,
                        DATA_LENGTH : item.DATA_LNGTH,
                        CONV_LENGTH : item.CNVS_LNGTH,
                        DATA_TYPE : item.DATA_TYPE,
                        BANK_REQUIRED_YN : item.BANK_REQ_YN,
                        CUSTOMER_REQUIRED_YN : item.CUST_RQST_YN,
                        DESCR : item.DSCTN,
                        OPTIONAL_YN : item.CHC_YN,
                        COLUMN_NAME : item.COL_NM,
                        DEFAULT_VALUE : item.BSS_VL,
                        HANGUL_YN : item.KORN_YN,
                        PAD_TYPE : item.FILL_TYPE,
                        FILL_VALUE : item.FILL_VL,
