...
<script type="text/javascript">
    ...
    const fn_importExcelData = async function (e) {
        ...
        for (const item of headers) {
            ...
            }else if (_.isEqual('기준소득월액', item)){
                jsonHeaders.push('BASE_INCOME_AMT');
            }else if (_.isEqual('월보험료(계)', item)){
                jsonHeaders.push('TOTAL_INSURE_AMT');
            }else if (_.isEqual('사용자부담금', item)){
                jsonHeaders.push('COMP_INSURE_AMT');
            }else if (_.isEqual('본인기여금', item)){
            ...
            }else if (_.isEqual('급여(직원)', item)){
                jsonHeaders.push('EMP_PAY_AMT');
            }else if (_.isEqual('급여항목(회사)', item)){ //jsonPayItem
                jsonHeaders.push('COMP_PAY_ITEM');
            }else if (_.isEqual('급여(회사)', item)){
    ...
    const fn_exportData = function () {
        ...
        if(gfn_comConfirm("Q0000","엑셀의 양식을 xlsx으로 다운로드 받으시겠습니까?")){
            ...
            const msg = {
                arrRemoveCols   : [0,5,14,15],
                combolabel      : true,
                sheetName       : "국민연금 내역 등록"
            }

            gvwListGrid.exportData("xlsx", "국민연금 내역 등록", true, msg);
        }
