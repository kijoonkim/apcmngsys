<%
    /**
     * @Class Name        : hrw5400.jsp
     * @Description       : 고용보험 내역 관리 정보 화면
     * @author            : 인텔릭아이앤에스
     * @since             : 2024.07.02
     * @version           : 1.0
...
<html lang="ko">
<head>
    <title>title : 고용보험 내역 관리 정보</title>
...
<script type="text/javascript">
    ...
    var tabJsonData = [
        { "id" : "0", "pid" : "-1", "order" : "1", "text" : "고용보험 내역 등록",     "targetid" : "tabInfo1" , "targetvalue" : "고용보험 내역 등록"},
        { "id" : "1", "pid" : "-1", "order" : "2", "text" : "고용보험 집계"    ,     "targetid" : "tabInfo2" , "targetvalue" : "고용보험 집계"}
    ];
    ...
    function fn_createGrid(chMode, rowData) {
        ...
        SBGridProperties.columns = [
            ...
            {caption: ['급여년월'], ref: 'PAY_YYYYMM', width:'100px',	type: 'inputdate', style: 'text-align: center', sortable: false,
    ...
    const fn_search = async function (workType) {
        ...
        const postJsonPromise = gfn_postJSON("/hr/hrp/pay/selectHrp5400List.do", {
            getType: 'json',
            workType: workType,
            cv_count: '3',
            ...
            if (_.isEqual("S", data.resultStatus)) {
                ...
                data.cv_1.forEach((item, index) => {
                    const msg = {
                        ...
                        COMP_STABILITY_AMT       : gfn_nvl(item.BIZ_HIRE_INSRNC_AMT),
                        EMP_INSURE_YE_AMT        : gfn_nvl(item.UNEMPSLRY_YE_AMT),
                        COMP_INSURE_YE_AMT       : gfn_nvl(item.CO_LTRM_RCPR_YRTXCAL_AMT),
                        STABILITY_INCOME_YE      : gfn_nvl(item.HIRE_STBLTY_INCM_YRTXCAL),
                        EMP_INSURE_ADJ_AMT       : gfn_nvl(item.UNEMPSLRY_AJMT_AMT),
                        COMP_INSURE_ADJ_AMT      : gfn_nvl(item.CO_UNEMPSLRY_AJMT_AMT),
                        STABILITY_INCOME_ADJ     : gfn_nvl(item.HIRE_STBLTY_INCM_AJMT),
                        UNEMPLOY_INCOME          : gfn_nvl(item.UNEMPSLRY_CAL),
                        STABILITY_INCOME         : gfn_nvl(item.HIRE_STBLTY_INCM),
                ...
                data.cv_3.forEach((item, index) => {
                    const msg = {
                        ...
                        EMP_INSURE_AMT          : gfn_nvl(item.EMP_INSURE_AMT),          //산정실업급여(사원)
                        COMP_INSURE_AMT         : gfn_nvl(item.COMP_INSURE_AMT),         //산정실업급여(회사)
                        COMP_STABILITY_AMT      : gfn_nvl(item.COMP_STABILITY_AMT),      //사업주고안직능
                        EMP_INSURE_YE_AMT       : gfn_nvl(item.EMP_INSURE_YE_AMT),       //연말실업급여(사원)
                        COMP_INSURE_YE_AMT      : gfn_nvl(item.COMP_INSURE_YE_AMT),      //연말실업급여(회사)
                        STABILITY_INCOME_YE     : gfn_nvl(item.STABILITY_INCOME_YE),     //고안직능(연말)
                        EMP_INSURE_ADJ_AMT      : gfn_nvl(item.EMP_INSURE_ADJ_AMT),      //정산실업급여(사원)
                        COMP_INSURE_ADJ_AMT     : gfn_nvl(item.COMP_INSURE_ADJ_AMT),     //정산실업급여(회사)
                        STABILITY_INCOME_ADJ    : gfn_nvl(item.STABILITY_INCOME_ADJ),    //고안직능(정산)
    ...
    const fn_btnApply = async function (reData, type) {
        ...
        const postJsonPromise = gfn_postJSON("/hr/hrp/pay/insertHrp5400S1.do", {
            getType: 'json',
            workType: type,
            cv_count: '0',
            params: gfnma_objectToString(paramObj)
    ...
    const fn_importExcelData = async function (e){
        ...
        for (const item of headers) {
            ...
            if (_.isEqual('사번', item)){
                ...
                jsonHeaders.push('EMP_INSURE_AMT');
            }else if (_.isEqual('산정실업급여(회사)', item)){
                jsonHeaders.push('COMP_INSURE_AMT');
            }else if (_.isEqual('사업주고안직능', item)){
                jsonHeaders.push('COMP_STABILITY_AMT');
            }else if (_.isEqual('실업급여산정', item)){
                jsonHeaders.push('UNEMPLOY_INCOME');
            }else if (_.isEqual('고안직능산정', item)){
                jsonHeaders.push('STABILITY_INCOME');
            }else if (_.isEqual('연말실업급여(사원)', item)){
                jsonHeaders.push('EMP_INSURE_YE_AMT');
            }else if (_.isEqual('연말실업급여(회사)', item)){
                jsonHeaders.push('COMP_INSURE_YE_AMT');
            }else if (_.isEqual('고안직능(연말)', item)){
                jsonHeaders.push('STABILITY_INCOME_YE');
            }else if (_.isEqual('정산실업급여(사원)', item)){
                jsonHeaders.push('EMP_INSURE_ADJ_AMT');
            }else if (_.isEqual('정산실업급여(회사)', item)){
                jsonHeaders.push('COMP_INSURE_ADJ_AMT');
            }else if (_.isEqual('고안직능(정산)', item)){
                jsonHeaders.push('STABILITY_INCOME_ADJ');
            ...
            }else if (_.isEqual('급여년월', item)){     //yyyy-mm
            ...
            }else if (_.isEqual('급여항목(직원)', item)){         //jsonPayItem
                jsonHeaders.push('EMP_PAY_ITEM');
            }else if (_.isEqual('고용보험료(사원)', item)){
                jsonHeaders.push('EMP_PAY_AMT');
            }else if (_.isEqual('고용보험급여항목(회사)', item)){   //jsonPayItem
                jsonHeaders.push('COMP_PAY_ITEM');
            }else if (_.isEqual('고용보험료(회사)', item)){
                jsonHeaders.push('COMP_PAY_AMT');
            }else if (_.isEqual('연말고용보험급여항목(사원)', item)){   //jsonPayItem
                jsonHeaders.push('EMP_YE_PAY_ITEM');
            }else if (_.isEqual('연말정산고용보험료(사원)', item)){
                jsonHeaders.push('EMP_YE_PAY_AMT');
            }else if (_.isEqual('연말고용보험급여항목(회사)', item)){   //jsonPayItem
                jsonHeaders.push('COMP_YE_PAY_ITEM');
            }else if (_.isEqual('연말정산고용보험료(회사)', item)){
                jsonHeaders.push('COMP_YE_PAY_AMT');
            }else if (_.isEqual('정산고용보험급여항목(사원)', item)){   //jsonPayItem
                jsonHeaders.push('EMP_ADJ_PAY_ITEM');
            }else if (_.isEqual('정산고용보험료(사원)', item)){
                jsonHeaders.push('EMP_ADJ_PAY_AMT');
            }else if (_.isEqual('정산고용보험급여항목(회사)', item)){   //jsonPayItem
                jsonHeaders.push('COMP_ADJ_PAY_ITEM');
            }else if (_.isEqual('정산고용보험료(회사)', item)){
                jsonHeaders.push('COMP_ADJ_PAY_AMT');
            }else{
                chkCount = 1;
            }
        }
        ...
        for (const jsonData of jsonDatas) {
            ...
            for (let i = 0; i < jsonData.length; i++) {
                ...
                } else if (_.isEqual(jsonHeaders[i], 'EMP_PAY_ITEM')) {    //combo 공통코드
                ...
                } else if (_.isEqual(jsonHeaders[i], 'COMP_PAY_ITEM')) {    //combo 공통코드
                ...
                } else if (_.isEqual(jsonHeaders[i], 'EMP_YE_PAY_ITEM')) {    //combo 공통코드
                ...
                } else if (_.isEqual(jsonHeaders[i], 'COMP_YE_PAY_ITEM')) {    //combo 공통코드
                ...
                } else if (_.isEqual(jsonHeaders[i], 'EMP_ADJ_PAY_ITEM')) {    //combo 공통코드
                ...
                } else if (_.isEqual(jsonHeaders[i], 'COMP_ADJ_PAY_ITEM')) {    //combo 공통코드
    ...
    const fn_check = async function () {
        ...
        var paramObj = {
            ...
            , V_P_COMP_STABILITY_AMT_D   : ''
            , V_P_EMP_INSURE_YE_AMT_D    : ''
            , V_P_COMP_INSURE_YE_AMT_D   : ''
            , V_P_STABILITY_INCOME_YE_D  : ''
            , V_P_EMP_INSURE_ADJ_AMT_D   : ''
            , V_P_COMP_INSURE_ADJ_AMT_D  : ''
            , V_P_STABILITY_INCOME_ADJ_D : ''
        ...
        const postJsonPromise = gfn_postJSON("/hr/hrp/pay/insertHrp5400S.do", {
            getType: 'json',
            workType: 'CHECK',
            cv_count: '0',
            params: gfnma_objectToString(paramObj)
    ...
    const fn_save = async function () {
        ...
        let strcomp_stability_amt_d     = '';
        let stremp_insure_ye_amt_d      = '';
        let strcomp_insure_ye_amt_d     = '';
        let strstability_income_ye_d    = '';
        let stremp_insure_adj_amt_d     = '';
        let strcomp_insure_adj_amt_d    = '';
        let strstability_income_adj_d   = '';
        let isChecked = false;
        ...
        updateData.forEach((item, index) => {
            ...
            strcomp_stability_amt_d     = item.COMP_STABILITY_AMT;
            stremp_insure_ye_amt_d      = item.EMP_INSURE_YE_AMT;
            strcomp_insure_ye_amt_d     = item.COMP_INSURE_YE_AMT;
            strstability_income_ye_d    = item.STABILITY_INCOME_YE;
            stremp_insure_adj_amt_d     = item.EMP_INSURE_ADJ_AMT;
            strcomp_insure_adj_amt_d    = item.COMP_INSURE_ADJ_AMT;
            strstability_income_adj_d   = item.STABILITY_INCOME_ADJ;
        ...
        if (stremp_code_d.length >0){
            ...
            strcomp_stability_amt_d     =   strcomp_stability_amt_d.slice(0, -1);
            stremp_insure_ye_amt_d      =   stremp_insure_ye_amt_d.slice(0, -1);
            strcomp_insure_ye_amt_d     =   strcomp_insure_ye_amt_d.slice(0, -1);
            strstability_income_ye_d    =   strstability_income_ye_d.slice(0, -1);
            stremp_insure_adj_amt_d     =   stremp_insure_adj_amt_d.slice(0, -1);
            strcomp_insure_adj_amt_d    =   strcomp_insure_adj_amt_d.slice(0, -1);
            strstability_income_adj_d   =   strstability_income_adj_d.slice(0, -1);
        }
        ...
        var paramObj = {
            ...
            , V_P_COMP_STABILITY_AMT_D   : strcomp_stability_amt_d
            , V_P_EMP_INSURE_YE_AMT_D    : stremp_insure_ye_amt_d
            , V_P_COMP_INSURE_YE_AMT_D   : strcomp_insure_ye_amt_d
            , V_P_STABILITY_INCOME_YE_D  : strstability_income_ye_d
            , V_P_EMP_INSURE_ADJ_AMT_D   : stremp_insure_adj_amt_d
            , V_P_COMP_INSURE_ADJ_AMT_D  : strcomp_insure_adj_amt_d
            , V_P_STABILITY_INCOME_ADJ_D : strstability_income_adj_d
