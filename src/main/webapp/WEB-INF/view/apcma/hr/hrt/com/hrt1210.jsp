...
<script type="text/javascript">
    ...
    const fn_initSBSelect = async function() {
        ...
        let rst = await Promise.all([
            ...
            gfnma_multiSelectInit({
                ...
                ,colValue		: 'SHWORK_CD'
                ,colLabel		: 'SHWORK_TEAM_NM'
                ,columns		:[
                    {caption: "교대조코드",		ref: 'SHWORK_CD', 			width:'150px',  	style:'text-align:left'},
                    {caption: "교대조명", 		ref: 'SHWORK_TEAM_NM',    		width:'150px',  	style:'text-align:left'}
                ]
            }),
            // 사용여부
            gfnma_setComSelect(['SRCH_USE_YN'], jsonUseYn, 'L_COM036', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
            // 근무일
            ...
            gfnma_setComSelect(['WORK_ON_DAY_TYPE', 'WORK_OFF_DAY_TYPE'
                ...
                , jsonDayType, 'L_HRT011', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
            // 근태시간
            gfnma_setComSelect(['WORK_ON_HH', 'WORK_OFF_HH'
                ...
                , jsonHh, 'L_HRT009', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
            // 근태분
            gfnma_setComSelect(['WORK_ON_MM', 'WORK_OFF_MM'
                ...
                , jsonMm, 'L_HRT010', '', gv_ma_selectedCorpCd, gv_ma_selectedClntCd, 'SBSD_CD', 'CD_NM', 'Y', ''),
            // 근태항목
    ...
    const fn_save = async function() {
        ...
        let SHIFT_NAME = gfn_nvl(SBUxMethod.get("SHIFT_NAME"));
        let USE_YN = gfn_nvl(SBUxMethod.get("USE_YN").USE_YN);
        let FIVE_WEEK_YN = gfn_nvl(SBUxMethod.get("FIVE_WEEK_YN").FIVE_WEEK_YN);
        let SHIFT_CATEGORY = gfn_nvl(SBUxMethod.get("SHIFT_CATEGORY"));
        let WORK_TYPE_CODE = gfn_nvl(SBUxMethod.get("WORK_TYPE_CODE"));
        let WORK_DAY_TYPE = gfn_nvl(SBUxMethod.get("WORK_DAY_TYPE"));
        let BACK_COLOR = gfn_nvl(SBUxMethod.get("BACK_COLOR"));
        let PAY_ALLOWANCE_FLAG = gfn_nvl(SBUxMethod.get("PAY_ALLOWANCE_FLAG"));
        let WORK_ON_DAY_TYPE = gfn_nvl(SBUxMethod.get("WORK_ON_DAY_TYPE"));
        let WORK_ON_HHMM = gfn_nvl(SBUxMethod.get("WORK_ON_HH")) + gfn_nvl(SBUxMethod.get("WORK_ON_MM"));
        let WORK_OFF_DAY_TYPE = gfn_nvl(SBUxMethod.get("WORK_OFF_DAY_TYPE"));
        let WORK_OFF_HHMM = gfn_nvl(SBUxMethod.get("WORK_OFF_HH")) + gfn_nvl(SBUxMethod.get("WORK_OFF_MM"));
        let NORMAL_START_DAY_TYPE = gfn_nvl(SBUxMethod.get("NORMAL_START_DAY_TYPE"));
        let NORMAL_START_HHMM = gfn_nvl(SBUxMethod.get("NORMAL_START_HH")) + gfn_nvl(SBUxMethod.get("NORMAL_START_MM"));
        let NORMAL_END_DAY_TYPE = gfn_nvl(SBUxMethod.get("NORMAL_END_DAY_TYPE"));
        let NORMAL_END_HHMM = gfn_nvl(SBUxMethod.get("NORMAL_END_HH")) + gfn_nvl(SBUxMethod.get("NORMAL_END_MM"));
        let NORMAL_TIME_ITEM_CODE = gfn_nvl(SBUxMethod.get("NORMAL_TIME_ITEM_CODE"));
        let NORMAL_BREAK_APPLY_YN = gfn_nvl(SBUxMethod.get("NORMAL_BREAK_APPLY_YN").NORMAL_BREAK_APPLY_YN);
        let OVER_START_DAY_TYPE = gfn_nvl(SBUxMethod.get("OVER_START_DAY_TYPE"));
        let OVER_START_HHMM = gfn_nvl(SBUxMethod.get("OVER_START_HH")) + gfn_nvl(SBUxMethod.get("OVER_START_MM"));
        let OVER_END_DAY_TYPE = gfn_nvl(SBUxMethod.get("OVER_END_DAY_TYPE"));
        let OVER_END_HHMM = gfn_nvl(SBUxMethod.get("OVER_END_HH")) + gfn_nvl(SBUxMethod.get("OVER_END_MM"));
        let OVER_TIME_ITEM_CODE = gfn_nvl(SBUxMethod.get("OVER_TIME_ITEM_CODE"));
        let OVER_BREAK_APPLY_YN = gfn_nvl(SBUxMethod.get("OVER_BREAK_APPLY_YN").OVER_BREAK_APPLY_YN);
        let NIGHT_START_DAY_TYPE = gfn_nvl(SBUxMethod.get("NIGHT_START_DAY_TYPE"));
        let NIGHT_START_HHMM = gfn_nvl(SBUxMethod.get("NIGHT_START_HH")) + gfn_nvl(SBUxMethod.get("NIGHT_START_MM"));
        let NIGHT_END_DAY_TYPE = gfn_nvl(SBUxMethod.get("NIGHT_END_DAY_TYPE"));
        let NIGHT_END_HHMM = gfn_nvl(SBUxMethod.get("NIGHT_END_HH")) + gfn_nvl(SBUxMethod.get("NIGHT_END_MM"));
        let NIGHT_TIME_ITEM_CODE = gfn_nvl(SBUxMethod.get("NIGHT_TIME_ITEM_CODE"));
        let NIGHT_BREAK_APPLY_YN = gfn_nvl(SBUxMethod.get("NIGHT_BREAK_APPLY_YN").NIGHT_BREAK_APPLY_YN);
        let AFTER1_START_DAY_TYPE = gfn_nvl(SBUxMethod.get("AFTER1_START_DAY_TYPE"));
        let AFTER1_START_HHMM = gfn_nvl(SBUxMethod.get("AFTER1_START_HH")) + gfn_nvl(SBUxMethod.get("AFTER1_START_MM"));
        let AFTER1_END_DAY_TYPE = gfn_nvl(SBUxMethod.get("AFTER1_END_DAY_TYPE"));
        let AFTER1_END_HHMM = gfn_nvl(SBUxMethod.get("AFTER1_END_HH")) + gfn_nvl(SBUxMethod.get("AFTER1_END_MM"));
        let AFTER1_TIME_ITEM_CODE = gfn_nvl(SBUxMethod.get("AFTER1_TIME_ITEM_CODE"));
        let AFTER1_BREAK_APPLY_YN = gfn_nvl(SBUxMethod.get("AFTER1_BREAK_APPLY_YN").AFTER1_BREAK_APPLY_YN);
        let AFTER2_START_DAY_TYPE = gfn_nvl(SBUxMethod.get("AFTER2_START_DAY_TYPE"));
        let AFTER2_START_HHMM = gfn_nvl(SBUxMethod.get("AFTER2_START_HH")) + gfn_nvl(SBUxMethod.get("AFTER2_START_MM"));
        let AFTER2_END_DAY_TYPE = gfn_nvl(SBUxMethod.get("AFTER2_END_DAY_TYPE"));
        let AFTER2_END_HHMM = gfn_nvl(SBUxMethod.get("AFTER2_END_HH")) + gfn_nvl(SBUxMethod.get("AFTER2_END_MM"));
        let AFTER2_TIME_ITEM_CODE = gfn_nvl(SBUxMethod.get("AFTER2_TIME_ITEM_CODE"));
        let AFTER2_BREAK_APPLY_YN = gfn_nvl(SBUxMethod.get("AFTER2_BREAK_APPLY_YN").AFTER2_BREAK_APPLY_YN);
        let AFTER3_START_DAY_TYPE = gfn_nvl(SBUxMethod.get("AFTER3_START_DAY_TYPE"));
        let AFTER3_START_HHMM = gfn_nvl(SBUxMethod.get("AFTER3_START_HH")) + gfn_nvl(SBUxMethod.get("AFTER3_START_MM"));
        let AFTER3_END_DAY_TYPE = gfn_nvl(SBUxMethod.get("AFTER3_END_DAY_TYPE"));
        let AFTER3_END_HHMM = gfn_nvl(SBUxMethod.get("AFTER3_END_HH")) + gfn_nvl(SBUxMethod.get("AFTER3_END_MM"));
        let AFTER3_TIME_ITEM_CODE = gfn_nvl(SBUxMethod.get("AFTER3_TIME_ITEM_CODE"));
        let AFTER4_START_DAY_TYPE = gfn_nvl(SBUxMethod.get("AFTER4_START_DAY_TYPE"));
        let AFTER4_START_HHMM = gfn_nvl(SBUxMethod.get("AFTER4_START_HH")) + gfn_nvl(SBUxMethod.get("AFTER4_START_MM"));
        let AFTER4_END_DAY_TYPE = gfn_nvl(SBUxMethod.get("AFTER4_END_DAY_TYPE"));
        let AFTER4_END_HHMM = gfn_nvl(SBUxMethod.get("AFTER4_END_HH")) + gfn_nvl(SBUxMethod.get("AFTER4_END_MM"));
        let AFTER4_TIME_ITEM_CODE = gfn_nvl(SBUxMethod.get("AFTER4_TIME_ITEM_CODE"));
        let BREAK1_START_DAY_TYPE = gfn_nvl(SBUxMethod.get("BREAK1_START_DAY_TYPE"));
        let BREAK1_START_HHMM = gfn_nvl(SBUxMethod.get("BREAK1_START_HH")) + gfn_nvl(SBUxMethod.get("BREAK1_START_MM"));
        let BREAK1_END_DAY_TYPE = gfn_nvl(SBUxMethod.get("BREAK1_END_DAY_TYPE"));
        let BREAK1_END_HHMM = gfn_nvl(SBUxMethod.get("BREAK1_END_HH")) + gfn_nvl(SBUxMethod.get("BREAK1_END_MM"));
        let BREAK1_TIME_ITEM_CODE = gfn_nvl(SBUxMethod.get("BREAK1_TIME_ITEM_CODE"));
        let BREAK2_START_DAY_TYPE = gfn_nvl(SBUxMethod.get("BREAK2_START_DAY_TYPE"));
        let BREAK2_START_HHMM = gfn_nvl(SBUxMethod.get("BREAK2_START_HH")) + gfn_nvl(SBUxMethod.get("BREAK2_START_MM"));
        let BREAK2_END_DAY_TYPE = gfn_nvl(SBUxMethod.get("BREAK2_END_DAY_TYPE"));
        let BREAK2_END_HHMM = gfn_nvl(SBUxMethod.get("BREAK2_END_HH")) + gfn_nvl(SBUxMethod.get("BREAK2_END_MM"));
        let BREAK2_TIME_ITEM_CODE = gfn_nvl(SBUxMethod.get("BREAK2_TIME_ITEM_CODE"));
        let BREAK3_START_DAY_TYPE = gfn_nvl(SBUxMethod.get("BREAK3_START_DAY_TYPE"));
        let BREAK3_START_HHMM = gfn_nvl(SBUxMethod.get("BREAK3_START_HH")) + gfn_nvl(SBUxMethod.get("BREAK3_START_MM"));
        let BREAK3_END_DAY_TYPE = gfn_nvl(SBUxMethod.get("BREAK3_END_DAY_TYPE"));
        let BREAK3_END_HHMM = gfn_nvl(SBUxMethod.get("BREAK3_END_HH")) + gfn_nvl(SBUxMethod.get("BREAK3_END_MM"));
        let BREAK3_TIME_ITEM_CODE = gfn_nvl(SBUxMethod.get("BREAK3_TIME_ITEM_CODE"));
        let BREAK4_START_DAY_TYPE = gfn_nvl(SBUxMethod.get("BREAK4_START_DAY_TYPE"));
        let BREAK4_START_HHMM = gfn_nvl(SBUxMethod.get("BREAK4_START_HH")) + gfn_nvl(SBUxMethod.get("BREAK4_START_MM"));
        let BREAK4_END_DAY_TYPE = gfn_nvl(SBUxMethod.get("BREAK4_END_DAY_TYPE"));
        let BREAK4_END_HHMM = gfn_nvl(SBUxMethod.get("BREAK4_END_HH")) + gfn_nvl(SBUxMethod.get("BREAK4_END_MM"));
        let BREAK4_TIME_ITEM_CODE = gfn_nvl(SBUxMethod.get("BREAK4_TIME_ITEM_CODE"));
        let BREAK5_START_DAY_TYPE = gfn_nvl(SBUxMethod.get("BREAK5_START_DAY_TYPE"));
        let BREAK5_START_HHMM = gfn_nvl(SBUxMethod.get("BREAK5_START_HH")) + gfn_nvl(SBUxMethod.get("BREAK5_START_MM"));
        let BREAK5_END_DAY_TYPE = gfn_nvl(SBUxMethod.get("BREAK5_END_DAY_TYPE"));
        let BREAK5_END_HHMM = gfn_nvl(SBUxMethod.get("BREAK5_END_HH")) + gfn_nvl(SBUxMethod.get("BREAK5_END_MM"));
        let BREAK5_TIME_ITEM_CODE = gfn_nvl(SBUxMethod.get("BREAK5_TIME_ITEM_CODE"));
            ...
            if (_.isEqual("S", data.resultStatus)) {
                ...
                data.cv_1.forEach((item, index) => {
                    const msg = {
                        ...
                        OVER_TIME_ITEM_CODE : item.PRLG_WORK_ITEM_CD,
                        OVER_START_DAY_TYPE : item.PRLG_BGNG_HR_TYPE,
                        OVER_START_HHMM : item.PRLG_WORK_HR,
                        OVER_START_HH : item.OVER_START_HH,
                        OVER_START_MM : item.OVER_START_MM,
                        OVER_END_DAY_TYPE : item.PRLG_END_HR_TYPE,
                        OVER_END_HHMM : item.PRLG_END_HR,
                        ...
                        AFTER1_TIME_ITEM_CODE : item.OVTM_WORK1_ITEM_CD,
                        AFTER1_START_DAY_TYPE : item.OVTM_WORK1_BGNG_DAY_TYPE,
                        AFTER1_START_HHMM : item.OVTM_WORK1_BGNG_HR,
                        AFTER1_START_HH : item.AFTER1_START_HH,
                        AFTER1_START_MM : item.AFTER1_START_MM,
                        AFTER1_END_DAY_TYPE : item.OVTM_WORK1_END_DAY_TYPE,
                        AFTER1_END_HHMM : item.OVTM_WORK1_END_HR,
                        AFTER1_END_HH : item.AFTER1_END_HH,
                        AFTER1_END_MM : item.AFTER1_END_MM,
                        AFTER1_HOURS : item.AFTER1_HOURS,
                        AFTER2_TIME_ITEM_CODE : item.OVTM_WORK2_ITEM_CD,
                        AFTER2_START_DAY_TYPE : item.OVTM_WORK2_BGNG_DAY_TYPE,
                        AFTER2_START_HHMM : item.OVTM_WORK2_BGNG_HR,
                        AFTER2_START_HH : item.AFTER2_START_HH,
                        AFTER2_START_MM : item.AFTER2_START_MM,
                        AFTER2_END_DAY_TYPE : item.OVTM_WORK2_END_DAY_TYPE,
                        AFTER2_END_HHMM : item.OVTM_WORK2_END_HR,
                        AFTER2_END_HH : item.AFTER2_END_HH,
                        AFTER2_END_MM : item.AFTER2_END_MM,
                        AFTER2_HOURS : item.AFTER2_HOURS,
                        AFTER3_TIME_ITEM_CODE : item.OVTM_WORK3_ITEM_CD,
                        AFTER3_START_DAY_TYPE : item.OVTM_WORK3_BGNG_DAY_TYPE,
                        AFTER3_START_HHMM : item.OVTM_WORK3_BGNG_HR,
                        AFTER3_START_HH : item.AFTER3_START_HH,
                        AFTER3_START_MM : item.AFTER3_START_MM,
                        AFTER3_END_DAY_TYPE : item.OVTM_WORK3_END_DAY_TYPE,
                        AFTER3_END_HHMM : item.OVTM_WORK3_END_HR,
                        AFTER3_END_HH : item.AFTER3_END_HH,
                        AFTER3_END_MM : item.AFTER3_END_MM,
                        AFTER3_HOURS : item.AFTER3_HOURS,
                        AFTER4_TIME_ITEM_CODE : item.OVTM_WORK4_ITEM_CD,
                        AFTER4_START_DAY_TYPE : item.OVTM_WORK4_BGNG_DAY_TYPE,
                        AFTER4_START_HHMM : item.OVTM_WORK4_BGNG_HR,
                        AFTER4_START_HH : item.AFTER4_START_HH,
                        AFTER4_START_MM : item.AFTER4_START_MM,
                        AFTER4_END_DAY_TYPE : item.OVTM_WORK4_END_DAY_TYPE,
                        AFTER4_END_HHMM : item.OVTM_WORK4_END_HR,
                        AFTER4_END_HH : item.AFTER4_END_HH,
                        AFTER4_END_MM : item.AFTER4_END_MM,
                        AFTER4_HOURS : item.AFTER4_HOURS,
                        BREAK1_TIME_ITEM_CODE : item.BRTM1_ITEM_CD,
                        BREAK1_START_DAY_TYPE : item.BRTM1_BGNG_DAY_TYPE,
                        BREAK1_START_HHMM : item.BRTM1_BGNG_TM,
                        BREAK1_START_HH : item.BREAK1_START_HH,
                        BREAK1_START_MM : item.BREAK1_START_MM,
                        BREAK1_END_DAY_TYPE : item.BRTM1_END_DAY_TYPE,
                        BREAK1_END_HHMM : item.BRTM1_END_TM,
                        BREAK1_END_HH : item.BREAK1_END_HH,
                        BREAK1_END_MM : item.BREAK1_END_MM,
                        BREAK1_HOURS : item.BREAK1_HOURS,
                        BREAK2_TIME_ITEM_CODE : item.BRTM2_ITEM_CD,
                        BREAK2_START_DAY_TYPE : item.BRTM2_BGNG_DAY_TYPE,
                        BREAK2_START_HHMM : item.BRTM2_BGNG_TM,
                        BREAK2_START_HH : item.BREAK2_START_HH,
                        BREAK2_START_MM : item.BREAK2_START_MM,
                        BREAK2_END_DAY_TYPE : item.BRTM2_END_DAY_TYPE,
                        BREAK2_END_HHMM : item.BRTM2_END_TM,
                        BREAK2_END_HH : item.BREAK2_END_HH,
                        BREAK2_END_MM : item.BREAK2_END_MM,
                        BREAK2_HOURS : item.BREAK2_HOURS,
                        BREAK3_TIME_ITEM_CODE : item.BRTM3_ITEM_CD,
                        BREAK3_START_DAY_TYPE : item.BRTM3_BGNG_DAY_TYPE,
                        BREAK3_START_HHMM : item.BRTM3_BGNG_TM,
                        BREAK3_START_HH : item.BREAK3_START_HH,
                        BREAK3_START_MM : item.BREAK3_START_MM,
                        BREAK3_END_DAY_TYPE : item.BRTM3_END_DAY_TYPE,
                        BREAK3_END_HHMM : item.BRTM3_END_TM,
                        BREAK3_END_HH : item.BREAK3_END_HH,
                        BREAK3_END_MM : item.BREAK3_END_MM,
                        BREAK3_HOURS : item.BREAK3_HOURS,
                        BREAK4_TIME_ITEM_CODE : item.BRTM4_ITEM_CD,
                        BREAK4_START_DAY_TYPE : item.BRTM4_BGNG_DAY_TYPE,
                        BREAK4_START_HHMM : item.BRTM4_BGNG_TM,
                        BREAK4_START_HH : item.BREAK4_START_HH,
                        BREAK4_START_MM : item.BREAK4_START_MM,
                        BREAK4_END_DAY_TYPE : item.BRTM4_END_DAY_TYPE,
                        BREAK4_END_HHMM : item.BRTM4_END_TM,
                        BREAK4_END_HH : item.BREAK4_END_HH,
                        BREAK4_END_MM : item.BREAK4_END_MM,
                        BREAK4_HOURS : item.BREAK4_HOURS,
                        BREAK5_TIME_ITEM_CODE : item.BRTM5_ITEM_CD,
                        BREAK5_START_DAY_TYPE : item.BRTM5_BGNG_DAY_TYPE,
                        BREAK5_START_HHMM : item.BRTM5_BGNG_TM,
                        BREAK5_START_HH : item.BREAK5_START_HH,
                        BREAK5_START_MM : item.BREAK5_START_MM,
                        BREAK5_END_DAY_TYPE : item.BRTM5_END_DAY_TYPE,
                        BREAK5_END_HHMM : item.BRTM5_END_TM,
                        BREAK5_END_HH : item.BREAK5_END_HH,
                        BREAK5_END_MM : item.BREAK5_END_MM,
                        BREAK5_HOURS : item.BREAK5_HOURS,
                        NORMAL_BREAK_APPLY_YN : item.NML_BRTM_APLY_YN,
                        OVER_BREAK_APPLY_YN : item.PRLG_WORK_BRTM_APLY_YN,
                        NIGHT_BREAK_APPLY_YN : item.NGHT_BRTM_APLY_YN,
                        AFTER1_BREAK_APPLY_YN : item.OVTM_WORK1_BRTM_APLY_YN,
                        AFTER2_BREAK_APPLY_YN : item.OVTM_WORK2_BRTM_APLY_YN,
                        MEMO : item.MEMO,
                        USE_YN : item.USE_YN,
