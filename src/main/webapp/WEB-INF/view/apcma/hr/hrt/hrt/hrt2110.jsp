...
<script type="text/javascript">
    ...
    const fn_initSBSelect = async function() {
        ...
        let rst = await Promise.all([
            ...
            // 구분
            gfnma_setComSelect(['SRCH_TIME_CATEGORY', 'bandgvwInfo'], jsonTimeCategory, 'L_HRT024_04', '', gv_ma_selectedApcCd, gv_ma_selectedClntCd, 'SUB_CODE', 'CODE_NAME', 'Y', ''),
            // 상태
            gfnma_setComSelect(['SRCH_REQUEST_STATUS_CODE', 'SRCH_RESULT_STATUS_CODE', 'bandgvwInfo'], jsonStatusCode, 'L_FIG002', '', gv_ma_selectedApcCd, gv_ma_selectedClntCd, 'SUB_CODE', 'CODE_NAME', 'Y', ''),
            // 직위
            gfnma_setComSelect(['bandgvwInfo'], jsonPositionCode, 'L_HRI002', '', gv_ma_selectedApcCd, gv_ma_selectedClntCd, 'SUB_CODE', 'CODE_NAME', 'Y', ''),
            // 직급
            gfnma_setComSelect(['bandgvwInfo'], jsonJobRank, 'L_HRI005', '', gv_ma_selectedApcCd, gv_ma_selectedClntCd, 'SUB_CODE', 'CODE_NAME', 'Y', ''),
            // 직책
            gfnma_setComSelect(['bandgvwInfo'], jsonDutyCode, 'L_HRI003', '', gv_ma_selectedApcCd, gv_ma_selectedClntCd, 'SUB_CODE', 'CODE_NAME', 'Y', ''),
            // 교대조
            gfnma_setComSelect(['bandgvwInfo'], jsonShiftCode, 'L_HRT_SHIFTCODE', '', gv_ma_selectedApcCd, gv_ma_selectedClntCd, 'SHIFT_CODE', 'SHIFT_NAME', 'Y', ''),
            // 휴일여부
    ...
    function fn_createBandgvwInfoGrid() {
        ...
        SBGridProperties.columns = [
            {caption: ["신청자정보", ""],			    ref: 'CHK_YN', 			        type:'checkbox',  	width:'45px',  	style:'text-align:center', typeinfo : {fixedcellcheckbox : { usemode : true , rowindex : 1 , deletecaption : false }, checkedvalue: 'Y', uncheckedvalue: 'N'}},
        ...
        updatedData.forEach((item, index) => {
            const param = {
                ...
                params: gfnma_objectToString({
                    ...
                    V_P_HISTORY_SEQ : gfn_nvl(item.data.HISTORY_SEQ) == '' ? 0 : item.data.HISTORY_SEQ,
                    ...
                    V_P_REQUEST_DATE : item.data.REQUEST_DATE,
                    V_P_CREATE_TYPE : item.data.CREATE_TYPE,
                    V_P_SUBSTITUTE_VACATION_YN : item.data.SUBSTITUTE_VACATION_YN,
                    V_P_TXN_ID_D : '',
                    V_P_HOLIDAY_WORK_DATE : item.data.HOLIDAY_WORK_DATE,
            ...
            if (_.isEqual("S", listData.resultStatus)) {
                ...
                listData.cv_1.forEach((item, index) => {
                    const msg = {
                        ...
                        SHIFT_CODE_ORIG : item.SHIFT_CODE_ORIG,
                        ...
                        TIME_CONFIRM_DATE : item.TIME_CONFIRM_DATE,
                        ...
                        REQUEST_STATUS_CODE : item.REQUEST_STATUS_CODE,
                        REQUEST_APPROVE_DATE : item.REQUEST_APPROVE_DATE,
                        REQUEST_APPROVE_YN : item.REQUEST_APPROVE_YN,
                        REQUEST_REJECT_YN : item.REQUEST_REJECT_YN,
                        REQUEST_REJECT_DATE : item.REQUEST_REJECT_DATE,
                        REQUEST_REJECT_REASON : item.REQUEST_REJECT_REASON,
                        RESULT_STATUS_CODE : item.RESULT_STATUS_CODE,
                        RESULT_APPROVE_DATE : item.RESULT_APPROVE_DATE,
                        RESULT_APPROVE_YN : item.RESULT_APPROVE_YN,
                        RESULT_REJECT_YN : item.RESULT_REJECT_YN,
                        RESULT_REJECT_DATE : item.RESULT_REJECT_DATE,
                        RESULT_REJECT_REASON : item.RESULT_REJECT_REASON,
                        ...
                        SUBSTITUTE_VACATION_YN : item.SUBSTITUTE_VACATION_YN,
                        REFERENCE_TXN_ID : item.REFERENCE_TXN_ID,
                        ...
                        ALTER_EMP_CODE : item.ALTER_EMP_CODE,
                        ALTER_TIME_ITEM_CODE : item.ALTER_TIME_ITEM_CODE,
