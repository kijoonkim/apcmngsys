...
function compopcom3420(options) {
    ...
    const fn_setCom3420Grid = async function() {
    	...
    	let SRCH_CS_CODE 	= gfnma_nvl2(SBUxMethod.get("SRCH_CS_CODE"));
    	let SRCH_CS_NAME 	= gfnma_nvl2(SBUxMethod.get("SRCH_CS_NAME"));
  			...
  			if (_.isEqual("S", data.resultStatus)) {
  	        	...
  	        	data.cv_1.forEach((item, index) => {
  					const msg = {
  							...
  							CS_FULLNAME				: gfnma_nvl2(item.CNPT_FLNM),
  							CS_ABBR_NAME			: gfnma_nvl2(item.CNPT_NM_ABBR),			
  							CS_NAME_ENG			: gfnma_nvl2(item.CNPT_NM_ENG),		
  							CS_GROUP	: gfnma_nvl2(item.CNPT_GROUP),
  							BANK_NO	: gfnma_nvl2(item.BANK_CD),
  							CS_NAME	: gfnma_nvl2(item.CNPT_NM),
  							FOREIGN_YN	: gfnma_nvl2(item.FRCNY_YN),
  							NATION_CODE	: gfnma_nvl2(item.NTN_CD),
  							REGION_CODE	: gfnma_nvl2(item.RGN_CD),
  							BIZ_REGNO	: gfnma_nvl2(item.BRNO),
  							TAX_TYPE	: gfnma_nvl2(item.TX_TYPE),
  							COM_TYPE	: gfnma_nvl2(item.CO_TYPE),
  							BIZ_TYPE	: gfnma_nvl2(item.CORP_TYPE),
  							COMP_REGNO	: gfnma_nvl2(item.CORP_REGNO),
  							CHIEF_NAME	: gfnma_nvl2(item.CEO_NM),
  							RESIDENT_YN	: gfnma_nvl2(item.INHTNT_DLNG_YN),
  							BIZ_CATEGORY	: gfnma_nvl2(item.BZSTAT),
  							BIZ_ITEMS	: gfnma_nvl2(item.TPBIZ),
  							PURCHASE_YN	: gfnma_nvl2(item.PCPL_YN),
  							SALE_YN	: gfnma_nvl2(item.SLS_CNPT_YN),
  							TRANSPORT_YN	: gfnma_nvl2(item.TRSPRT_CNPT_YN),
  							CURRENCY_CODE	: gfnma_nvl2(item.CRN_CD),
  							SUB_NO	: gfnma_nvl2(item.SBSD_BPLC_NO),
  							USE_YN	: gfnma_nvl2(item.USE_YN),
  							LIQUOR_YN	: gfnma_nvl2(item.LIQR_YN),
  							ZIP_CODE	: gfnma_nvl2(item.ZIP_CD),
  							ADDRESS	: gfnma_nvl2(item.ADDR),
  							TEL	: gfnma_nvl2(item.TELNO),
  							FAX	: gfnma_nvl2(item.FX_NO),
  							EMAIL_ID	: gfnma_nvl2(item.EML_ID),
  							WEB_URL	: gfnma_nvl2(item.HMPG_URL),
  							START_DATE	: gfnma_nvl2(item.BGNG_YMD).replace(/(\d{4})(\d{2})(\d{2})/, "$1-$2-$3"),
  							END_DATE	: gfnma_nvl2(item.END_YMD).replace(/(\d{4})(\d{2})(\d{2})/, "$1-$2-$3"),
  							MEMO	: gfnma_nvl2(item.MEMO),
  							DEFER_YN	: gfnma_nvl2(item.PAY_HLDOF_YN),
  							DEFER_REASON	: gfnma_nvl2(item.DFRMNT_RSN),
  							AP_ACC_CODE	: gfnma_nvl2(item.APS_ACNT_CD),
  							AP_ACC_CODE_FOREIGN	: gfnma_nvl2(item.APS_ACNT_CD_FRCNCY),
  							AR_ACC_CODE	: gfnma_nvl2(item.AR_ACNT_CD),
  							AR_ACC_CODE_FOREIGN	: gfnma_nvl2(item.AR_ACNT_CD_FRCNCY),
  							EMP_CODE	: gfnma_nvl2(item.EMP_CD),
  							TXN_STOP_YN	: gfnma_nvl2(item.TRSC_HLT_YN),
  							TXN_STOP_REASON	: gfnma_nvl2(item.TRSC_HLT_RSN),
  							BANKRUPTCY_YN	: gfnma_nvl2(item.BNKRP_CNPT_YN),
  							PREPAY_ACC_CODE	: gfnma_nvl2(item.PRPY_ACNT_CD),
  							ADVANCE_ACC_CODE	: gfnma_nvl2(item.ADPYR_ACNTL_CD),
  							PUR_HANDOV_CNDT_CD	: gfnma_nvl2(item.PORDR_DLVRTRM_CD),
  							PAY_DATE_RULE	: gfnma_nvl2(item.PAY_CNDTN),
  							DELIVERY_TYPE	: gfnma_nvl2(item.DLVRTRM),
  							RECEIPT_DATE_RULE	: gfnma_nvl2(item.RCPT_YMD_RULE),
  							SHIPPING_TYPE	: gfnma_nvl2(item.SHPG_TYPE),
  							AP_CLR_ACC	: gfnma_nvl2(item.APS_CLCLN_ACNT),
  							CUSTOMER_CATEGORY	: gfnma_nvl2(item.CUST_CTRGY),
  							LANGUAGE	: gfnma_nvl2(item.LANG_ID),
  							TAX_CODE	: gfnma_nvl2(item.TX_CD),
  							MAKER_YN	: gfnma_nvl2(item.MKR_YN),
  							EXPECTED_CUSTOMER_YN	: gfnma_nvl2(item.PRNMNT_CUST_YN),
  							WITHHOLD_TAX_YN	: gfnma_nvl2(item.WTHD_TX_YN),
  							WITHHOLD_TAX_OFFICE	: gfnma_nvl2(item.WTHD_TXOFC),
  							WITHHOLD_TAX_OFFICE2	: gfnma_nvl2(item.WTHD_TXOFC2),
  							WITHHOLD_TAX_TYPE	: gfnma_nvl2(item.WTHD_TX_TYPE),
  							TAX_SEND	: gfnma_nvl2(item.TX_SEND),
  							INTERNAL_PAY_RULE_YN	: gfnma_nvl2(item.INR_PAY_RULE_YN),
  							SALES_TAX_SEND	: gfnma_nvl2(item.SLS_TXIV_TRSM),
  							RCPT_CHK_RANGE	: gfnma_nvl2(item.RCPT_CHCK_LIM),
  							STEEL_SCRAP_PAY_YN	: gfnma_nvl2(item.SCRAP_DLNG_YN),
  							OSP_YN	: gfnma_nvl2(item.OTSRC_CO_YN),
  							REF_CS_CODE	: gfnma_nvl2(item.RFRNC_CNPT_CD),
  							SAP_VENDOR_CODE	: gfnma_nvl2(item.ERP_CNPT_CD2),
  							SAP_CUSTOMER_CODE	: gfnma_nvl2(item.ERP_CNPT_CD)
  					}
