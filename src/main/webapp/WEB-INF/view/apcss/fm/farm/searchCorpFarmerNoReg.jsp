		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdSearchCorpFarmerNoReg" style="height:150px; width: 100%;"></div>
				</div>
...
<script type="text/javascript">
	...
	//설비 등록
	var grdSearchCorpFarmerNoReg; // 그리드를 담기위한 객체 선언
	var jsonSearchCorpFarmerNoRegList = []; // 그리드의 참조 데이터 주소 선언
	var jsonsearchCorpFarmerNoReg = []; // 그리드의 참조 데이터 주소 선언
	var jsonComfarmGubun = [];

	const fn_initSBSelectfarm = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdSearchCorpFarmerNoReg', 		jsonComfarmGubun, 	'farm_GUBUN') 		// 설비구분
		])
	...
	const fn_farmMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdSearchCorpFarmerNoReg';
	    SBGridProperties.id = 'grdSearchCorpFarmerNoReg';
	    SBGridProperties.jsonref = 'jsonsearchCorpFarmerNoReg';
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdSearchCorpFarmerNoReg\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdSearchCorpFarmerNoReg\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdSearchCorpFarmerNoReg\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	    ...
	    grdSearchCorpFarmerNoReg = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectfarm(),
		    fn_searchfarmList()
		])
		grdSearchCorpFarmerNoReg.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdSearchCorpFarmerNoReg.bind('click', 'fn_view');
	}
	...
	const fn_searchfarmList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/fm/farm/selectsearchCorpFarmerNo.do", {apcCd : apcCd});
    	let postJsonPromise = gfn_postJSON("/fm/farm/selectSearchCorpFarmerNoRegList.do", {
    		 frmerSn : frmerSn
    		,bzobRgno : bzobRgno
		});
        let data = await postJsonPromise;
        try{
        	jsonsearchCorpFarmerNoReg.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let searchCorpFarmerNoRegVO = {
				...
				jsonsearchCorpFarmerNoReg.push(searchCorpFarmerNoRegVO);
			});



        	grdSearchCorpFarmerNoReg.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdSearchCorpFarmerNoReg.addRow();
        }catch (e) {
...
//     		// 변경 저장
	...
	/* Grid Row 저장 기능*/
// 	const fn_saveFmList = async function(){
// 		let gridData = grdSearchCorpFarmerNoReg.getGridDataAll();
// 		let saveList = [];

// 		for(var i=1; i<=gridData.length; i++ ){
// 			let rowData = grdSearchCorpFarmerNoReg.getRowData(i);
// 			let rowSts = grdSearchCorpFarmerNoReg.getRowStatus(i);
...
// 			let postJsonPromise = gfn_postJSON("/fm/farm/multiSavesearchCorpFarmerNoRegList.do", saveList);
// 	        let data = await postJsonPromise;
// 	        try {
...
// 	}
	...
	/*생산농가 상세내역 수정*/
    	...
    	const postJsonPromise2 = gfn_postJSON("/fm/farm/updateSearchCorpFarmerNoReg.do", {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdSearchCorpFarmerNoReg") {
            	grdSearchCorpFarmerNoReg.setCellData(nRow, nCol, "N", true);
            	//grdSearchCorpFarmerNoReg.setCellData(nRow, 5, gv_apcCd, true);
            	grdSearchCorpFarmerNoReg.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdSearchCorpFarmerNoReg") {
            	if(grdSearchCorpFarmerNoReg.getRowStatus(nRow) == 0 || grdSearchCorpFarmerNoReg.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var searchCorpFarmerNoRegVO = grdSearchCorpFarmerNoReg.getRowData(nRow);
            			fn_deleteRsrc(searchCorpFarmerNoRegVO);
            			grdSearchCorpFarmerNoReg.deleteRow(nRow);
            		}
            	}else{
            		grdSearchCorpFarmerNoReg.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(searchCorpFarmerNoRegVO){
		let postJsonPromise = gfn_postJSON("/fm/farm/deleteSearchCorpFarmerNoReg.do", searchCorpFarmerNoRegVO);
        let data = await postJsonPromise;
    ...
    function fn_view() {
    	console.log("******************fn_view**********************************");
    	var nCol = grdSearchCorpFarmerNoReg.getCol();
        //특정 열 부터 이벤트 적용
        if (nCol < 1) {
            return;
        }
        var nRow = grdSearchCorpFarmerNoReg.getRow();
		if (nRow < 1) {
            return;
		}
        ...
        let rowData = grdSearchCorpFarmerNoReg.getRowData(nRow);
		console.log(rowData);
    ...
    function gridClick(){
		...
		//grdSearchCorpFarmerNoReg 그리드 객체
        selGridRow = grdSearchCorpFarmerNoReg.getRow();
        selGridCol = grdSearchCorpFarmerNoReg.getCol();


        let delYnCol = grdSearchCorpFarmerNoReg.getColRef('delYn');
        let delYnValue = grdSearchCorpFarmerNoReg.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdSearchCorpFarmerNoReg.getColRef('cc');
        let ddCol = grdSearchCorpFarmerNoReg.getColRef('dd');
        let iiCol = grdSearchCorpFarmerNoReg.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdSearchCorpFarmerNoReg.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdSearchCorpFarmerNoReg.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdSearchCorpFarmerNoReg.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdSearchCorpFarmerNoReg.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdSearchCorpFarmerNoReg.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdSearchCorpFarmerNoReg.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdSearchCorpFarmerNoReg.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdSearchCorpFarmerNoReg.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdSearchCorpFarmerNoReg.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
