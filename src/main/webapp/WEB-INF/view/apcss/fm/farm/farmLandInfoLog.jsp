...
<script type="text/javascript">
	...
	const fn_initSBSelectFclt = async function() {
		...
		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdfarmLandInfoLog', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	    ...
	    SBGridProperties.columns = [
	       ...
	       /*  {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdfarmLandInfoLog\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdfarmLandInfoLog\", " + nRow + ")'>삭제</button>";
	        	}
	        }} */
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdfarmLandInfoLog\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdfarmLandInfoLog\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdfarmLandInfoLog\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
		...
		grdfarmLandInfoLog.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdfarmLandInfoLog.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/fm/farm/selectfarmLandInfoLog.do", {apcCd : apcCd});
	...
	const fn_saveFmList = async function(){
		let gridData = grdfarmLandInfoLog.getGridDataAll();
		let saveList = [];

		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdfarmLandInfoLog.getRowData(i);
			let rowSts = grdfarmLandInfoLog.getRowStatus(i);
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveFarmLandInfoLogList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdfarmLandInfoLog") {
            	grdfarmLandInfoLog.setCellData(nRow, nCol, "N", true);
            	//grdfarmLandInfoLog.setCellData(nRow, 5, gv_apcCd, true);
            	grdfarmLandInfoLog.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdfarmLandInfoLog") {
            	if(grdfarmLandInfoLog.getRowStatus(nRow) == 0 || grdfarmLandInfoLog.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var farmLandInfoLogVO = grdfarmLandInfoLog.getRowData(nRow);
            			fn_deleteRsrc(farmLandInfoLogVO);
            			grdfarmLandInfoLog.deleteRow(nRow);
            		}
            	}else{
            		grdfarmLandInfoLog.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(farmLandInfoLogVO){
		let postJsonPromise = gfn_postJSON("/fm/farm/deleteFarmLandInfoLog.do", farmLandInfoLogVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		...
		//grdfarmLandInfoLog 그리드 객체
        selGridRow = grdfarmLandInfoLog.getRow();
        selGridCol = grdfarmLandInfoLog.getCol();


        let delYnCol = grdfarmLandInfoLog.getColRef('delYn');
        let delYnValue = grdfarmLandInfoLog.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdfarmLandInfoLog.getColRef('cc');
        let ddCol = grdfarmLandInfoLog.getColRef('dd');
        let iiCol = grdfarmLandInfoLog.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdfarmLandInfoLog.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdfarmLandInfoLog.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdfarmLandInfoLog.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdfarmLandInfoLog.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdfarmLandInfoLog.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdfarmLandInfoLog.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdfarmLandInfoLog.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdfarmLandInfoLog.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdfarmLandInfoLog.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
	...
	const fn_saveFrmerSn= async function(){
		...
		if(confirm(regMsg)){
			...
			//let postJsonPromise = gfn_postJSON("/co/cd/multiSaveComCdDtlList.do", saveList);
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveReleyFarmLandInfoLogList.do", {
	    		 frmerSn : frmerSn
	 		});
	        let data = await postJsonPromise;
	        try {
