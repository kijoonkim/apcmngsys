		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdFarmerEducationInfoReg" style="height:150px; width: 100%;"></div>
				</div>
...
<script type="text/javascript">
	...
	var jsonFarmerEducationInfoRegList = []; // 그리드의 참조 데이터 주소 선언
	...
	const fn_initSBSelectfarm = async function() {
		...
		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdFarmerEducationInfoReg', 		jsonComfarmGubun, 	'farm_GUBUN') 		// 설비구분
		])
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdFarmerEducationInfoReg\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdFarmerEducationInfoReg\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdFarmerEducationInfoReg\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
		...
		grdFarmerEducationInfoReg.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	...
	const fn_searchfarmList = async function(){
        ...
        try{
        	...
        	data.resultList.forEach((item, index) => {
				...
				jsonfarmerEducationInfoReg.push(farmerEducationInfoRegVO);
			});
...
//     		// 변경 저장
	...
	/* Grid Row 저장 기능*/
// 	const fn_saveFmList = async function(){
// 		let gridData = grdFarmerEducationInfoReg.getGridDataAll();
// 		let saveList = [];

// 		for(var i=1; i<=gridData.length; i++ ){
// 			let rowData = grdFarmerEducationInfoReg.getRowData(i);
// 			let rowSts = grdFarmerEducationInfoReg.getRowStatus(i);
...
// 			let postJsonPromise = gfn_postJSON("/fm/farm/multiSavefarmerEducationInfoRegList.do", saveList);
// 	        let data = await postJsonPromise;
// 	        try {
...
// 	}
	...
	/*생산농가 상세내역 수정*/
    	...
    	const postJsonPromise2 = gfn_postJSON("/fm/farm/updateFarmerEducationInfoReg.do", {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdFarmerEducationInfoReg") {
            	...
            	//grdFarmerEducationInfoReg.setCellData(nRow, 5, gv_apcCd, true);
            ...
            if (grid === "grdFarmerEducationInfoReg") {
            	if(grdFarmerEducationInfoReg.getRowStatus(nRow) == 0 || grdFarmerEducationInfoReg.getRowStatus(nRow) == 2){
            		...
            		if(confirm(delMsg)){
            			var farmerEducationInfoRegVO = grdFarmerEducationInfoReg.getRowData(nRow);
            			fn_deleteRsrc(farmerEducationInfoRegVO);
    ...
    function gridClick(){
		...
		//grdFarmerEducationInfoReg 그리드 객체
        selGridRow = grdFarmerEducationInfoReg.getRow();
        selGridCol = grdFarmerEducationInfoReg.getCol();


        let delYnCol = grdFarmerEducationInfoReg.getColRef('delYn');
        let delYnValue = grdFarmerEducationInfoReg.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdFarmerEducationInfoReg.getColRef('cc');
        let ddCol = grdFarmerEducationInfoReg.getColRef('dd');
        let iiCol = grdFarmerEducationInfoReg.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdFarmerEducationInfoReg.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdFarmerEducationInfoReg.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdFarmerEducationInfoReg.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdFarmerEducationInfoReg.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdFarmerEducationInfoReg.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdFarmerEducationInfoReg.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdFarmerEducationInfoReg.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdFarmerEducationInfoReg.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdFarmerEducationInfoReg.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
