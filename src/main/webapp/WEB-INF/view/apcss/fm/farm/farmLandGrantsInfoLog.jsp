...
<script type="text/javascript">
	...
	const fn_initSBSelectFclt = async function() {
		...
		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdfarmLandGrantsInfoLog', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	    ...
	    SBGridProperties.columns = [
	       ...
	       /*  {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdfarmLandGrantsInfoLog\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdfarmLandGrantsInfoLog\", " + nRow + ")'>삭제</button>";
	        	}
	        }} */
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdfarmLandGrantsInfoLog\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdfarmLandGrantsInfoLog\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdfarmLandGrantsInfoLog\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
		...
		grdfarmLandGrantsInfoLog.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdfarmLandGrantsInfoLog.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/fm/farm/selectfarmLandGrantsInfoLog.do", {apcCd : apcCd});
        ...
        try{
        	...
        	data.resultList.forEach((item, index) => {
				...
				jsonfarmLandGrantsInfoLog.push(farmLandGrantsInfoLogVO);
			});
        	...
        	//비어 있는 마지막 줄 추가용도?
        	grdfarmLandGrantsInfoLog.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdfarmLandGrantsInfoLog.getGridDataAll();
		let saveList = [];

		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdfarmLandGrantsInfoLog.getRowData(i);
			let rowSts = grdfarmLandGrantsInfoLog.getRowStatus(i);
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveFarmLandGrantsInfoLogList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdfarmLandGrantsInfoLog") {
            	grdfarmLandGrantsInfoLog.setCellData(nRow, nCol, "N", true);
            	//grdfarmLandGrantsInfoLog.setCellData(nRow, 5, gv_apcCd, true);
            	grdfarmLandGrantsInfoLog.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdfarmLandGrantsInfoLog") {
            	if(grdfarmLandGrantsInfoLog.getRowStatus(nRow) == 0 || grdfarmLandGrantsInfoLog.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var farmLandGrantsInfoLogVO = grdfarmLandGrantsInfoLog.getRowData(nRow);
            			fn_deleteRsrc(farmLandGrantsInfoLogVO);
            			grdfarmLandGrantsInfoLog.deleteRow(nRow);
            		}
            	}else{
            		grdfarmLandGrantsInfoLog.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(farmLandGrantsInfoLogVO){
		let postJsonPromise = gfn_postJSON("/fm/farm/deleteFarmLandGrantsInfoLog.do", farmLandGrantsInfoLogVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		...
		//grdfarmLandGrantsInfoLog 그리드 객체
        selGridRow = grdfarmLandGrantsInfoLog.getRow();
        selGridCol = grdfarmLandGrantsInfoLog.getCol();


        let delYnCol = grdfarmLandGrantsInfoLog.getColRef('delYn');
        let delYnValue = grdfarmLandGrantsInfoLog.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdfarmLandGrantsInfoLog.getColRef('cc');
        let ddCol = grdfarmLandGrantsInfoLog.getColRef('dd');
        let iiCol = grdfarmLandGrantsInfoLog.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdfarmLandGrantsInfoLog.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdfarmLandGrantsInfoLog.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdfarmLandGrantsInfoLog.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdfarmLandGrantsInfoLog.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdfarmLandGrantsInfoLog.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdfarmLandGrantsInfoLog.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdfarmLandGrantsInfoLog.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdfarmLandGrantsInfoLog.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdfarmLandGrantsInfoLog.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
	...
	const fn_saveFrmerSn= async function(){
		...
		if(confirm(regMsg)){
			...
			//let postJsonPromise = gfn_postJSON("/co/cd/multiSaveComCdDtlList.do", saveList);
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveReleyFarmLandGrantsInfoLogList.do", {
	    		 frmerSn : frmerSn
	 		});
	        let data = await postJsonPromise;
	        try {
