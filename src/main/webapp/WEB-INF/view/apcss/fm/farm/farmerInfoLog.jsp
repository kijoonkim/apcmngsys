...
<!-- 							<td colspan="2" style="border-left: hidden;"></td> -->
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdfarmerInfoLog" style="height:600px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonfarmerInfoLog = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdfarmerInfoLog', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdfarmerInfoLog';
	    SBGridProperties.id = 'grdfarmerInfoLog';
	    SBGridProperties.jsonref = 'jsonfarmerInfoLog';
	    ...
	    SBGridProperties.columns = [
	        ...
	        {caption: ["경영주 법인 명"], 	ref: 'bzmCorpNm',   	type:'output',     style:'text-align:left' ,width: '200px'},
	        ...
	        {caption: ["대표자 주소"], 	ref: 'reprAddr',   	type:'output',     style:'text-align:left' ,width: '300px'},
	       ...
	       /*  {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
			        ...
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdfarmerInfoLog\", " + nRow + ")'>삭제</button>";
	        	}
	        }} */
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdfarmerInfoLog\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdfarmerInfoLog\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdfarmerInfoLog\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	    ...
	    grdfarmerInfoLog = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdfarmerInfoLog.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdfarmerInfoLog.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
        ...
        try{
        	...
        	data.resultList.forEach((item, index) => {
				...
				jsonfarmerInfoLog.push(farmerInfoLogVO);
			});



        	grdfarmerInfoLog.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdfarmerInfoLog.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdfarmerInfoLog.getGridDataAll();
		let saveList = [];

		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdfarmerInfoLog.getRowData(i);
			let rowSts = grdfarmerInfoLog.getRowStatus(i);
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveFarmerInfoLogList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdfarmerInfoLog") {
            	grdfarmerInfoLog.setCellData(nRow, nCol, "N", true);
            	//grdfarmerInfoLog.setCellData(nRow, 5, gv_apcCd, true);
            	grdfarmerInfoLog.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdfarmerInfoLog") {
            	if(grdfarmerInfoLog.getRowStatus(nRow) == 0 || grdfarmerInfoLog.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var farmerInfoLogVO = grdfarmerInfoLog.getRowData(nRow);
            			fn_deleteRsrc(farmerInfoLogVO);
            			grdfarmerInfoLog.deleteRow(nRow);
            		}
            	}else{
            		grdfarmerInfoLog.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(farmerInfoLogVO){
		let postJsonPromise = gfn_postJSON("/fm/farm/deleteFarmerInfoLog.do", farmerInfoLogVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		...
		//grdfarmerInfoLog 그리드 객체
        selGridRow = grdfarmerInfoLog.getRow();
        selGridCol = grdfarmerInfoLog.getCol();


        let delYnCol = grdfarmerInfoLog.getColRef('delYn');
        let delYnValue = grdfarmerInfoLog.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdfarmerInfoLog.getColRef('cc');
        let ddCol = grdfarmerInfoLog.getColRef('dd');
        let iiCol = grdfarmerInfoLog.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdfarmerInfoLog.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdfarmerInfoLog.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdfarmerInfoLog.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdfarmerInfoLog.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdfarmerInfoLog.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdfarmerInfoLog.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdfarmerInfoLog.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdfarmerInfoLog.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdfarmerInfoLog.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
	...
	const fn_saveFrmerSn= async function(){
		...
		if(confirm(regMsg)){
			...
			//let postJsonPromise = gfn_postJSON("/co/cd/multiSaveComCdDtlList.do", saveList);
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveReleyFarmerInfoLogList.do", {
	    		 frmerSn : frmerSn
	 		});
	        let data = await postJsonPromise;
	        try {
