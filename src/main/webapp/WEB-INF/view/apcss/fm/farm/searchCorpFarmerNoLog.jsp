...
<!-- 					<sbux-button id="btnSaveFrm" name="btnSaveFrm" uitype="normal" text="연계 등록" class="btn btn-sm btn-outline-danger" onclick="fn_saveFrmerSn"></sbux-button> -->
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdsearchCorpFarmerNoLog" style="height:600px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonsearchCorpFarmerNoLog = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdsearchCorpFarmerNoLog', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdsearchCorpFarmerNoLog';
	    SBGridProperties.id = 'grdsearchCorpFarmerNoLog';
	    SBGridProperties.jsonref = 'jsonsearchCorpFarmerNoLog';
	    ...
	    SBGridProperties.columns = [
	       ...
	       /*  {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdsearchCorpFarmerNoLog\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdsearchCorpFarmerNoLog\", " + nRow + ")'>삭제</button>";
	        	}
	        }} */
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdsearchCorpFarmerNoLog\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdsearchCorpFarmerNoLog\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdsearchCorpFarmerNoLog\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	    ...
	    grdsearchCorpFarmerNoLog = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdsearchCorpFarmerNoLog.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdsearchCorpFarmerNoLog.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/fm/farm/selectsearchCorpFarmerNoLog.do", {apcCd : apcCd});
    	let postJsonPromise = gfn_postJSON("/fm/farm/selectSearchCorpFarmerNoLogList.do", {
    		 frmerSn : frmerSn
    		,bzobRgno : bzobRgno
		});
        let data = await postJsonPromise;
        try{
        	jsonsearchCorpFarmerNoLog.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let searchCorpFarmerNoLogVO = {
					seq : item.seq
				...
				jsonsearchCorpFarmerNoLog.push(searchCorpFarmerNoLogVO);
			});



        	grdsearchCorpFarmerNoLog.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdsearchCorpFarmerNoLog.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdsearchCorpFarmerNoLog.getGridDataAll();
		let saveList = [];

		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdsearchCorpFarmerNoLog.getRowData(i);
			let rowSts = grdsearchCorpFarmerNoLog.getRowStatus(i);
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveSearchCorpFarmerNoLogList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdsearchCorpFarmerNoLog") {
            	grdsearchCorpFarmerNoLog.setCellData(nRow, nCol, "N", true);
            	//grdsearchCorpFarmerNoLog.setCellData(nRow, 5, gv_apcCd, true);
            	grdsearchCorpFarmerNoLog.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdsearchCorpFarmerNoLog") {
            	if(grdsearchCorpFarmerNoLog.getRowStatus(nRow) == 0 || grdsearchCorpFarmerNoLog.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var searchCorpFarmerNoLogVO = grdsearchCorpFarmerNoLog.getRowData(nRow);
            			fn_deleteRsrc(searchCorpFarmerNoLogVO);
            			grdsearchCorpFarmerNoLog.deleteRow(nRow);
            		}
            	}else{
            		grdsearchCorpFarmerNoLog.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(searchCorpFarmerNoLogVO){
		let postJsonPromise = gfn_postJSON("/fm/farm/deleteSearchCorpFarmerNoLog.do", searchCorpFarmerNoLogVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		...
		//grdsearchCorpFarmerNoLog 그리드 객체
        selGridRow = grdsearchCorpFarmerNoLog.getRow();
        selGridCol = grdsearchCorpFarmerNoLog.getCol();


        let delYnCol = grdsearchCorpFarmerNoLog.getColRef('delYn');
        let delYnValue = grdsearchCorpFarmerNoLog.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdsearchCorpFarmerNoLog.getColRef('cc');
        let ddCol = grdsearchCorpFarmerNoLog.getColRef('dd');
        let iiCol = grdsearchCorpFarmerNoLog.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdsearchCorpFarmerNoLog.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdsearchCorpFarmerNoLog.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdsearchCorpFarmerNoLog.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdsearchCorpFarmerNoLog.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdsearchCorpFarmerNoLog.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdsearchCorpFarmerNoLog.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdsearchCorpFarmerNoLog.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdsearchCorpFarmerNoLog.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdsearchCorpFarmerNoLog.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
	...
	const fn_saveFrmerSn= async function(){
		...
		if(confirm(regMsg)){
			...
			//let postJsonPromise = gfn_postJSON("/co/cd/multiSaveComCdDtlList.do", saveList);
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveReleySearchCorpFarmerNoLogList.do", {
	    		 frmerSn : frmerSn
	 		});
	        let data = await postJsonPromise;
	        try {
