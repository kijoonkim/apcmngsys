		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdsearchFarmerNo" style="height:650px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonsearchFarmerNo = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdsearchFarmerNo', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdsearchFarmerNo';
	    SBGridProperties.id = 'grdsearchFarmerNo';
	    SBGridProperties.jsonref = 'jsonsearchFarmerNo';
	    ...
	    SBGridProperties.columns = [
	        ...
	        {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdsearchFarmerNo\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdsearchFarmerNo\", " + nRow + ")'>삭제</button>";
	        	}
	        }}
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdsearchFarmerNo\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdsearchFarmerNo\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdsearchFarmerNo\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	    ...
	    grdsearchFarmerNo = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdsearchFarmerNo.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdsearchFarmerNo.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	let postJsonPromise = gfn_postJSON("/fm/farm/selectSearchFarmerNoList.do", {
    		 frmerSn : frmerSn
    		,bzobRgno : bzobRgno
		});
        let data = await postJsonPromise;
        try{
        	jsonsearchFarmerNo.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let searchFarmerNoVO = {
				...
				jsonsearchFarmerNo.push(searchFarmerNoVO);
			});



        	grdsearchFarmerNo.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdsearchFarmerNo.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdsearchFarmerNo.getGridDataAll();
		let saveList = [];
		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdsearchFarmerNo.getRowData(i);
			let rowSts = grdsearchFarmerNo.getRowStatus(i);
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveSearchFarmerNoList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdsearchFarmerNo") {
            	grdsearchFarmerNo.setCellData(nRow, nCol, "N", true);
            	//grdsearchFarmerNo.setCellData(nRow, 5, gv_apcCd, true);
            	grdsearchFarmerNo.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdsearchFarmerNo") {
            	if(grdsearchFarmerNo.getRowStatus(nRow) == 0 || grdsearchFarmerNo.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var searchFarmerNoVO = grdsearchFarmerNo.getRowData(nRow);
            			fn_deleteRsrc(searchFarmerNoVO);
            			grdsearchFarmerNo.deleteRow(nRow);
            		}
            	}else{
            		grdsearchFarmerNo.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(searchFarmerNoVO){
		let postJsonPromise = gfn_postJSON("/fm/farm/deleteSearchFarmerNo.do", searchFarmerNoVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		console.log("================gridClick================");
		//grdsearchFarmerNo 그리드 객체
        selGridRow = grdsearchFarmerNo.getRow();
        selGridCol = grdsearchFarmerNo.getCol();


        let delYnCol = grdsearchFarmerNo.getColRef('delYn');
        let delYnValue = grdsearchFarmerNo.getCellData(selGridRow,delYnCol);
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdsearchFarmerNo.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdsearchFarmerNo.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdsearchFarmerNo.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdsearchFarmerNo.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdsearchFarmerNo.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdsearchFarmerNo.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdsearchFarmerNo.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdsearchFarmerNo.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdsearchFarmerNo.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
	...
	const fn_saveFrmerSn= async function(){
		...
		if(confirm(regMsg)){
			...
			//let postJsonPromise = gfn_postJSON("/co/cd/multiSaveComCdDtlList.do", saveList);
			// let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveReleysearchFarmerNoList.do", {
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveReleySearchFarmerNoJsoneList.do", {
	    		 frmerSn : frmerSn
	 		});
	        let data = await postJsonPromise;
	        try {
