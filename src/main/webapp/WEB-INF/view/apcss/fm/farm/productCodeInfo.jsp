		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdproductCodeInfo" style="height:650px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonproductCodeInfo = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdproductCodeInfo', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdproductCodeInfo';
	    SBGridProperties.id = 'grdproductCodeInfo';
	    SBGridProperties.jsonref = 'jsonproductCodeInfo';
	    ...
	    SBGridProperties.columns = [
	        ...
	        {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdproductCodeInfo\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdproductCodeInfo\", " + nRow + ")'>삭제</button>";
	        	}
	        }}
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdproductCodeInfo\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdproductCodeInfo\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdproductCodeInfo\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	    ...
	    grdproductCodeInfo = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdproductCodeInfo.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdproductCodeInfo.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
        ...
        try{
        	jsonproductCodeInfo.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let productCodeInfoVO = {
				...
				jsonproductCodeInfo.push(productCodeInfoVO);
			});



        	grdproductCodeInfo.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdproductCodeInfo.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdproductCodeInfo.getGridDataAll();
		let saveList = [];
		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdproductCodeInfo.getRowData(i);
			let rowSts = grdproductCodeInfo.getRowStatus(i);
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdproductCodeInfo") {
            	grdproductCodeInfo.setCellData(nRow, nCol, "N", true);
            	//grdproductCodeInfo.setCellData(nRow, 5, gv_apcCd, true);
            	grdproductCodeInfo.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdproductCodeInfo") {
            	if(grdproductCodeInfo.getRowStatus(nRow) == 0 || grdproductCodeInfo.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var productCodeInfoVO = grdproductCodeInfo.getRowData(nRow);
            			fn_deleteRsrc(productCodeInfoVO);
            			grdproductCodeInfo.deleteRow(nRow);
            		}
            	}else{
            		grdproductCodeInfo.deleteRow(nRow);
            	}
    ...
    function gridClick(){
		console.log("================gridClick================");
		//grdproductCodeInfo 그리드 객체
        selGridRow = grdproductCodeInfo.getRow();
        selGridCol = grdproductCodeInfo.getCol();


        let delYnCol = grdproductCodeInfo.getColRef('delYn');
        let delYnValue = grdproductCodeInfo.getCellData(selGridRow,delYnCol);
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdproductCodeInfo.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdproductCodeInfo.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdproductCodeInfo.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdproductCodeInfo.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdproductCodeInfo.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdproductCodeInfo.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdproductCodeInfo.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdproductCodeInfo.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdproductCodeInfo.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
