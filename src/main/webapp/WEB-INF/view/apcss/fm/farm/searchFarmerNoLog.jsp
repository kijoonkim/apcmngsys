...
<!-- 					<sbux-button id="btnSaveFrm" name="btnSaveFrm" uitype="normal" text="연계 등록" class="btn btn-sm btn-outline-danger" onclick="fn_saveFrmerSn"></sbux-button> -->
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdsearchFarmerNoLog" style="height:600px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonsearchFarmerNoLog = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdsearchFarmerNoLog', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdsearchFarmerNoLog';
	    SBGridProperties.id = 'grdsearchFarmerNoLog';
	    SBGridProperties.jsonref = 'jsonsearchFarmerNoLog';
	    ...
	    SBGridProperties.columns = [
	       ...
	       /*  {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdsearchFarmerNoLog\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdsearchFarmerNoLog\", " + nRow + ")'>삭제</button>";
	        	}
	        }} */
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdsearchFarmerNoLog\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdsearchFarmerNoLog\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdsearchFarmerNoLog\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	    ...
	    grdsearchFarmerNoLog = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdsearchFarmerNoLog.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdsearchFarmerNoLog.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/fm/farm/selectsearchFarmerNoLog.do", {apcCd : apcCd});
    	let postJsonPromise = gfn_postJSON("/fm/farm/selectSearchFarmerNoLogList.do", {
    		 frmerSn : frmerSn
    		,bzobRgno : bzobRgno
		});
        let data = await postJsonPromise;
        try{
        	jsonsearchFarmerNoLog.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let searchFarmerNoLogVO = {
					seq : item.seq
				...
				jsonsearchFarmerNoLog.push(searchFarmerNoLogVO);
			});



        	grdsearchFarmerNoLog.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdsearchFarmerNoLog.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdsearchFarmerNoLog.getGridDataAll();
		let saveList = [];

		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdsearchFarmerNoLog.getRowData(i);
			let rowSts = grdsearchFarmerNoLog.getRowStatus(i);
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveSearchFarmerNoLogList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdsearchFarmerNoLog") {
            	grdsearchFarmerNoLog.setCellData(nRow, nCol, "N", true);
            	//grdsearchFarmerNoLog.setCellData(nRow, 5, gv_apcCd, true);
            	grdsearchFarmerNoLog.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdsearchFarmerNoLog") {
            	if(grdsearchFarmerNoLog.getRowStatus(nRow) == 0 || grdsearchFarmerNoLog.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var searchFarmerNoLogVO = grdsearchFarmerNoLog.getRowData(nRow);
            			fn_deleteRsrc(searchFarmerNoLogVO);
            			grdsearchFarmerNoLog.deleteRow(nRow);
            		}
            	}else{
            		grdsearchFarmerNoLog.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(searchFarmerNoLogVO){
		let postJsonPromise = gfn_postJSON("/fm/farm/deleteSearchFarmerNoLog.do", searchFarmerNoLogVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		...
		//grdsearchFarmerNoLog 그리드 객체
        selGridRow = grdsearchFarmerNoLog.getRow();
        selGridCol = grdsearchFarmerNoLog.getCol();


        let delYnCol = grdsearchFarmerNoLog.getColRef('delYn');
        let delYnValue = grdsearchFarmerNoLog.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdsearchFarmerNoLog.getColRef('cc');
        let ddCol = grdsearchFarmerNoLog.getColRef('dd');
        let iiCol = grdsearchFarmerNoLog.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdsearchFarmerNoLog.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdsearchFarmerNoLog.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdsearchFarmerNoLog.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdsearchFarmerNoLog.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdsearchFarmerNoLog.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdsearchFarmerNoLog.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdsearchFarmerNoLog.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdsearchFarmerNoLog.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdsearchFarmerNoLog.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
	...
	const fn_saveFrmerSn= async function(){
		...
		if(confirm(regMsg)){
			...
			//let postJsonPromise = gfn_postJSON("/co/cd/multiSaveComCdDtlList.do", saveList);
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveReleySearchFarmerNoLogList.do", {
	    		 frmerSn : frmerSn
	 		});
	        let data = await postJsonPromise;
	        try {
