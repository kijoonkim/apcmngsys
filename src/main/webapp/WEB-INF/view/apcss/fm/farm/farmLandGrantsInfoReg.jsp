		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdFarmLandGrantsInfoReg" style="height:150px; width: 100%;"></div>
				</div>
...
<script type="text/javascript">
	...
	var jsonFarmLandGrantsInfoRegList = []; // 그리드의 참조 데이터 주소 선언
	...
	const fn_initSBSelectfarm = async function() {
		...
		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdFarmLandGrantsInfoReg', 		jsonComfarmGubun, 	'farm_GUBUN') 		// 설비구분
		])
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdFarmLandGrantsInfoReg\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdFarmLandGrantsInfoReg\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdFarmLandGrantsInfoReg\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
...
// 	}
	...
	/*생산농가 상세내역 수정*/
    	...
    	const postJsonPromise2 = gfn_postJSON("/fm/farm/updateFarmLandGrantsInfoReg.do", {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdFarmLandGrantsInfoReg") {
            	...
            	//grdFarmLandGrantsInfoReg.setCellData(nRow, 5, gv_apcCd, true);
    ...
    function gridClick(){
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdFarmLandGrantsInfoReg.getColRef('cc');
        let ddCol = grdFarmLandGrantsInfoReg.getColRef('dd');
        let iiCol = grdFarmLandGrantsInfoReg.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdFarmLandGrantsInfoReg.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdFarmLandGrantsInfoReg.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdFarmLandGrantsInfoReg.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdFarmLandGrantsInfoReg.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdFarmLandGrantsInfoReg.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdFarmLandGrantsInfoReg.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdFarmLandGrantsInfoReg.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdFarmLandGrantsInfoReg.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdFarmLandGrantsInfoReg.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
