		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdsearchCorpFarmerNo" style="height:650px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonsearchCorpFarmerNo = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdsearchCorpFarmerNo', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdsearchCorpFarmerNo';
	    SBGridProperties.id = 'grdsearchCorpFarmerNo';
	    SBGridProperties.jsonref = 'jsonsearchCorpFarmerNo';
	    ...
	    SBGridProperties.columns = [
	        ...
	        {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdsearchCorpFarmerNo\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdsearchCorpFarmerNo\", " + nRow + ")'>삭제</button>";
	        	}
	        }}
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdsearchCorpFarmerNo\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdsearchCorpFarmerNo\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdsearchCorpFarmerNo\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	    ...
	    grdsearchCorpFarmerNo = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdsearchCorpFarmerNo.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdsearchCorpFarmerNo.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	let postJsonPromise = gfn_postJSON("/fm/farm/selectSearchCorpFarmerNoList.do", {
    		 frmerSn : frmerSn
    		,bzobRgno : bzobRgno
		});
        let data = await postJsonPromise;
        try{
        	jsonsearchCorpFarmerNo.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let searchCorpFarmerNoVO = {
				...
				jsonsearchCorpFarmerNo.push(searchCorpFarmerNoVO);
			});



        	grdsearchCorpFarmerNo.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdsearchCorpFarmerNo.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdsearchCorpFarmerNo.getGridDataAll();
		let saveList = [];
		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdsearchCorpFarmerNo.getRowData(i);
			let rowSts = grdsearchCorpFarmerNo.getRowStatus(i);
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveSearchCorpFarmerNoList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdsearchCorpFarmerNo") {
            	grdsearchCorpFarmerNo.setCellData(nRow, nCol, "N", true);
            	//grdsearchCorpFarmerNo.setCellData(nRow, 5, gv_apcCd, true);
            	grdsearchCorpFarmerNo.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdsearchCorpFarmerNo") {
            	if(grdsearchCorpFarmerNo.getRowStatus(nRow) == 0 || grdsearchCorpFarmerNo.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var searchCorpFarmerNoVO = grdsearchCorpFarmerNo.getRowData(nRow);
            			fn_deleteRsrc(searchCorpFarmerNoVO);
            			grdsearchCorpFarmerNo.deleteRow(nRow);
            		}
            	}else{
            		grdsearchCorpFarmerNo.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(searchCorpFarmerNoVO){
		let postJsonPromise = gfn_postJSON("/fm/farm/deleteSearchCorpFarmerNo.do", searchCorpFarmerNoVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		console.log("================gridClick================");
		//grdsearchCorpFarmerNo 그리드 객체
        selGridRow = grdsearchCorpFarmerNo.getRow();
        selGridCol = grdsearchCorpFarmerNo.getCol();


        let delYnCol = grdsearchCorpFarmerNo.getColRef('delYn');
        let delYnValue = grdsearchCorpFarmerNo.getCellData(selGridRow,delYnCol);
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdsearchCorpFarmerNo.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdsearchCorpFarmerNo.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdsearchCorpFarmerNo.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdsearchCorpFarmerNo.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdsearchCorpFarmerNo.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdsearchCorpFarmerNo.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdsearchCorpFarmerNo.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdsearchCorpFarmerNo.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdsearchCorpFarmerNo.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
	...
	const fn_saveFrmerSn= async function(){
		...
		if(confirm(regMsg)){
			...
			//let postJsonPromise = gfn_postJSON("/co/cd/multiSaveComCdDtlList.do", saveList);
			// let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveReleysearchCorpFarmerNoList.do", {
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveReleySearchCorpFarmerNoJsoneList.do", {
	    		 frmerSn : frmerSn
	 		});
	        let data = await postJsonPromise;
	        try {
