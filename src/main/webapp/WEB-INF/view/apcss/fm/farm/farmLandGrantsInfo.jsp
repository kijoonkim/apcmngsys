		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdfarmLandGrantsInfo" style="height:650px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonfarmLandGrantsInfo = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdfarmLandGrantsInfo', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdfarmLandGrantsInfo';
	    SBGridProperties.id = 'grdfarmLandGrantsInfo';
	    SBGridProperties.jsonref = 'jsonfarmLandGrantsInfo';
	    ...
	    SBGridProperties.columns = [
	        ...
	        {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdfarmLandGrantsInfo\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdfarmLandGrantsInfo\", " + nRow + ")'>삭제</button>";
	        	}
	        }}
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdfarmLandGrantsInfo\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdfarmLandGrantsInfo\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdfarmLandGrantsInfo\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	    ...
	    grdfarmLandGrantsInfo = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdfarmLandGrantsInfo.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdfarmLandGrantsInfo.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/fm/farm/selectfarmLandGrantsInfo.do", {apcCd : apcCd});
    	let postJsonPromise = gfn_postJSON("/fm/farm/selectfarmLandGrantsInfoList.do", {
    		 frmerSn : frmerSn
    		,bzobRgno : bzobRgno
		});
        let data = await postJsonPromise;
        try{
        	jsonfarmLandGrantsInfo.length = 0;
        	...
        	data.resultList.forEach((item, index) => {
				...
				jsonfarmLandGrantsInfo.push(farmLandGrantsInfoVO);
			});



        	grdfarmLandGrantsInfo.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdfarmLandGrantsInfo.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdfarmLandGrantsInfo.getGridDataAll();
		let saveList = [];
		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdfarmLandGrantsInfo.getRowData(i);
			let rowSts = grdfarmLandGrantsInfo.getRowStatus(i);
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSavefarmLandGrantsInfoList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdfarmLandGrantsInfo") {
            	grdfarmLandGrantsInfo.setCellData(nRow, nCol, "N", true);
            	//grdfarmLandGrantsInfo.setCellData(nRow, 5, gv_apcCd, true);
            	grdfarmLandGrantsInfo.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdfarmLandGrantsInfo") {
            	if(grdfarmLandGrantsInfo.getRowStatus(nRow) == 0 || grdfarmLandGrantsInfo.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var farmLandGrantsInfoVO = grdfarmLandGrantsInfo.getRowData(nRow);
            			fn_deleteRsrc(farmLandGrantsInfoVO);
            			grdfarmLandGrantsInfo.deleteRow(nRow);
            		}
            	}else{
            		grdfarmLandGrantsInfo.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(farmLandGrantsInfoVO){
		let postJsonPromise = gfn_postJSON("/fm/farm/deletefarmLandGrantsInfo.do", farmLandGrantsInfoVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		console.log("================gridClick================");
		//grdfarmLandGrantsInfo 그리드 객체
        selGridRow = grdfarmLandGrantsInfo.getRow();
        selGridCol = grdfarmLandGrantsInfo.getCol();


        let delYnCol = grdfarmLandGrantsInfo.getColRef('delYn');
        let delYnValue = grdfarmLandGrantsInfo.getCellData(selGridRow,delYnCol);
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdfarmLandGrantsInfo.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdfarmLandGrantsInfo.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdfarmLandGrantsInfo.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdfarmLandGrantsInfo.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdfarmLandGrantsInfo.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdfarmLandGrantsInfo.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdfarmLandGrantsInfo.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdfarmLandGrantsInfo.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdfarmLandGrantsInfo.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
	...
	const fn_saveFrmerSn= async function(){
		...
		if(confirm(regMsg)){
			...
			//let postJsonPromise = gfn_postJSON("/co/cd/multiSaveComCdDtlList.do", saveList);
			// let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveReleyfarmLandGrantsInfoList.do", {
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveReleyfarmLandGrantsInfoJsoneList.do", {
	    		 frmerSn : frmerSn
	 		});
	        let data = await postJsonPromise;
	        try {
