...
<!-- 					<sbux-button id="btnSaveFrm" name="btnSaveFrm" uitype="normal" text="연계 등록" class="btn btn-sm btn-outline-danger" onclick="fn_saveFrmerSn"></sbux-button> -->
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdproductCodeInfoLog" style="height:600px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonproductCodeInfoLog = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdproductCodeInfoLog', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdproductCodeInfoLog';
	    SBGridProperties.id = 'grdproductCodeInfoLog';
	    SBGridProperties.jsonref = 'jsonproductCodeInfoLog';
	    ...
	    SBGridProperties.columns = [
	       ...
	       /*  {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdproductCodeInfoLog\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdproductCodeInfoLog\", " + nRow + ")'>삭제</button>";
	        	}
	        }} */
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdproductCodeInfoLog\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdproductCodeInfoLog\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdproductCodeInfoLog\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	    ...
	    grdproductCodeInfoLog = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdproductCodeInfoLog.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdproductCodeInfoLog.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/fm/farm/selectproductCodeInfoLog.do", {apcCd : apcCd});
    	let postJsonPromise = gfn_postJSON("/fm/farm/selectProductCodeInfoLogList.do", {
    		 frmerSn : frmerSn
    		,bzobRgno : bzobRgno
		});
        let data = await postJsonPromise;
        try{
        	jsonproductCodeInfoLog.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let productCodeInfoLogVO = {
					seq : item.seq
				...
				jsonproductCodeInfoLog.push(productCodeInfoLogVO);
			});



        	grdproductCodeInfoLog.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdproductCodeInfoLog.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdproductCodeInfoLog.getGridDataAll();
		let saveList = [];

		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdproductCodeInfoLog.getRowData(i);
			let rowSts = grdproductCodeInfoLog.getRowStatus(i);
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveProductCodeInfoLogList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdproductCodeInfoLog") {
            	grdproductCodeInfoLog.setCellData(nRow, nCol, "N", true);
            	//grdproductCodeInfoLog.setCellData(nRow, 5, gv_apcCd, true);
            	grdproductCodeInfoLog.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdproductCodeInfoLog") {
            	if(grdproductCodeInfoLog.getRowStatus(nRow) == 0 || grdproductCodeInfoLog.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var productCodeInfoLogVO = grdproductCodeInfoLog.getRowData(nRow);
            			fn_deleteRsrc(productCodeInfoLogVO);
            			grdproductCodeInfoLog.deleteRow(nRow);
            		}
            	}else{
            		grdproductCodeInfoLog.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(productCodeInfoLogVO){
		let postJsonPromise = gfn_postJSON("/fm/farm/deleteProductCodeInfoLog.do", productCodeInfoLogVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		...
		//grdproductCodeInfoLog 그리드 객체
        selGridRow = grdproductCodeInfoLog.getRow();
        selGridCol = grdproductCodeInfoLog.getCol();


        let delYnCol = grdproductCodeInfoLog.getColRef('delYn');
        let delYnValue = grdproductCodeInfoLog.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdproductCodeInfoLog.getColRef('cc');
        let ddCol = grdproductCodeInfoLog.getColRef('dd');
        let iiCol = grdproductCodeInfoLog.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdproductCodeInfoLog.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdproductCodeInfoLog.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdproductCodeInfoLog.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdproductCodeInfoLog.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdproductCodeInfoLog.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdproductCodeInfoLog.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdproductCodeInfoLog.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdproductCodeInfoLog.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdproductCodeInfoLog.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
	...
	const fn_saveFrmerSn= async function(){
		...
		if(confirm(regMsg)){
			...
			//let postJsonPromise = gfn_postJSON("/co/cd/multiSaveComCdDtlList.do", saveList);
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveReleyProductCodeInfoLogList.do", {
	    		 frmerSn : frmerSn
	 		});
	        let data = await postJsonPromise;
	        try {
