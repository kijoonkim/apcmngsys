		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdFarmerSnCertificationInfoReg" style="height:150px; width: 100%;"></div>
				</div>
...
<script type="text/javascript">
	...
	var jsonFarmerSnCertificationInfoRegList = []; // 그리드의 참조 데이터 주소 선언
	...
	const fn_initSBSelectfarm = async function() {
		...
		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdFarmerSnCertificationInfoReg', 		jsonComfarmGubun, 	'farm_GUBUN') 		// 설비구분
		])
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdFarmerSnCertificationInfoReg\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdFarmerSnCertificationInfoReg\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdFarmerSnCertificationInfoReg\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
		...
		grdFarmerSnCertificationInfoReg.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	...
	const fn_searchfarmList = async function(){
        ...
        try{
        	...
        	data.resultList.forEach((item, index) => {
				let farmerSnCertificationInfoRegVO = {
				...
				jsonfarmerSnCertificationInfoReg.push(farmerSnCertificationInfoRegVO);
			});
...
//     		// 변경 저장
	...
	/* Grid Row 저장 기능*/
// 	const fn_saveFmList = async function(){
// 		let gridData = grdFarmerSnCertificationInfoReg.getGridDataAll();
// 		let saveList = [];

// 		for(var i=1; i<=gridData.length; i++ ){
// 			let rowData = grdFarmerSnCertificationInfoReg.getRowData(i);
// 			let rowSts = grdFarmerSnCertificationInfoReg.getRowStatus(i);
...
// 			let postJsonPromise = gfn_postJSON("/fm/farm/multiSavefarmerSnCertificationInfoRegList.do", saveList);
// 	        let data = await postJsonPromise;
// 	        try {
...
// 	}
	...
	/*생산농가 상세내역 수정*/
    	...
    	const postJsonPromise2 = gfn_postJSON("/fm/farm/updateFarmerSnCertificationInfoReg.do", {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdFarmerSnCertificationInfoReg") {
            	...
            	//grdFarmerSnCertificationInfoReg.setCellData(nRow, 5, gv_apcCd, true);
            ...
            if (grid === "grdFarmerSnCertificationInfoReg") {
            	if(grdFarmerSnCertificationInfoReg.getRowStatus(nRow) == 0 || grdFarmerSnCertificationInfoReg.getRowStatus(nRow) == 2){
            		...
            		if(confirm(delMsg)){
            			var farmerSnCertificationInfoRegVO = grdFarmerSnCertificationInfoReg.getRowData(nRow);
            			fn_deleteRsrc(farmerSnCertificationInfoRegVO);
	...
	async function fn_deleteRsrc(farmerSnCertificationInfoRegVO){
		let postJsonPromise = gfn_postJSON("/fm/farm/deleteFarmerSnCertificationInfoReg.do", farmerSnCertificationInfoRegVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		...
		//grdFarmerSnCertificationInfoReg 그리드 객체
        selGridRow = grdFarmerSnCertificationInfoReg.getRow();
        selGridCol = grdFarmerSnCertificationInfoReg.getCol();


        let delYnCol = grdFarmerSnCertificationInfoReg.getColRef('delYn');
        let delYnValue = grdFarmerSnCertificationInfoReg.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdFarmerSnCertificationInfoReg.getColRef('cc');
        let ddCol = grdFarmerSnCertificationInfoReg.getColRef('dd');
        let iiCol = grdFarmerSnCertificationInfoReg.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdFarmerSnCertificationInfoReg.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdFarmerSnCertificationInfoReg.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdFarmerSnCertificationInfoReg.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdFarmerSnCertificationInfoReg.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdFarmerSnCertificationInfoReg.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdFarmerSnCertificationInfoReg.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdFarmerSnCertificationInfoReg.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdFarmerSnCertificationInfoReg.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdFarmerSnCertificationInfoReg.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
