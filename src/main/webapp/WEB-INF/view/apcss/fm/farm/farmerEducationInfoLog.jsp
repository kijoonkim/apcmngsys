...
<script type="text/javascript">
	...
	//설비 등록
	var jsonfarmerEducationInfoLog = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdfarmerEducationInfoLog', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.jsonref = 'jsonfarmerEducationInfoLog';
	    ...
	    SBGridProperties.columns = [
	       ...
	       /*  {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdfarmerEducationInfoLog\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdfarmerEducationInfoLog\", " + nRow + ")'>삭제</button>";
	        	}
	        }} */
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdfarmerEducationInfoLog\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdfarmerEducationInfoLog\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdfarmerEducationInfoLog\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
		...
		grdfarmerEducationInfoLog.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdfarmerEducationInfoLog.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/fm/farm/selectfarmerEducationInfoLog.do", {apcCd : apcCd});
        ...
        try{
        	jsonfarmerEducationInfoLog.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let farmerEducationInfoLogVO = {
					seq : item.seq
				...
				jsonfarmerEducationInfoLog.push(farmerEducationInfoLogVO);
			});
        	...
        	//비어 있는 마지막 줄 추가용도?
        	grdfarmerEducationInfoLog.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdfarmerEducationInfoLog.getGridDataAll();
		let saveList = [];

		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdfarmerEducationInfoLog.getRowData(i);
			let rowSts = grdfarmerEducationInfoLog.getRowStatus(i);
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSavefarmerEducationInfoLogList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdfarmerEducationInfoLog") {
            	grdfarmerEducationInfoLog.setCellData(nRow, nCol, "N", true);
            	//grdfarmerEducationInfoLog.setCellData(nRow, 5, gv_apcCd, true);
            	grdfarmerEducationInfoLog.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdfarmerEducationInfoLog") {
            	if(grdfarmerEducationInfoLog.getRowStatus(nRow) == 0 || grdfarmerEducationInfoLog.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var farmerEducationInfoLogVO = grdfarmerEducationInfoLog.getRowData(nRow);
            			fn_deleteRsrc(farmerEducationInfoLogVO);
            			grdfarmerEducationInfoLog.deleteRow(nRow);
            		}
            	}else{
            		grdfarmerEducationInfoLog.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(farmerEducationInfoLogVO){
		let postJsonPromise = gfn_postJSON("/fm/farm/deletefarmerEducationInfoLog.do", farmerEducationInfoLogVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		...
		//grdfarmerEducationInfoLog 그리드 객체
        selGridRow = grdfarmerEducationInfoLog.getRow();
        selGridCol = grdfarmerEducationInfoLog.getCol();


        let delYnCol = grdfarmerEducationInfoLog.getColRef('delYn');
        let delYnValue = grdfarmerEducationInfoLog.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdfarmerEducationInfoLog.getColRef('cc');
        let ddCol = grdfarmerEducationInfoLog.getColRef('dd');
        let iiCol = grdfarmerEducationInfoLog.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdfarmerEducationInfoLog.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdfarmerEducationInfoLog.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdfarmerEducationInfoLog.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdfarmerEducationInfoLog.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdfarmerEducationInfoLog.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdfarmerEducationInfoLog.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdfarmerEducationInfoLog.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdfarmerEducationInfoLog.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdfarmerEducationInfoLog.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
	...
	const fn_saveFrmerSn= async function(){
		...
		if(confirm(regMsg)){
			...
			//let postJsonPromise = gfn_postJSON("/co/cd/multiSaveComCdDtlList.do", saveList);
			let postJsonPromise = gfn_postJSON("/fm/farm/multiSaveReleyfarmerEducationInfoLogList.do", {
	    		 frmerSn : frmerSn
	 		});
	        let data = await postJsonPromise;
	        try {
