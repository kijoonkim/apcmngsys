...
<script type="text/javascript">
	...
	const fn_hiddenGrd = async function() {
		...
		SBGridProperties.columns = [
			...
			{caption: ["규격"],			ref:'sortMchnSpcfct',	type:'output',width:'70px',style:'text-align:center'},
			...
			{caption: ["가공설비"],			ref:'mnfcMchn',			type:'output',width:'70px',style:'text-align:center'},
			{caption: ["제조사"],			ref:'mkrNm',			type:'output',width:'70px',style:'text-align:center'},
	...
	const fn_hiddenGrdSelect = async function(){
		...
		let postJsonPromise = gfn_postJSON("/fm/fclt/selectFcltGdsMchnInfoRawDataList.do", {
			crtrYr : crtrYr
		});
		...
		try{
			...
			data.resultList.forEach((item, index) => {
				let hiddenGrdVO = {
					...
					,sortMchnSpcfct		:item.sortMchnSpcfct
	...
	function fn_excelDown(){
		...
		let fileName = formattedDate + "_3.2상품화설비현황_로우데이터";
