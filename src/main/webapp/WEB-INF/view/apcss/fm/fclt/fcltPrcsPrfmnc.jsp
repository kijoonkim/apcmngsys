...
<script type="text/javascript">
	...
	const fn_hiddenGrd = async function() {
		...
		SBGridProperties.columns = [
			...
			{caption: ["APC 일반취급(단순 매취 등)(백만원)"],			ref:'apcGnrlTrmtAmt',	type:'output',width:'70px',style:'text-align:center'},
	...
	const fn_hiddenGrdSelect = async function(){
		...
		let postJsonPromise = gfn_postJSON("/fm/fclt/selectFcltPrcsPrfmncRawDataList.do", {
			crtrYr : crtrYr
		});
		...
		try{
			...
			data.resultList.forEach((item, index) => {
				let hiddenGrdVO = {
					...
					, tmSpmtAmt				: item.tmSpmtAmt
					, tmSpmtRt				: item.tmSpmtRt
	...
	function fn_excelDown(){
		...
		let fileName = formattedDate + "_5.2APC처리실적_로우데이터";
