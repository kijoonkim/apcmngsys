			...
			<table class="table table-bordered tbl_row tbl_fixed">
				...
				<tbody>
					...
					<tr>
						<th>소유조직</th>
						<td>
							<sbux-input id="srch-inp-psnOgnzNm" name="srch-inp-psnOgnzNm" uitype="text" class="form-control input-sm" placeholder="" ></sbux-input>
						</td>
					</tr>
					<tr>
						<th>소유조직 사업자번호</th>
						<td>
							<sbux-input id="srch-inp-psnOgnzBrno" name="srch-inp-psnOgnzBrno" uitype="text" class="form-control input-sm" placeholder="소유자가 지자체로 사업자번호 없을 경우 공란" ></sbux-input>
						</td>
					</tr>
					<tr>
						<th>운영조직</th>
						<td>
							<sbux-input id="srch-inp-operOgnzNm" name="srch-inp-operOgnzNm" uitype="text" class="form-control input-sm" placeholder="최초 페이 내용 그대로 끌어오기" ></sbux-input>
						</td>
					</tr>
					<tr>
						<th>운영조직사업자번호</th>
						<td>
							<sbux-input id="srch-inp-operOgnzBrno" name="srch-inp-operOgnzBrno" uitype="text" class="form-control input-sm" placeholder="" ></sbux-input>
						</td>
					</tr>
					<tr>
						<th>운영조직 대표자</th>
						<td>
							<sbux-input id="srch-inp-operOgnzRprsv" name="srch-inp-operOgnzRprsv" uitype="text" class="form-control input-sm" placeholder="" ></sbux-input>
						</td>
					</tr>
					<tr>
						<th>운영조직 주소</th>
						<td>
							<sbux-input id="operOgnzAddr" name="operOgnzAddr" uitype="text" class="form-control input-sm" placeholder="주소 검색하여 작성" ></sbux-input>
						</td>
					</tr>
					<tr>
						<th>운영조직 조직유형</th>
					...
					<tr>
						<th>운영조직 취급 대표품목1</th>
						<td>
							<sbux-input id="operOgnzTrmtItemCn1" name="operOgnzTrmtItemCn1" uitype="text" class="form-control input-sm" placeholder="운영조직 취급액 중 가장많은 품목 1
							" ></sbux-input>
						</td>
					</tr>
					<tr>
						<th>운영조직 취급 대표품목2</th>
						<td>
							<sbux-input id="operOgnzTrmtItemCn2" name="operOgnzTrmtItemCn2" uitype="text" class="form-control input-sm" placeholder="“해당없음” 옵션 추가 / 운영조직 취급액 중 가장많은 품목 2
							" ></sbux-input>
						</td>
					</tr>
					<tr>
						<th>운영조직 취급 대표품목3</th>
						<td>
							<sbux-input id="operOgnzTrmtItemCn3" name="operOgnzTrmtItemCn3" uitype="text" class="form-control input-sm" placeholder="“해당없음” 옵션 추가 / 운영조직 취급액 중 가장많은 품목 3
							" ></sbux-input>
						</td>
					</tr>
					<tr>
						<th>운영조직 취급 기타품목</th>
						<td>
							<sbux-input id="operOgnzTrmtItemCn4" name="operOgnzTrmtItemCn4" uitype="text" class="form-control input-sm" placeholder="“해당없음” 혹은 추가하기 버튼 누르고 수기입력" ></sbux-input>
						</td>
					</tr>
					<tr>
						<th>APC명</th>
						<td>
							<sbux-input id="srch-inp-apcNm" name="srch-inp-apcNm" uitype="text" class="form-control input-sm" placeholder="최초 페이지 내용 그대로 끌어오기
							" ></sbux-input>
						</td>
					</tr>
					<tr>
						<th>APC 사업자번호</th>
						<td>
							<sbux-input id="srch-inp-apcBrno" name="srch-inp-apcBrno" uitype="text" class="form-control input-sm" placeholder="사업자번호 없을 경우 공란
							" ></sbux-input>
						</td>
					</tr>
					<tr>
						<th>APC 주소</th>
						<td>
							<sbux-input id="srch-inp-apcAddr" name="srch-inp-apcAddr" uitype="text" class="form-control input-sm" placeholder="주소 검색하여 작성
							" ></sbux-input>
						</td>
					</tr>
					<tr>
...
<script type="text/javascript">
    ...
    //grid 초기화
    var grdFcltfcltOperInfoList; // 그리드를 담기위한 객체 선언
    var jsonFcltfcltOperInfoList = []; // 그리드의 참조 데이터 주소 선언

    function fn_createGrid() {
        var SBGridProperties = {};
	    SBGridProperties.parentid = 'sb-area-spmtDsctn';
	    SBGridProperties.id = 'grdFcltfcltOperInfoList';
	    SBGridProperties.jsonref = 'jsonFcltfcltOperInfoList';
        ...
        grdFcltfcltOperInfoList = _SBGrid.create(SBGridProperties);
        grdFcltfcltOperInfoList.bind('click', 'fn_view');
        grdFcltfcltOperInfoList.bind('beforepagechanged', 'fn_pagingFcltfcltOperInfoList');
    }
    ...
    const fn_search = async function() {
    	...
    	// set pagination
    	let pageSize = grdFcltfcltOperInfoList.getPageSize();
    	let pageNo = 1;

    	fn_setGrdFcltfcltOperInfoList(pageSize, pageNo);
    }

    /**
     *
     */
    const fn_pagingFcltfcltOperInfoList = async function() {
    	let recordCountPerPage = grdFcltfcltOperInfoList.getPageSize();   		// 몇개의 데이터를 가져올지 설정
    	let currentPageNo = grdFcltfcltOperInfoList.getSelectPageIndex(); 		// 몇번째 인덱스 부터 데이터를 가져올지 설정
    	fn_setGrdFcltfcltOperInfoList(recordCountPerPage, currentPageNo);
    }
     ...
     * @param {number} pageNo
     */
    const fn_setGrdFcltfcltOperInfoList = async function(pageSize, pageNo) {

    	// form clear
    	fn_clearForm();

		grdFcltfcltOperInfoList.clearStatus();
        ...
        try {
        	...
        	jsonFcltfcltOperInfoList.length = 0;
        	data.resultList.forEach((item, index) => {
				...
				jsonFcltfcltOperInfoList.push(msg);
        	...
        	if (jsonFcltfcltOperInfoList.length > 0) {

        		if(grdFcltfcltOperInfoList.getPageTotalCount() != totalRecordCount){   // TotalCount가 달라지면 rebuild, setPageTotalCount 해주는 부분입니다
        			grdFcltfcltOperInfoList.setPageTotalCount(totalRecordCount); 		// 데이터의 총 건수를 'setPageTotalCount' 메소드에 setting
        			grdFcltfcltOperInfoList.rebuild();
				}else{
					grdFcltfcltOperInfoList.refresh()
				}
        	} else {
        		grdFcltfcltOperInfoList.setPageTotalCount(totalRecordCount);
        		grdFcltfcltOperInfoList.rebuild();
        	}
    ...
    const fn_save = async function() {
    	...
    	let msgKey = SBUxMethod.get("dtl-input-psnOgnzNm");

    	if (!SBUxMethod.get("dtl-input-apcBrno")) {
            alert("apc사업자번호를 입력하세요.");
            return;
        }
    ...
    function fn_delete() {
         ...
         * @type {any[]}
         */
        const rows = grdFcltfcltOperInfoList.getGridDataAll();
        rows.forEach((row) => {
    ...
    function fn_view() {
    	...
    	var nCol = grdFcltfcltOperInfoList.getCol();
        //특정 열 부터 이벤트 적용
        if (nCol < 1) {
            return;
        }
        var nRow = grdFcltfcltOperInfoList.getRow();
		if (nRow < 1) {
            return;
		}

        let rowData = grdFcltfcltOperInfoList.getRowData(nRow);
    ...
    function fn_checkAll(obj){
        var gridList = grdFcltfcltOperInfoList.getGridDataAll();
        var checkedYn = obj.checked ? "Y" : "N";
        for (var i=0; i<gridList.length; i++ ){
        	grdFcltfcltOperInfoList.setCellData(i+1, 1, checkedYn, true, false);
        }
    }
