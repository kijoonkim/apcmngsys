		...
		<div class="box-header" style="display:flex; justify-content: flex-start;" >
			<div>
				<c:set scope="request" var="menuNm" value="${comMenuVO.menuNm}"></c:set>
					<h3 class="box-title"> ▶ <c:out value='${menuNm}'></c:out></h3><!-- 전사조사일정관리 -->
			</div>
...
<script type="text/javascript">
    ...
    const fn_setGrdComMsgList = async function(pageSize, pageNo) {
        ...
        const postJsonPromise = gfn_postJSON("/fm/fclt/selectＷholExmnSchdlMngList.do", {
        	msgKnd: msgKnd,
        	msgKey: msgKey,
        	msgCn: msgCn,
        	// pagination
    ...
    const fn_subInsert = async function (isConfirmed){
    	...
    	const postJsonPromise = gfn_postJSON("/fm/fclt/insertＷholExmnSchdlMng.do", {
	...
	const fn_subUpdate = async function (isConfirmed){
    	...
    	const postJsonPromise = gfn_postJSON("/fm/fclt/updateＷholExmnSchdlMng.do", {
    ...
    const fn_subDelete = async function (isConfirmed, list){
     	...
     	const postJsonPromise = gfn_postJSON("/fm/fclt/deleteＷholExmnSchdlMngList.do", list);
