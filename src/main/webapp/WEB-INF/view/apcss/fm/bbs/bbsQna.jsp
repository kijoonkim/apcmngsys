...
<body oncontextmenu="return false">
		...
		<div class="box box-solid">
			...
			<div class="box-body">
				<table class="table table-bordered tbl_fixed">
					...
					<tr>
						<th scope="row" class="th_bg">답변상태</th>
						<td class="td_input" style="border-right: hidden;">
							<sbux-select class="form-control input-sm" id="srch-select-ansStts" name="srch-select-ansStts" uitype="single"
								jsondata-ref="jsonComansStts"
								unselected-text="전체"
								style="width:120px;"
							></sbux-select>
						</td>
	...
	<!-- 게시글 팝업 Modal -->
	<div>
		<sbux-modal id="modal-bbsDtl" name="modal-bbsDtl" uitype="middle" header-title="문의게시글" body-html-id="body-modal-bbsDtl" footer-is-close-button="false" style="width:1000px"></sbux-modal>
	</div>
	<div id="body-modal-bbsDtl">
		<jsp:include page="/WEB-INF/view/apcss/fm/bbs/popup/bbsQnaDtl.jsp"></jsp:include>
	</div>
...
<script type="text/javascript">
	...
	const fn_initSBSelect = async function() {
		...
		let rst = await Promise.all([
			...
			gfn_setComCdSBSelect('srch-select-ansStts', 	jsonComansStts, 	'ANS_STTS'), 	//답변상태
		]);
	}
	...
	function fn_createGrid() {
		...
		SBGridProperties.columns = [
			...
			{caption: ["답변상태"],  	ref: 'ansSttsNm',		type:'output',  width:'10%',    style:'text-align:center'},
	...
	const fn_create = function() {
		popBbsDtl.init(fn_bbsDtlCallback , '2' , null);
	...
	function fn_openMaodalBbsDtl(nRow){
		...
		//공통코드 BBS_SE_CD 확인
		popBbsDtl.init(fn_bbsDtlCallback , '2' , bbsNo);
		SBUxMethod.openModal('modal-bbsDtl');
	}
	...
	const fn_report = async function() {
		...
		gfn_popClipReport("문의사항리스트", "pd/bbsDoc.crf", {
		...
		try {
			...
			data.resultList.forEach((item, index) => {
				const msg = {
					...
					ansStts:				item.ansStts,
					ansSttsNm:				item.ansSttsNm,
