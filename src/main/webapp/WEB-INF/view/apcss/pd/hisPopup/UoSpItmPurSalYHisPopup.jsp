	...
	<section>
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="box-header" style="display:flex; justify-content: flex-start;" >
					<div style="margin-left: auto;">
						<sbux-button id="prdcrOgnHis-btnSearch-prdcrOgnz" name="prdcrOgnHis-btnSearch-prdcrOgnz" uitype="normal" text="조회" class="btn btn-sm btn-outline-danger" onclick="fn_searchUoSpItmPurSalY"></sbux-button>
					</div>
				</div>
					...
					<div class="ad_tbl_top">
						<ul class="ad_tbl_count">
							<li>
								...
								<!--
								<span style="font-size:12px">(조회건수 <span id="listCntUoSpItmPurSalY">0</span>건)</span>
								-->
							</li>
						</ul>
					</div>
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdUoSpItmPurSalY" style="height:650px; width: 100%;"></div>
				</div>
				<br>
			</div>
		</div>
...
<script type="text/javascript">
	...
	const fn_init = async function() {
		...
		await fn_createGridUoSpItmPurSalY(year); //통합조직 전문품목 리스트 그리드
	...
	var jsonUoSpItmPurSalY = []; // 그리드의 참조 데이터 주소 선언
	var grdUoSpItmPurSalY;
	...
	function fn_excelDwnld02() {
		...
		grdUoSpItmPurSalY.exportLocalExcel( fileNm, {bSaveLabelData: true, bNullToBlank: true, bSaveSubtotalValue: true, bCaptionConvertBr: true, arrSaveConvertText: true});
	}

	/* 통합조직 전문품목 매입매출 그리드 생성 */
	const fn_createGridUoSpItmPurSalY = async function(_year) {
		// 년도에 따른 설정 가져오기
		const { columnsUoSpItmPurSalY } = yrSettings[_year] || defaultSettings;

		let SBGridProperties = {};
		SBGridProperties.parentid = 'sb-area-grdUoSpItmPurSalY';
		SBGridProperties.id = 'grdUoSpItmPurSalY';
		SBGridProperties.jsonref = 'jsonUoSpItmPurSalY';
		...
		SBGridProperties.columns = columnsUoSpItmPurSalY;

		grdUoSpItmPurSalY = _SBGrid.create(SBGridProperties);
		//클릭 이벤트 바인드
		//grdUoSpItmPurSalY.bind('click','fn_viewUoSpItmPurSalN');
		//grdUoSpItmPurSalY.bind('keyup','fn_keyupUoSpItmPurSalN');
	}
	...
	const fn_clearGridUo = async function() {
		jsonUoSpItmPurSalY.length = 0;
		grdUoSpItmPurSalY.rebuild();
		//document.querySelector('#listCntUoSpItmPurSalY').innerText = 0;
	}
	...
	const fn_search = async function() {
		...
		//년도에 따른 그리드 변경 처리
		await fn_createGridUoSpItmPurSalY(yr); //생산자조직 리스트
	...
	const fn_searchUoSpItmPurSalY = async function() {
		...
		// 년도에 따른 설정 가져오기
		const { urlUoSpItmPurSalY } = yrSettings[yr] || defaultSettings;

		let postJsonPromise = gfn_postJSON(urlUoSpItmPurSalY, {
			yr : yr
			,brno : brno
		});
		let data = await postJsonPromise;
		try{
			...
			data.resultList.forEach((item, index) => {
				...
				jsonUoSpItmPurSalY.push(itemVo);

			});
			//document.querySelector('#listCntUoSpItmPurSalY').innerText = jsonUoSpItmPurSalY.length;
			grdUoSpItmPurSalY.rebuild();
			fn_gridCustom();
		}catch (e) {
			if (!(e instanceof Error)) {
	...
	const fn_gridCustom = async function(){
		let objGrid = grdUoSpItmPurSalY;
		let yr = SBUxMethod.get('dtl-inp-yr');

		//console.log("=========fn_gridCustom================");
		let gridData01 = objGrid.getGridDataAll();

		let seNm = objGrid.getColRef("seNm");//구분
		let seDtlNm = objGrid.getColRef("seDtlNm");//구분상세
		//let sttgUpbrItemNm = objGrid.getColRef("sttgUpbrItemNm");//품목구분
		let trmtTypeNm = objGrid.getColRef("trmtTypeNm");//취급유형
		let prchsNm = objGrid.getColRef("prchsNm");//매입처
		let slsCnsgnPrchsAmt = objGrid.getColRef("slsCnsgnPrchsAmt");//판매위임 매입 금액 인덱스
		let slsCnsgnSlsAmt = objGrid.getColRef("slsCnsgnSlsAmt");//판매위임 매출 금액 인덱스

		let slsCnsgnPrchsVlm = objGrid.getColRef("slsCnsgnPrchsVlm");//판매위임 매입 물량 인덱스
		let slsCnsgnSlsVlm = objGrid.getColRef("slsCnsgnSlsVlm");//판매위임 매출 물량 인덱스

		let rmrk = objGrid.getColRef("rmrk");//비고
		...
		for(var i=2; i <= gridData01.length+1; i++ ){
			...
			if(rowData01.typeSeNo == '1'){
				//배경 속성 추가
				if(yr == '2024'){
					objGrid.setCellStyle('background-color', i, slsCnsgnSlsAmt, i, slsCnsgnSlsAmt, 'lightgray');
				}
				objGrid.setCellStyle('background-color', i, slsCnsgnPrchsVlm, i, slsCnsgnPrchsVlm, 'lightgreen');
				objGrid.setCellStyle('background-color', i, slsCnsgnPrchsAmt, i, slsCnsgnPrchsAmt, 'lightgreen');
				//해당 타입 위치 저장
			...
			if(rowData01.typeSeNo == '2'){
				// 배경 속성 추가
				objGrid.setCellStyle('background-color', i, trmtTypeNm, i, rmrk, 'lightgray');
				if(yr == '2024'){
					objGrid.setCellStyle('background-color', i, slsCnsgnSlsAmt, i, slsCnsgnSlsAmt, 'white');
				}
				//셀단위 병합
				objGrid.setMergeByFree(i,trmtTypeNm,i,prchsNm,true);
			}
			...
			if(rowData01.typeSeNo == '4'){
				// 배경 속성 추가
				objGrid.setCellStyle('background-color', i, seDtlNm, i, rmrk, 'lightgray');
				//셀단위 병합
				objGrid.setMergeByFree(i,seDtlNm,i,prchsNm,true);
			}
			...
			if(rowData01.typeSeNo == '6'){
				if(rowData01.trmtType == '0'){
					...
					// 배경 속성 추가
					objGrid.setCellStyle('background-color', i, trmtTypeNm, i, slsCnsgnPrchsAmt, 'lightgray');
					objGrid.setCellStyle('background-color', i, slsCnsgnSlsVlm, i, slsCnsgnSlsAmt, 'lightgray');
					objGrid.setCellStyle('background-color', i, rmrk, i, rmrk, 'lightgray');
				}
				//셀단위 병합
				objGrid.setMergeByFree(i,trmtTypeNm,i,prchsNm,true);
			}
			...
			if(rowData01.typeSeNo == '8'){
				//배경 속성 추가
				objGrid.setCellStyle('background-color', i, seNm, i, rmrk, 'lightgray');
				//셀단위 병합
				objGrid.setMergeByFree(i,seNm,i,prchsNm,true);
			}
		}
		//판매 위임 매입 금액 세로 셀단위 병합
		//objGrid.setMergeByFree([0],slsCnsgnSlsAmt,[typeSeOne.length-1],slsCnsgnSlsAmt,true);
		//objGrid.setMergeByFree([0],slsCnsgnSlsAmt,[typeSeFive.length-1],slsCnsgnSlsAmt,true);
	}
	...
	function fn_grdTot(){
		...
		//그리드 추가 용 1줄 합계용 1줄
		let objGrid = grdUoSpItmPurSalY;
		let grdJson = jsonUoSpItmPurSalY;
	...
	const columnsUoSpItmPurSalY2024 = [
		{caption : ['품목명','품목명'],
	...
	const columnsUoSpItmPurSalY2025 = [
		{caption : ['품목명','품목명'],
			ref : "itemNm",   width : '100px',        style : 'text-align:center',     type : 'output'},
	...
	const yrSettings = {
		"2024": {
			columnsUoSpItmPurSalY : columnsUoSpItmPurSalY2024
			, urlUoSpItmPurSalY : '/pd/hisPopup/selectHisUoSpItmPurSalY2024.do'
		},
		"2025": {
			columnsUoSpItmPurSalY : columnsUoSpItmPurSalY2025
			, urlUoSpItmPurSalY : '/pd/hisPopup/selectHisUoSpItmPurSalY2025.do'
		},
	};
	//기본 설정
