		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdPrdcrOgnGenalTblMng" style="height:350px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonPrdcrOgnGenalTblMng = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdPrdcrOgnGenalTblMng', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdPrdcrOgnGenalTblMng';
	    SBGridProperties.id = 'grdPrdcrOgnGenalTblMng';
	    SBGridProperties.jsonref = 'jsonPrdcrOgnGenalTblMng';
	    ...
	    SBGridProperties.columns = [
	    	...
	    	{caption: ["생산자 조직명","생산자 조직명"], 	ref: 'aa',   	type:'combo',  width:'200px',    style:'text-align:center;',
...
// 	        {caption: ["사업자등록번호"], 	ref: 'bb',   type:'input',  width:'250px',    style:'text-align:center', validate : gfn_chkByte.bind({byteLimit: 100})},
	        {caption: ["생산자조직 생산","조직원수"], 	ref: 'ii',   	type:'output',  width:'180px',    style:'text-align:center'},
	        {caption: ["생산자조직 생산","생산면적(㎡)"], 	ref: 'iiCode',   	type:'input',  width:'180px',    style:'text-align:center' ,  editable : false},
	        {caption: ["생산자조직 생산","생산액(A)(천원)"], 		ref: 'cc',   type:'output',  width:'200px',    style:'text-align:center', validate : gfn_chkByte.bind({byteLimit: 1000})},
	        {caption: ["생산자조직 출하","출하액(B)(천원)"], 	ref: 'ccCode',   	type:'input', width:'180px', style:'text-align:center' ,  editable : false},
	        {caption: ["생산자조직 출하","출하율(B/A)"], 	ref: 'dd',   	type:'output',  width:'100px',    style:'text-align:center', typeinfo : {mask : {alias : 'numeric'}}},
	        {caption: ["전속출하 대상 조직명","전속출하 대상 조직명"], 	ref: 'ddCode',   	type:'input',width:'80px', style:'text-align:center' ,  editable : false},
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdPrdcrOgnGenalTblMng\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdPrdcrOgnGenalTblMng\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdPrdcrOgnGenalTblMng\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	        ...
	        */
	        {caption: ["처리","처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdPrdcrOgnGenalTblMng\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdPrdcrOgnGenalTblMng\", " + nRow + ")'>삭제</button>";
	        	}
	        }}
	    ];

	    grdPrdcrOgnGenalTblMng = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdPrdcrOgnGenalTblMng.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdPrdcrOgnGenalTblMng.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/pd/pom/selectPrdcrOgnGenalTblMng.do", {apcCd : apcCd});
    	let postJsonPromise = gfn_postJSON("/pd/pom/selectPrdcrOgnGenalTblMngList.do", {
    		ccCode : ccCode
    		,iiCode : iiCode
		});
        let data = await postJsonPromise;
        try{
        	jsonPrdcrOgnGenalTblMng.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let PrdcrOgnGenalTblMngVO = {
					testNo 		: item.testNo
				  , aa 		: item.aa
				  , bb 		: item.bb
				  , cc 	: item.cc
				...
				jsonPrdcrOgnGenalTblMng.push(PrdcrOgnGenalTblMngVO);
			});



        	grdPrdcrOgnGenalTblMng.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdPrdcrOgnGenalTblMng.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdPrdcrOgnGenalTblMng.getGridDataAll();
		let saveList = [];

		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdPrdcrOgnGenalTblMng.getRowData(i);
			let rowSts = grdPrdcrOgnGenalTblMng.getRowStatus(i);
			let bb = rowData.bb;
			let aa = rowData.aa;
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/pd/pom/multiSavePrdcrOgnGenalTblMngList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdPrdcrOgnGenalTblMng") {
            	grdPrdcrOgnGenalTblMng.setCellData(nRow, nCol, "N", true);
            	//grdPrdcrOgnGenalTblMng.setCellData(nRow, 5, gv_apcCd, true);
            	grdPrdcrOgnGenalTblMng.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdPrdcrOgnGenalTblMng") {
            	if(grdPrdcrOgnGenalTblMng.getRowStatus(nRow) == 0 || grdPrdcrOgnGenalTblMng.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var PrdcrOgnGenalTblMngVO = grdPrdcrOgnGenalTblMng.getRowData(nRow);
            			fn_deleteRsrc(PrdcrOgnGenalTblMngVO);
            			grdPrdcrOgnGenalTblMng.deleteRow(nRow);
            		}
            	}else{
            		grdPrdcrOgnGenalTblMng.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(PrdcrOgnGenalTblMngVO){
		let postJsonPromise = gfn_postJSON("/pd/pom/deletePrdcrOgnGenalTblMng.do", PrdcrOgnGenalTblMngVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		...
		//grdPrdcrOgnGenalTblMng 그리드 객체
        selGridRow = grdPrdcrOgnGenalTblMng.getRow();
        selGridCol = grdPrdcrOgnGenalTblMng.getCol();


        let delYnCol = grdPrdcrOgnGenalTblMng.getColRef('delYn');
        let delYnValue = grdPrdcrOgnGenalTblMng.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdPrdcrOgnGenalTblMng.getColRef('cc');
        let ddCol = grdPrdcrOgnGenalTblMng.getColRef('dd');
        let iiCol = grdPrdcrOgnGenalTblMng.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdPrdcrOgnGenalTblMng.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdPrdcrOgnGenalTblMng.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdPrdcrOgnGenalTblMng.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdPrdcrOgnGenalTblMng.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);

			let grdStatus = grdPrdcrOgnGenalTblMng.getRowStatus(selGridRow);
		 	if(grdStatus != '1'){
		 		grdPrdcrOgnGenalTblMng.setRowStatus(selGridRow,'update');
		 	}
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdPrdcrOgnGenalTblMng.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdPrdcrOgnGenalTblMng.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdPrdcrOgnGenalTblMng.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdPrdcrOgnGenalTblMng.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdPrdcrOgnGenalTblMng.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
