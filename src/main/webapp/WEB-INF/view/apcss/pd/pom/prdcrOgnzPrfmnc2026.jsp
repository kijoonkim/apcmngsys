<%
  /**
   * @Class Name : prdcrOgnzPrfmnc2026.jsp
   * @Description : 생산자조직 현황
   * @author SI개발부
   * @since 2025.11.12
   * @version 1.0
  ...
  <div class="box box-solid">
    ...
    <div class="box-body">
      ...
      <div style="border:1px solid #f4f4f4; background-color: #f4f4f4; border-radius: 10px; padding: 10px;">
        ...
        <br>
        <p>o 통합조직 소속 출자출하조직이 입력한 생산자조직 정보 확인 및 수정 가능</p>
      ...
      <div class="ad_section_top">
        ...
        <div id="pruoPrgrsSttsPrdcrOgnzArea" style="border:1px solid #red; background-color: #ffc0cb; border-radius: 10px; padding: 10px; display: none;  max-height: 200px; overflow: auto;">
          <sbux-label id="dtl-inp-pruoPrgrsStttsPrdcrOgnz" name="dtl-inp-pruoPrgrsStttsPrdcrOgnz" uitype="normal" style="font-weight: bold"></sbux-label>
        </div>
...
<script type="text/javascript">
  ...
  const fn_init = async function() {
    ...
    const crtrYr = SBUxMethod.get("srch-input-yr");
  ...
  const fn_fcltMngCreateGrid01 = async function() {
    ...
    SBGridProperties.columns = [
      ...
      {caption: ["구분"], 	ref: 'sttgUpbrItemSe',   type:'combo',  width:'60px',    style:'text-align:center', disabled:true , oneclickedit:true
        ,typeinfo : {ref:'jsonComGrdSttgUpbrItemSe_1', label:'label', value:'value', displayui : false}},
      {caption: ["부류"], 			ref: 'clsfCd',   	type:'combo',  width:'70px',    style:'text-align:center', disabled:true , oneclickedit:true
        ...
        ,typeinfo : {locale : 'ko' , dateformat :'yyyymm' , yearrange : 150, calendartype : 'yearmonth', displayui : true}, format : {type:'date', rule:'yyyy-mm', origin : 'yyyymm' }},
      {
        ...
        format : {type:'number', rule:'#,###'}, datatype : 'number'
      },
      ...
      {caption: ["적합여부\n(육성->승인)"],	ref: 'extStbltYn',	type:'output',  width:'100px',    style:'text-align:center'},
      /*
  ...
  async function fn_saveFmList01() {
    ...
    let regMsg = "생산자조직 저장 하시겠습니까?";
    if(confirm(regMsg)){

      // let postJsonPromise = gfn_postJSON("/pd/pom/multiSaveTbEvFrmhsApoList.do", saveList);
      const postJsonPromise = gfn_postJSON("/pd/aom/insertPrdcrOgnzList.do", saveList);
      let data = await postJsonPromise;
      try {
  ...
  async function fn_saveFmList02() {
    ...
    const postJsonPromise = gfn_postJSON("/pd/aom/insertFrmhsList.do", saveList);
    let data = await postJsonPromise;
    try {
      if (_.isEqual("S", data.resultStatus)) {
        gfn_comAlert("I0001") 			// I0001 	처리 되었습니다.
  ...
  const fn_view = async function(){
    ...
    console.log("통합조직리스트 클릭시 rowData",rowData);
    ...
    if (_.isEqual(rowData.apoSe, '1')) {
      SBUxMethod.attr('dtl-input-selUoBrno','readonly',true);
    } else if (_.isEqual(rowData.apoSe, '2')) {
      <c:if test="${loginVO.userType eq '01' || loginVO.userType eq '00' || loginVO.userType eq '02' || loginVO.userType eq '91'}">
  ...
  async function fn_clearForm() {
    ...
    grdPrdcrOgnCurntMng01.setColHidden(grdPrdcrOgnCurntMng01.getColRef("extStbltYn"), true);
  }
   ...
   * @function
   */
  const fn_dtlGridSearch01 = async function() {

    /** 진척상태 초기화 **/
    document.getElementById('pruoPrgrsSttsPrdcrOgnzArea').style.display = 'none';
    SBUxMethod.set('dtl-inp-pruoPrgrsStttsPrdcrOgnz',"");
    ...
    const apoSe = SBUxMethod.get('dtl-input-apoSe');
    let uoBrno = null;
    ...
    /** 진행상태 표시 **/
    const apoCd = SBUxMethod.get("dtl-input-apoCd");
    const pruoPrgrsSttsPrdcrOgnz = await gfn_chkPruoPrgrsStts(yr, apoCd, "PD_004_001_01");
    fn_pruoPrgrsStts(pruoPrgrsSttsPrdcrOgnz, "pruoPrgrsSttsPrdcrOgnzArea", "dtl-inp-pruoPrgrsStttsPrdcrOgnz");
    ...
    try {
      ...
      const defaultUrl = "/pd/pom/selectTbEvFrmhsApoList.do";
      const upbrToAprvUrl = "/pd/pom/selectPrdcrOgnzSummaryList.do";
      ...
      const postJsonPromise = gfn_postJSON(postUrl, paramObj);
      const data = await postJsonPromise;
      ...
      if (_.isEqual("S", data['resultStatus'])) {
        ...
        grdPrdcrOgnCurntMng01.setColHidden(grdPrdcrOgnCurntMng01.getColRef("extStbltYn"), needsColHidden);
  ...
  async function fn_dtlGridSearch02() {
    ...
    try {
      ...
      const postJsonPromise = gfn_postJSON("/pd/pom/selectPrdcrOgnCurntMngDtlList.do", {
        yr: yr,
        apoCd: apoCd,
  ...
  const fn_hiddenGrd = async function() {
    ...
    SBGridProperties.rowheader="seq";
    SBGridProperties.columns = [
      {caption: ["등록년도"], 		ref: 'yr',   		type:'output',  width:'180px',    style:'text-align:center'},
      {caption: ["조직구분"], 		ref: 'apoSeNm',   	type:'output',  width:'180px',    style:'text-align:center'},
      {caption: ["통합조직명"], 		ref: 'uoCorpNm',   	type:'output',  width:'180px',    style:'text-align:center'},
      {caption: ["통합조직 사업자등록번호"],ref: 'uoBrno',		type:'output',  width:'180px',    style:'text-align:center'},
      {caption: ["조직명"], 			ref: 'corpNm',   	type:'output',  width:'180px',    style:'text-align:center'},
      {caption: ["사업자등록번호"], 	ref: 'brno',   		type:'output',  width:'180px',    style:'text-align:center'},
      ...
      {caption: ["계약시기"], 		ref: 'ctrtDt',   	type:'output',  width:'140px',    style:'text-align:center'},
      {caption: ["계약물량(톤)\n[A]"], 	ref: 'ecSpmtPlanVlmTot',   	type:'output',  width:'140px',    style:'text-align:center'},
      {caption: ["출하시작시기"], 		ref: 'spmtBgngDt',   	type:'output',  width:'140px',    style:'text-align:center'},
      {caption: ["출하종료시기"], 		ref: 'spmtEndDt',   	type:'output',  width:'140px',    style:'text-align:center'},
      {caption: ["출하량(톤)\n[B]"], 		ref: 'ecSpmtVlmTot',   	type:'output',  width:'140px',    style:'text-align:center'},

      {caption: ["출하대금\n지급액(천원)"], 	ref: 'spmtPrcTot',   	type:'output',  width:'140px',    style:'text-align:center'},

      {caption: ["출하비율(%)\n[B/A]"], 	ref: 'ecSpmtRateB',   	type:'output',  width:'100px',    style:'text-align:center;'},
  ...
  function fn_pruoPrgrsStts(pruoPrgrsStts, areaId, labelId) {
    // pruoPrgrsSttsPrdcrOgnzArea dtl-inp-pruoPrgrsStttsPrdcrOgnz
