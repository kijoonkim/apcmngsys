		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdPrdcrOgnPrdShipRsltMng" style="height:350px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonPrdcrOgnPrdShipRsltMng = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdPrdcrOgnPrdShipRsltMng', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdPrdcrOgnPrdShipRsltMng';
	    SBGridProperties.id = 'grdPrdcrOgnPrdShipRsltMng';
	    SBGridProperties.jsonref = 'jsonPrdcrOgnPrdShipRsltMng';
...
// 	        {caption: ["사업자등록번호"], 	ref: 'bb',   type:'input',  width:'250px',    style:'text-align:center', validate : gfn_chkByte.bind({byteLimit: 100})},
	        ...
	        {caption: ["재배면적(㎡)","계"], 		ref: 'cc',   type:'output',  width:'80px',    style:'text-align:center', validate : gfn_chkByte.bind({byteLimit: 1000})},
	        {caption: ["생산량(결과)(톤)","홍로"], 	ref: 'ccCode',   	type:'input', width:'80px', style:'text-align:center' ,  editable : false},
	        {caption: ["생산량(결과)(톤)","부사"], 	ref: 'dd',   	type:'output',  width:'100px',    style:'text-align:center', typeinfo : {mask : {alias : 'numeric'}}},
	        {caption: ["생산량(결과)(톤)","계"], 	ref: 'ddCode',   	type:'input',width:'80px', style:'text-align:center' ,  editable : false},
	        {caption: ["전속출하량(결과)(톤)","홍로"], 	ref: 'ee',   	type:'input',  width:'100px',    style:'text-align:center', typeinfo : {mask : {alias : 'numeric'}}},
	        {caption: ["전속출하량(결과)(톤)","부사"], 	ref: 'ff',   	type:'input',  width:'100px',    style:'text-align:center', typeinfo : {mask : {alias : 'numeric'}}},
	        {caption: ["전속출하량(결과)(톤)","계"], 	ref: 'gg',   	type:'input',  width:'100px',    style:'text-align:center', typeinfo : {mask : {alias : 'numeric'}}},
	        {caption: ["출하대금 지급액(천원)","출하대금 지급액(천원)"], 	ref: 'hh',   	type:'input',  width:'100px',    style:'text-align:center', typeinfo : {mask : {alias : 'numeric'}}},
	        {caption: ["비고","비고"], 	ref: 'hh',   	type:'input',  width:'120px',    style:'text-align:center', typeinfo : {mask : {alias : 'numeric'}}},
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdPrdcrOgnPrdShipRsltMng\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdPrdcrOgnPrdShipRsltMng\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdPrdcrOgnPrdShipRsltMng\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	        ...
	        */
	        {caption: ["처리","처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdPrdcrOgnPrdShipRsltMng\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdPrdcrOgnPrdShipRsltMng\", " + nRow + ")'>삭제</button>";
	        	}
	        }}
	    ];

	    grdPrdcrOgnPrdShipRsltMng = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdPrdcrOgnPrdShipRsltMng.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdPrdcrOgnPrdShipRsltMng.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/pd/pom/selectPrdcrOgnPrdShipRsltMng.do", {apcCd : apcCd});
    	let postJsonPromise = gfn_postJSON("/pd/pom/selectPrdcrOgnPrdShipRsltMngList.do", {
    		ccCode : ccCode
    		,iiCode : iiCode
		});
        let data = await postJsonPromise;
        try{
        	jsonPrdcrOgnPrdShipRsltMng.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let PrdcrOgnPrdShipRsltMngVO = {
					testNo 		: item.testNo
				  , aa 		: item.aa
				  , bb 		: item.bb
				  , cc 	: item.cc
				...
				jsonPrdcrOgnPrdShipRsltMng.push(PrdcrOgnPrdShipRsltMngVO);
			});



        	grdPrdcrOgnPrdShipRsltMng.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdPrdcrOgnPrdShipRsltMng.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdPrdcrOgnPrdShipRsltMng.getGridDataAll();
		let saveList = [];

		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdPrdcrOgnPrdShipRsltMng.getRowData(i);
			let rowSts = grdPrdcrOgnPrdShipRsltMng.getRowStatus(i);
			let bb = rowData.bb;
			let aa = rowData.aa;
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdPrdcrOgnPrdShipRsltMng") {
            	grdPrdcrOgnPrdShipRsltMng.setCellData(nRow, nCol, "N", true);
            	//grdPrdcrOgnPrdShipRsltMng.setCellData(nRow, 5, gv_apcCd, true);
            	grdPrdcrOgnPrdShipRsltMng.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdPrdcrOgnPrdShipRsltMng") {
            	if(grdPrdcrOgnPrdShipRsltMng.getRowStatus(nRow) == 0 || grdPrdcrOgnPrdShipRsltMng.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var PrdcrOgnPrdShipRsltMngVO = grdPrdcrOgnPrdShipRsltMng.getRowData(nRow);
            			fn_deleteRsrc(PrdcrOgnPrdShipRsltMngVO);
            			grdPrdcrOgnPrdShipRsltMng.deleteRow(nRow);
            		}
            	}else{
            		grdPrdcrOgnPrdShipRsltMng.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(PrdcrOgnPrdShipRsltMngVO){
		let postJsonPromise = gfn_postJSON("/pd/pom/deletePrdcrOgnPrdShipRsltMng.do", PrdcrOgnPrdShipRsltMngVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		...
		//grdPrdcrOgnPrdShipRsltMng 그리드 객체
        selGridRow = grdPrdcrOgnPrdShipRsltMng.getRow();
        selGridCol = grdPrdcrOgnPrdShipRsltMng.getCol();


        let delYnCol = grdPrdcrOgnPrdShipRsltMng.getColRef('delYn');
        let delYnValue = grdPrdcrOgnPrdShipRsltMng.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdPrdcrOgnPrdShipRsltMng.getColRef('cc');
        let ddCol = grdPrdcrOgnPrdShipRsltMng.getColRef('dd');
        let iiCol = grdPrdcrOgnPrdShipRsltMng.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdPrdcrOgnPrdShipRsltMng.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdPrdcrOgnPrdShipRsltMng.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdPrdcrOgnPrdShipRsltMng.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdPrdcrOgnPrdShipRsltMng.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdPrdcrOgnPrdShipRsltMng.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdPrdcrOgnPrdShipRsltMng.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdPrdcrOgnPrdShipRsltMng.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdPrdcrOgnPrdShipRsltMng.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdPrdcrOgnPrdShipRsltMng.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
