...
<head>
	...
	<title>title : 활성화자금 확인 2025</title>
		...
		<div class="box box-solid">
			...
			<c:if test="${loginVO.userType eq '01' || loginVO.userType eq '00' || loginVO.userType eq '02' || loginVO.userType eq '91'}">
				...
				<div class="ad_section_top" style="width: 99%;">
					...
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdScrRslt" style="height:150px; width: 100%;"></div>
				</div>
				<!--[pp] 검색결과 상세보기-->
				<div class="ad_section_top" style="width: 99%;">
					<div class="ad_tbl_top">
...
<script type="text/javascript">
	...
	const fn_fcltMngCreateGrid06 = async function() {
		...
		SBGridProperties.columns = [
			...
			{caption: ["금리(%)"], 		ref: 'itrRt',			type:'output',  width:'80px',    style:'text-align:center;'
				,typeinfo : {mask : {alias: 'decimal', digits : 2}}, format : {type:'number', rule:'#,###.##'}, datatype : 'number'
			},
			{caption: ["선정여부\n(관리자입력)"], 	ref: 'mngrStbltYn',	type:'combo',  width:'90px',    style:'text-align:center;', disabled : true
				,typeinfo : {ref:'comStbltYn', label:'label', value:'value', displayui : false}
			},
	...
	const fn_hiddenGrd02 = async function() {
		...
		SBGridProperties.columns = [
			...
			{caption: [yr + " 신규 자금신청액 통합조직"],			ref:'pruoFundAplyAmt',	type:'output',width:'70px',style:'text-align:center'},
			{caption: [yr + " 신규 자금신청액 출자출하조직"],		ref:'isoFundAplyAmt',	type:'output',width:'70px',style:'text-align:center'},
			{caption: [yr + " 신규 자금신청액 합계"],			ref:'fundAplyAmtTot',	type:'output',width:'70px',style:'text-align:center'},

			{caption: [yr + " 신규 자금신청액(탈락조직 제외) 통합조직"],		ref:'pruoFundAplyAmt1',	type:'output',width:'70px',style:'text-align:center'},
			{caption: [yr + " 신규 자금신청액(탈락조직 제외) 출자출하조직"],		ref:'isoFundAplyAmtStbltTot',	type:'output',width:'70px',style:'text-align:center'},
			{caption: [yr + " 신규 자금신청액(탈락조직 제외) 합계"],			ref:'fundAplyAmtStbltTot',	type:'output',width:'70px',style:'text-align:center'},
			...
			{caption: ["가점"],				ref:'ednstScr',	type:'output',width:'70px',style:'text-align:center'},
