...
<head>
    ...
    <title>title : 등록결과 확인 2025</title>
		...
		<div class="box box-solid">
			<div class="box-header" style="display:flex; justify-content: flex-start;" >
				...
				<div style="margin-left: auto;">
					...
					<sbux-button id="btnRowData02" name="btnRowData02" uitype="normal" text="기존 로우데이터 다운" class="btn btn-sm btn-outline-danger" onclick="fn_hiddenGrdSelect02"></sbux-button>
			...
			<div class="box-body">
				...
				<div class="ad_section_top" style="width: 98%;">
					<div class="ad_tbl_top">
						<ul class="ad_tbl_count">
							<li>
								<span style="font-size:14px">▶ 출자출하조직 일반법인 기본요건 확인</span>
							</li>
						</ul>
					</div>
				-->
				...
				<div class="ad_section_top" style="width: 98%;">
					<div class="ad_tbl_top">
						<ul class="ad_tbl_count">
							<li>
								<span style="font-size:14px">▶ 통합조직 일반법인 기본요건 확인</span>
							</li>
						</ul>
					</div>
				-->
...
<script type="text/javascript">
	...
	*/
	...
	var jsonApoInfo = []; // 그리드의 참조 데이터 주소 선언
	var grdApoInfo;

	const objMenuListApoInfo = {
			"excelDwnld": {
				"name": "엑셀 다운로드",			//컨텍스트메뉴에 표시될 이름
				"accesskey": "e",					//단축키
				"callback": fn_excelDwnldApoInfo,			//콜백함수명
			}
		};


	function fn_excelDwnldApoInfo() {
		grdApoInfo.exportLocalExcel("통합조직 일반법인 기본요건 확인 리스트", {bSaveLabelData: true, bNullToBlank: true, bSaveSubtotalValue: true, bCaptionConvertBr: true, arrSaveConvertText: true});
	}

	/* Grid 화면 그리기 기능*/
	//통합조직 일반법인 기본요건 확인
	const fn_apoInfoCreateGrid = async function() {

		let SBGridProperties = {};
		SBGridProperties.parentid = 'sb-area-grdApoInfo';
		SBGridProperties.id = 'grdApoInfo';
		SBGridProperties.jsonref = 'jsonApoInfo';
		SBGridProperties.emptyrecords = '데이터가 없습니다.';
		SBGridProperties.selectmode = 'byrow';
		SBGridProperties.contextmenu = true;				// 우클린 메뉴 호출 여부
		SBGridProperties.contextmenulist = objMenuListApoInfo;	// 우클릭 메뉴 리스트
		//SBGridProperties.extendlastcol = 'scroll';
		SBGridProperties.emptyareaindexclear = false;//그리드 빈 영역 클릭시 인덱스 초기화 여부
		SBGridProperties.oneclickedit = false;//입력 활성화 true 1번클릭 false 더블클릭
		SBGridProperties.fixedrowheight=52;
		SBGridProperties.rowheight = 57;
		SBGridProperties.rowheader="seq";
		SBGridProperties.columns = [
			{caption: ["사업자번호"],		ref: 'brno',	type:'output',  width:'150px',    style:'text-align:center'},
			{caption: ["업체명"], 			ref: 'corpNm',	type:'output',  width:'55px',    style:'text-align:center;'},
			{caption: ["법인구분"], 		ref: 'corpSeCd',		type:'output',  width:'55px',    style:'text-align:center;'},
			{caption: ["법인형태"],			ref: 'corpDtlSeCd',		type:'output',  width:'70px',    style:'text-align:center'},
			{caption: ["출자금액"], 		ref: 'invstAmt',		type:'output',  width:'160px',    style:'text-align:center;'},
			{caption: ["출자금액확인\n(3억원 이상)"], 	ref: 'd',	type:'output',  width:'160px',    style:'text-align:center;'},
			{caption: ["농업인 출자금"], 	ref: 'frmerInvstAmt',		type:'output',  width:'180px',    style:'text-align:center'},
			{caption: ["생산자단체\n출자금"], 	ref: 'prdcrGrpInvstAmt',		type:'output',  width:'85px',    style:'text-align:center'},
			{caption: ["농업인\n출자지분율"], 	ref: 'frmerInvstAmtRt',   type:'output',  width:'160px',    style:'text-align:center;'},
			{caption: ["농업인\n출자지분율 확인\n(농업회사법인:50% 초과)"], 	ref: 'h',   	type:'output',  width:'160px',    style:'text-align:center;'},
			{caption: ["출자자수"], 		ref: 'invstNope',		type:'output',  width:'160px',    style:'text-align:center;'},
			{caption: ["출자자중\n농업인 수"],	ref: 'invstExpndFrmerNope',		type:'output',  width:'160px',    style:'text-align:center;'},
			{caption: ["농업인수 확인\n(농업법인:20명 이상)"], 	ref: 'k',   	type:'output',  width:'160px',    style:'text-align:center;'},
			{caption: ["농업인 수\n비율 확인\n(협동조합:50%초과)"],	ref: 'l',   	type:'output',  width:'150px',    style:'text-align:center'},
			{caption: ["법인설립일"],		ref: 'corpFndnDay',		type:'output',  width:'150px',    style:'text-align:center'},
			{caption: ["설립 후 운영\n실적 확인\n(3년 이상)"],	ref: 'n',   	type:'output',  width:'150px',    style:'text-align:center'},
			{caption: ["총취급액"],			ref: 'o',		type:'output',  width:'150px',    style:'text-align:center'},
			{caption: ["원에농산물\n취급앱\n(청과,화훼,약용작물류)"],	ref: 'p',   	type:'output',  width:'150px',    style:'text-align:center'},
			{caption: ["원예농산물\n취급비율"],	ref: 'h',		type:'output',  width:'150px',    style:'text-align:center'},
			{caption: ["원예농산물\n취급비율 확인\n(공통:50% 이상)"],ref: 'i',   	type:'output',  width:'150px',    style:'text-align:center'},
		...
		grdApoInfo = _SBGrid.create(SBGridProperties);

	}

	var jsonApoInfo1 = []; // 그리드의 참조 데이터 주소 선언
	var grdApoInfo1;

	const objMenuListApoInfo1 = {
			"excelDwnld": {
				"name": "엑셀 다운로드",			//컨텍스트메뉴에 표시될 이름
				"accesskey": "e",					//단축키
				"callback": fn_excelDwnldApoInfo1,			//콜백함수명
			}
		};


	function fn_excelDwnldApoInfo1() {
		grdApoInfo.exportLocalExcel("통합조직 일반법인 기본요건 확인 리스트", {bSaveLabelData: true, bNullToBlank: true, bSaveSubtotalValue: true, bCaptionConvertBr: true, arrSaveConvertText: true});
	}

	/* Grid 화면 그리기 기능*/
	//통합조직 일반법인 기본요건 확인
	const fn_apoInfoCreateGrid1 = async function() {

		let SBGridProperties = {};
		SBGridProperties.parentid = 'sb-area-grdApoInfo1';
		SBGridProperties.id = 'grdApoInfo1';
		SBGridProperties.jsonref = 'jsonApoInfo1';
		SBGridProperties.emptyrecords = '데이터가 없습니다.';
		SBGridProperties.selectmode = 'byrow';
		SBGridProperties.contextmenu = true;				// 우클린 메뉴 호출 여부
		SBGridProperties.contextmenulist = objMenuListApoInfo1;	// 우클릭 메뉴 리스트
		//SBGridProperties.extendlastcol = 'scroll';
		SBGridProperties.emptyareaindexclear = false;//그리드 빈 영역 클릭시 인덱스 초기화 여부
		SBGridProperties.oneclickedit = false;//입력 활성화 true 1번클릭 false 더블클릭
		SBGridProperties.fixedrowheight=52;
		...
		grdApoInfo1 = _SBGrid.create(SBGridProperties);
	...
	*/
	...
	const fn_listSave01 = async function(){
		...
		//테이블 해더 row수
		let captionRow = grdPrdcrOgnCurntMng06.getFixedRows();
	...
	const fn_listSave02 = async function(){
		...
		//테이블 해더 row수
		let captionRow = grdPrdcrOgnCurntMng07.getFixedRows();
		for(var i = captionRow; i < gridData.length + captionRow; i++ ){
