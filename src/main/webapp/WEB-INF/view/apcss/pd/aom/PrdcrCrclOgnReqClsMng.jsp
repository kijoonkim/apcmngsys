		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdPrdcrCrclOgnReqClsMng" style="height:350px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonPrdcrCrclOgnReqClsMng = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdPrdcrCrclOgnReqClsMng', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdPrdcrCrclOgnReqClsMng';
	    SBGridProperties.id = 'grdPrdcrCrclOgnReqClsMng';
	    SBGridProperties.jsonref = 'jsonPrdcrCrclOgnReqClsMng';
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdPrdcrCrclOgnReqClsMng\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdPrdcrCrclOgnReqClsMng\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdPrdcrCrclOgnReqClsMng\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	        ...
	        */
	        {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdPrdcrCrclOgnReqClsMng\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdPrdcrCrclOgnReqClsMng\", " + nRow + ")'>삭제</button>";
	        	}
	        }}
	    ];

	    grdPrdcrCrclOgnReqClsMng = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdPrdcrCrclOgnReqClsMng.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdPrdcrCrclOgnReqClsMng.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
        ...
        try{
        	jsonPrdcrCrclOgnReqClsMng.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let PrdcrCrclOgnReqClsMngVO = {
					testNo 		: item.testNo
				  , aa 		: item.aa
				  , bb 		: item.bb
				  , cc 	: item.cc
				...
				jsonPrdcrCrclOgnReqClsMng.push(PrdcrCrclOgnReqClsMngVO);
			});



        	grdPrdcrCrclOgnReqClsMng.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdPrdcrCrclOgnReqClsMng.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdPrdcrCrclOgnReqClsMng.getGridDataAll();
		let saveList = [];

		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdPrdcrCrclOgnReqClsMng.getRowData(i);
			let rowSts = grdPrdcrCrclOgnReqClsMng.getRowStatus(i);
			let bb = rowData.bb;
			let aa = rowData.aa;
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdPrdcrCrclOgnReqClsMng") {
            	grdPrdcrCrclOgnReqClsMng.setCellData(nRow, nCol, "N", true);
            	//grdPrdcrCrclOgnReqClsMng.setCellData(nRow, 5, gv_apcCd, true);
            	grdPrdcrCrclOgnReqClsMng.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdPrdcrCrclOgnReqClsMng") {
            	if(grdPrdcrCrclOgnReqClsMng.getRowStatus(nRow) == 0 || grdPrdcrCrclOgnReqClsMng.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var PrdcrCrclOgnReqClsMngVO = grdPrdcrCrclOgnReqClsMng.getRowData(nRow);
            			fn_deleteRsrc(PrdcrCrclOgnReqClsMngVO);
            			grdPrdcrCrclOgnReqClsMng.deleteRow(nRow);
            		}
            	}else{
            		grdPrdcrCrclOgnReqClsMng.deleteRow(nRow);
            	}
    ...
    function gridClick(){
        ...
        selGridRow = grdPrdcrCrclOgnReqClsMng.getRow();
        selGridCol = grdPrdcrCrclOgnReqClsMng.getCol();


        let delYnCol = grdPrdcrCrclOgnReqClsMng.getColRef('delYn');
        let delYnValue = grdPrdcrCrclOgnReqClsMng.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdPrdcrCrclOgnReqClsMng.getColRef('cc');
        let ddCol = grdPrdcrCrclOgnReqClsMng.getColRef('dd');
        let iiCol = grdPrdcrCrclOgnReqClsMng.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdPrdcrCrclOgnReqClsMng.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdPrdcrCrclOgnReqClsMng.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdPrdcrCrclOgnReqClsMng.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdPrdcrCrclOgnReqClsMng.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);

			let grdStatus = grdPrdcrCrclOgnReqClsMng.getRowStatus(selGridRow);
		 	if(grdStatus != '1'){
		 		grdPrdcrCrclOgnReqClsMng.setRowStatus(selGridRow,'update');
		 	}
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdPrdcrCrclOgnReqClsMng.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdPrdcrCrclOgnReqClsMng.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdPrdcrCrclOgnReqClsMng.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdPrdcrCrclOgnReqClsMng.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdPrdcrCrclOgnReqClsMng.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
