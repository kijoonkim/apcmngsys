<%
  /**
   * @Class Name : isoMajorItem2026.jsp
   * @Description : 출자출하조직 관리 전문품목 매입·매출 2026
   * @author SI개발부
   * @since 2025.11.13
   * @version 1.0
...
<head>
  ...
  <title>title : 출자출하조직 관리 전문품목 매입·매출 2026</title>
...
<section>
  <div class="box box-solid">
    ...
    <div class="box-body">
      ...
      <div id="pruoPrgrsSttsArea" style="border:1px solid #red; background-color: #ffc0cb; border-radius: 10px; padding: 10px; display: none;  max-height: 200px; overflow: auto;">
      ...
      <div class="ad_section_top">
        ...
        <div style="border:1px solid #f4f4f4; background-color: #f4f4f4; border-radius: 10px; padding: 10px;">
          <p>o (생산자조직)전속출하 : 생산자조직 요건을 충족한 적합(Y) 생산자조직에 대한 매출현황 입력(매입현황은 "생산자조직 현황"에 입력한 요건충족 생산자조직의 출하량, 출하대금지급액이 자동 연계)</p>
...
<script type="text/javascript">
  ...
  */
  function fn_totSum(objGrid, nRow) {
    ...
    let sumMap = {
      'Y': {
        '0': { slsCnsgnPrchsAmt: 0, uoSpmtAmt: 0, uoOtherSpmtAmt: 0, slsCnsgnPrchsVlm: 0, uoSpmtVlm: 0, uoOtherSpmtVlm: 0 },
        '1': { slsCnsgnPrchsAmt: 0, uoSpmtAmt: 0, uoOtherSpmtAmt: 0, slsCnsgnPrchsVlm: 0, uoSpmtVlm: 0, uoOtherSpmtVlm: 0 },
        '2': { slsCnsgnPrchsAmt: 0, uoSpmtAmt: 0, uoOtherSpmtAmt: 0, slsCnsgnPrchsVlm: 0, uoSpmtVlm: 0, uoOtherSpmtVlm: 0 },
        '3': { slsCnsgnPrchsAmt: 0, uoSpmtAmt: 0, uoOtherSpmtAmt: 0, slsCnsgnPrchsVlm: 0, uoSpmtVlm: 0, uoOtherSpmtVlm: 0 },
      },
      'N': {
        '0': { slsCnsgnPrchsAmt: 0, uoSpmtAmt: 0, uoOtherSpmtAmt: 0, slsCnsgnPrchsVlm: 0, uoSpmtVlm: 0, uoOtherSpmtVlm: 0 },
        '1': { slsCnsgnPrchsAmt: 0, uoSpmtAmt: 0, uoOtherSpmtAmt: 0, slsCnsgnPrchsVlm: 0, uoSpmtVlm: 0, uoOtherSpmtVlm: 0 },
    ...
    for (let i = captionRow; i < gridData01.length + captionRow; i++) {
      ...
      if (rowData01.typeSeNo === '5' && rowData01.itemCd === itemCdVal) {
        ...
        if (sumMap[ognzStbltYn] && sumMap[ognzStbltYn][trmtType]) {
          ...
          sumMap[ognzStbltYn][totSet].uoSpmtAmt			+= Number(rowData01.uoSpmtAmt || 0);
          sumMap[ognzStbltYn][totSet].uoOtherSpmtAmt		+= Number(rowData01.uoOtherSpmtAmt || 0);
          //취급유형별 합계
          sumMap[ognzStbltYn][trmtType].slsCnsgnPrchsAmt 	+= Number(rowData01.slsCnsgnPrchsAmt || 0);
          sumMap[ognzStbltYn][trmtType].uoSpmtAmt 		+= Number(rowData01.uoSpmtAmt || 0);
          sumMap[ognzStbltYn][trmtType].uoOtherSpmtAmt 	+= Number(rowData01.uoOtherSpmtAmt || 0);

          //물량
          //전속출하 합계
          sumMap[ognzStbltYn][totSet].slsCnsgnPrchsVlm	+= Number(rowData01.slsCnsgnPrchsVlm || 0);
          sumMap[ognzStbltYn][totSet].uoSpmtVlm			+= Number(rowData01.uoSpmtVlm || 0);
          sumMap[ognzStbltYn][totSet].uoOtherSpmtVlm		+= Number(rowData01.uoOtherSpmtVlm || 0);
          //취급유형별 합계
          sumMap[ognzStbltYn][trmtType].slsCnsgnPrchsVlm 	+= Number(rowData01.slsCnsgnPrchsVlm || 0);
          sumMap[ognzStbltYn][trmtType].uoSpmtVlm			+= Number(rowData01.uoSpmtVlm || 0);
          sumMap[ognzStbltYn][trmtType].uoOtherSpmtVlm 	+= Number(rowData01.uoOtherSpmtVlm || 0);
        }
      }
    ...
    for (let i = captionRow; i < gridData01.length + captionRow; i++) {
      ...
      if (rowData01.itemCd === itemCdVal) {
        ...
        if (rowData01.typeSeNo === '5' || rowData01.typeSeNo === '7') {
          ...
          totalSum.uoSpmtAmt 			+= Number(rowData01.uoSpmtAmt || 0);
          totalSum.uoOtherSpmtAmt 	+= Number(rowData01.uoOtherSpmtAmt || 0);
          totalSum.SpmtAmtTot 		+= Number(rowData01.SpmtAmtTot || 0);

          totalSum.slsCnsgnPrchsVlm 	+= Number(rowData01.slsCnsgnPrchsVlm || 0);
          totalSum.uoSpmtVlm 			+= Number(rowData01.uoSpmtVlm || 0);
          totalSum.uoOtherSpmtVlm 	+= Number(rowData01.uoOtherSpmtVlm || 0);
          totalSum.SpmtVlmTot 		+= Number(rowData01.SpmtVlmTot || 0);
        }
  ...
  const fn_dtlSearch = async function(){
    ...
    try {
      ...
      //const postJsonPromise = gfn_postJSON("/pd/aom/selectPrdcrCrclOgnReqMngList.do", param);
      const postJsonPromise = gfn_postJSON("/pd/aom/selectUoIsoOgnzList.do", param);
      const data = await postJsonPromise;

      console.log("출자로 들어와서 조회한결과", data.resultList);
  ...
  async function fn_clearForm() {
    jsonPrdcrOgnCurntMng01.length= 0; // 판매위임[매입] 및 출하[매출] 실적
    grdPrdcrOgnCurntMng01.rebuild();
    SBUxMethod.set('dtl-input-uoBrno' , null);
    SBUxMethod.set('dtl-input-selUoBrno' , null);
    //임시저장 표기 및 진행상태 표시
    // $("#pruoPrgrsSttsArea").hide();
    document.getElementById('pruoPrgrsSttsArea').style.display = 'none';
    $("#tmprStrgRsn").text("");
  ...
  const fn_dtlGridSearch = async function(){
    ...
    const pruoPrgrsStts = await gfn_chkPruoPrgrsStts(yr, apoCd, "PD_005_002");

    fn_pruoPrgrsStts(pruoPrgrsStts);
      ...
      switch (typeSeNo) {
        case '5': // 생산자조직
          ...
          if (gfn_isEmpty(rowData01.prchsNm)) {
            objGrid.setCellDisabled(i, columnRefs.uoSpmtVlm, i, columnRefs.uoOtherSpmtAmt, true);
          }
        ...
        case '6': // 생산자조직 소계
          objGrid.setCellDisabled(i, columnRefs.slsCnsgnPrchsVlm, i, columnRefs.uoOtherSpmtAmt, true);
  ...
  const fn_listSave = async function(_tmprStrgYn){
    ...
    // let postJsonPromise = gfn_postJSON("/pd/isom/multiSaveInvShipOgnSpeczItmPurSalMngListNew.do", saveList);
    const postJsonPromise = gfn_postJSON("/pd/aom/insertIsoNtslDlgtSlsPrfmnc.do", saveList);
    let data = await postJsonPromise;
  ...
  //통합조직 콤보박스 선택시 값 변경
  async function fn_changeSelUoBrno() {
  ...
  const fn_hiddenGrdSelect = async function(){
    ...
    try {
      const param = {yr : yr};
      const postJsonPromise = gfn_postJSON("/pd/isom/selectInvShipOgnSpeczItmPurSalMngRawDataList2025.do", param);
      //const postJsonPromise = gfn_postJSON("/pd/isom/selectIsoMajorItemPurchaseRawDataList.do", param);
  ...
  function fn_excelDown(yr){
    ...
    //let fileName = formattedDate + "_전문품목매입매출_출자출하조직_로우데이터" + yr + "년";
    let fileName = gfn_dateToYmd(currentDate) + "_전문품목매입매출_출자출하조직_로우데이터" + yr + "년";
  ...
  const fn_chkPruoPrgrsStts = async function () {
    const apoCd = SBUxMethod.get('dtl-input-apoCd');
    ...
    // PD_005_002
    const postJsonPromise = gfn_postJSON("/pd/aom/selectPruoPrgrsApoDtl.do", {
      crtrYr: crtrYr,
      apoCd: apoCd,
      prgrmId : "PD_005_002"
    });
    ...
    try {
      if (_.isEqual("S", data.resultStatus)) {
        ...
        document.getElementById('pruoPrgrsSttsArea').style.display = 'block';
        if (resultVO == null) {
          SBUxMethod.set('dtl-inp-pruoPrgrsStts',"미작성 상태입니다.");
        } else {
          const cmptnYn = resultVO.cmptnYn;
          const tmprStrgYn = resultVO.tmprStrgYn;
          if (_.isEqual(cmptnYn, "N") && _.isEqual(tmprStrgYn, "Y")) {
            SBUxMethod.set('dtl-inp-pruoPrgrsStts',"임시저장 상태입니다.");
          } else if (_.isEqual(cmptnYn, "Y") && _.isEqual(tmprStrgYn, "N")) {
            SBUxMethod.set('dtl-inp-pruoPrgrsStts',"작성완료 상태입니다.");
            $("#tmprStrgRsn").text("");
          } else {
            document.getElementById('pruoPrgrsSttsArea').style.display = 'none';
  ...
  function fn_pruoPrgrsStts(pruoPrgrsStts) {
