<%
  /**
   * @Class Name : isoTotal2026.jsp
   * @Description : 출자출하조직 관리 총 매입·매출 2026
   * @author SI개발부
   * @since 2026.11.13
   * @version 1.0
...
<head>
  ...
  <title>title : 출하조직 관리 총 매입·매출 2026</title>
    ...
    <div class="box-body">
      ...
      <div class="ad_section_top">
        <div style="border:1px solid #f4f4f4; background-color: #f4f4f4; border-radius: 10px; padding: 10px;">
          ...
          <p>　* 요건 충족, 요건 미달된 생산자조직 실적 모두 불려옴</p>
          <p> - 입력필요 값 : (전문품목 매입매출 화면에서의) <ins><b>사입한 물량에 대해, 단순수탁 및 단순매취로 구분작성</b></ins>하여, 총매입물량/매입금액(합계)과 맞춰야 함("합계" 메뉴에서 보이는 "차이" 값이 0이 되는지 확인)</p>
          <br>
          <p> 2) 기타품목</p>
          <p> - 불려오는 값이 별도 없으므로, 취급유형에 맞춰서 물량/금액 입력</p>
          <p>　* 사업신청 시 등록한 품목이 자동 세팅 됨</p>
          <br>
          <p>  ⇒ 전문/육성/기타품목에 대해 입력하여 조직의 총매입액과 일치여부 확인 후 "매입저장"</p>
        </div>
        ...
        <div id="pruoPrgrsSttsPrchArea" style="border:1px solid #red; background-color: #ffc0cb; border-radius: 10px; padding: 10px; display: none;  max-height: 200px; overflow: auto;">
          <sbux-label id="dtl-inp-prchTotal" name="dtl-inp-prchTotal" uitype="normal" style="font-weight: bold"></sbux-label>
        </div>
        ...
        <div class="ad_tbl_top">
          <ul class="ad_tbl_count">
            <li>
              ...
              <span class="spn-crtr-yr" style="font-size:12px"></span>
            </li>
          </ul>
          <div style="margin-left: auto;">
      ...
      <div class="ad_section_top">
        <div style="border:1px solid #f4f4f4; background-color: #f4f4f4; border-radius: 10px; padding: 10px;">
          ...
          <br>
          <p> 1) 전문/육성품목</p>
          <p> - 조정취급실적 : 전문품목 매입매출 화면에서 입력한 매출실적이 불려옴(불려온 값)</p>
          <p> - 총취급실적 : 출자출하조직 내부자료(경제통합시스템 등)로 품목별 증빙 가능한 “금액”, “물량” (자동산출)</p>
          <p> - 취급액 공제실적 : 전문품목 매입매출 화면에서 공제실적으로 매출액 작성한 조직 필수입력(입력값)</p>
          <p>　* 자체수출실적/자체공판장·판매장실적/군납/학교급식 중 공제 필요한 항목에 대해 “물량”, “금액” 입력</p>
          <p>　* (불려온 값) 조정취급실적 + (입력값) 취급액 공제 실적 = (자동산출값) 총취급실적</p>
          <br>
          <p> 2) 기타품목</p>
          <p> - 불려오는 값이 별도 없으므로, “총취급실적＂에 물량/금액 입력</p>
          <br>
          <p>  ⇒ 전문/육성/기타품목에 대해 입력하여 조직의 총매출액과 일치여부 확인 후 ＂매출저장"  </p>
        </div>
        <div class="box-header" style="display:flex; justify-content: flex-start;" >
        ...
        <div id="pruoPrgrsSttsSlsArea" style="border:1px solid #red; background-color: #ffc0cb; border-radius: 10px; padding: 10px; display: none;  max-height: 200px; overflow: auto;">
          <sbux-label id="dtl-inp-slsTotal" name="dtl-inp-slsTotal" uitype="normal" style="font-weight: bold"></sbux-label>
        </div>
...
<script type="text/javascript">
  ...
  const fn_init = async function() {
    ...
    $("#grid3").hide(); // 출자출하조직 총 매출현황 - 통합조직 출하실적
    </c:if>
  ...
  const fn_initSBSelect = async function() {
    ...
    let rst = await Promise.all([
      ...
      gfn_setComCdSBSelect('grdPrdcrOgnCurntMng', 	jsonGrdComCtpv, 	'UNTY_CTPV'), //시도
      //gfn_setComCdSBSelect('grdPrdcrOgnCurntMng', 	jsonGrdComSgg, 		'CMPTN_INST_SIGUN'),//시군
      gfn_setComCdSBSelect('grdPrdcrOgnCurntMng', 	jsonGrdComSgg, 		'UNTY_SGG'),//시군
  ...
  const fn_setGrdFcltList = async function(pageSize, pageNo){
    ...
    param.uoBrno = SBUxMethod.get("srch-input-uoBrno");
    ...
    <c:if test="${loginVO.apoSe eq '1'}">
    param.brno = '${loginVO.brno}';
    if (gfn_isEmpty(param.brno)) return;
    param.userType = "21";
    param.stbltYnBrno = '${loginVO.brno}';
    </c:if>
  ...
  // 출자조직인 경우 fn_init때 조직정보를 조회
  const fn_dtlSearch = async function(){

    let brno = '${loginVO.brno}';
    let yr = SBUxMethod.get('dtl-input-yr');
    const apoSe = '${loginVO.apoSe}';
    if (gfn_isEmpty(brno)) return;
  ...
  const fn_listSave01 = async function(_tmprStrgYn){
    ...
    const postJsonPromise = gfn_postJSON("/pd/aom/insertIsoTotPrchPrfmnc.do", saveList);
    let data = await postJsonPromise;
    try {
  ...
  const fn_listSave02 = async function(_tmprStrgYn){
    ...
    const postJsonPromise = gfn_postJSON("/pd/aom/insertIsoTotSlsPrfmnc.do", saveList);
    let data = await postJsonPromise;
    try {
      if (_.isEqual("S", data.resultStatus)) {
        gfn_comAlert("I0001") 			// I0001 	처리 되었습니다.
  ...
  const fn_view = async function(){
    ...
    SBUxMethod.set('dtl-input-yr',gfn_nvl(rowData.yr));			// 기준연도
  ...
  async function fn_dtlGridSearch() {
    ...
    const pruoPrgrsSttsMaster = await gfn_chkPruoPrgrsStts(yr, apoCd, "PD_005_001");
    fn_pruoPrgrsStts(pruoPrgrsSttsMaster, "pruoPrgrsSttsMasterArea", "dtl-inp-pruoPrgrsStts");

    const pruoPrgrsSttsPrch = await gfn_chkPruoPrgrsStts(yr, apoCd, "PD_005_001_01");
    fn_pruoPrgrsStts(pruoPrgrsSttsPrch, "pruoPrgrsSttsPrchArea", "dtl-inp-prchTotal");

    const pruoPrgrsSttsSls = await gfn_chkPruoPrgrsStts(yr, apoCd, "PD_005_001_02");
    fn_pruoPrgrsStts(pruoPrgrsSttsSls, "pruoPrgrsSttsSlsArea", "dtl-inp-slsTotal");
  ...
  /**
   * @name fn_hiddenGrdSelect
   * @description rawdata 다운로드
   * @function
   */
  ...
  const fn_openMaodal = function() {
    ...
    var url = "/pd/hisPopup/IsoPurSalHisPopup.do"
    var title = "제출실적 보기";
  ...
  function fn_pruoPrgrsStts(pruoPrgrsStts, areaId, labelId) {
    // pruoPrgrsSttsMasterArea dtl-inp-pruoPrgrsStts
    // pruoPrgrsSttsPrchArea dtl-inp-prchTotal
    // pruoPrgrsSttsSlsArea dtl-inp-slsTotal
  ...
  function fn_clearPruoPrgrsStts() {
    ...
    SBUxMethod.set('dtl-inp-prchTotal',"");
    SBUxMethod.set('dtl-inp-slsTotal',"");
  }
</script>
</html>
