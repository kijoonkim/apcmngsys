<%
  /**
   * @Class Name : uoTotal2026.jsp
   * @Description : 통합조직관리 총 매입 매출 2026
   * @author SI개발부
   * @since 2025.11.15
   * @version 1.0
...
<head>
  ...
  <title>title : 통합조직관리 총 매입 매출 2026</title>
  ...
  <div class="box box-solid">
    <div class="box-header" style="display:flex; justify-content: flex-start; align-items: center;" >
      ...
      <div style="margin-left: auto;">
        ...
        <sbux-button id="btnSavePruoPrgrs" name="btnSavePruoPrgrs" uitype="normal" text="진행상태 저장" class="btn btn-sm btn-outline-danger" onclick="fn_savePruoPrgrs"></sbux-button>
      </div>
    </div>
    <div class="box-body">
      ...
      <%-- 진행상태 표시 --%>
      <div id="pruoPrgrsSttsMasterArea" style="border:1px solid #red; background-color: #ffc0cb; border-radius: 10px; padding: 10px; display: none; max-height: 200px; overflow: auto;">
      ...
      <div class="ad_section_top">
        ...
        <div id="pruoPrgrsSttsPrchArea" style="border:1px solid #red; background-color: #ffc0cb; border-radius: 10px; padding: 10px; display: none;  max-height: 200px; overflow: auto;">
          <sbux-label id="dtl-inp-pruoPrgrsSttsPrch" name="dtl-inp-pruoPrgrsSttsPrch" uitype="normal" style="font-weight: bold"></sbux-label>
        </div>
      ...
      <div class="ad_section_top">
        <div class="box-header" style="display:flex; justify-content: flex-start;" >
          <div style="margin-left: auto;">
            ...
            <c:if test="${loginVO.userType ne '02' && loginVO.userType ne '91'}">
              <sbux-button id="btnTempSave3" name="btnTempSave3" uitype="normal" text="매출 임시저장" class="btn btn-sm btn-outline-danger" onclick="fn_listSave02('Y')"></sbux-button>
              <sbux-button id="btnSaveFclt3" name="btnSaveFclt3" uitype="normal" text="매출 저장" class="btn btn-sm btn-outline-danger" onclick="fn_listSave02"></sbux-button>
            </c:if>
        ...
        <%-- 상태 표시 --%>
        <div id="pruoPrgrsSttsSlsArea" style="border:1px solid #red; background-color: #ffc0cb; border-radius: 10px; padding: 10px; display: none;  max-height: 200px; overflow: auto;">
          <sbux-label id="dtl-inp-pruoPrgrsSttsSls" name="dtl-inp-pruoPrgrsSttsSls" uitype="normal" style="font-weight: bold"></sbux-label>
        </div>
      ...
      <div class="ad_section_top">
        <div class="box-header" style="display:flex; justify-content: flex-start;" >
          <div style="margin-left: auto;">
            ...
            <c:if test="${loginVO.userType ne '02' && loginVO.userType ne '91'}">
              <sbux-button id="btnTempSave4" name="btnTempSave4" uitype="normal" text="출하실적 임시저장" class="btn btn-sm btn-outline-danger" onclick="fn_listSave03('Y')"></sbux-button>
        ...
        <%-- 상태 표시 --%>
        <div id="pruoPrgrsSttsSpmtPrfmncArea" style="border:1px solid #red; background-color: #ffc0cb; border-radius: 10px; padding: 10px; display: none;  max-height: 200px; overflow: auto;">
          <sbux-label id="dtl-inp-pruoPrgrsSttsSpmtPrfmnc" name="dtl-inp-pruoPrgrsSttsSpmtPrfmnc" uitype="normal" style="font-weight: bold"></sbux-label>
        </div>
...
<script type="text/javascript">
  ...
  function fn_prchsVlmDiff(objGrid, nRow, nCol){
    ...
    let sumVal = 0;
    if (objGrid.getRowStatus(Number(nRow)) < 2) {
      sumVal = Number(gfn_nvl(rowData.prchsTotVlm)) - Number(gfn_nvl(rowData.prchsTrstVlm)) - Number(gfn_nvl(rowData.prchsEmspapVlm));
    } else {
  ...
  function fn_prchsAmtDiff(objGrid, nRow, nCol){
    ...
    if (objGrid.getRowStatus(Number(nRow)) < 2) {
      sumVal = Number(gfn_nvl(rowData.prchsTotAmt)) - Number(gfn_nvl(rowData.prchsTrstAmt)) - Number(gfn_nvl(rowData.prchsEmspapAmt));
    } else {
      //금액의 경우 기타인 경우만 합산 처리
      if (rowData.sttgUpbrItemSe != '3' && rowData.delYn == 'N'){
  ...
  const fn_fcltMngCreateGrid02 = async function() {
    ...
    SBGridProperties.columns = [
      ...
      /*수탁*/
      {caption: ["수탁","공동선별수탁","물량(톤)"], 	ref: 'slsCprtnSortTrstVlm',   	type:'input',  width:'50px',    style:'text-align:right'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["수탁","공동선별수탁","금액(천원)"], 	ref: 'slsCprtnSortTrstAmt',   	type:'input',  width:'80px',    style:'text-align:right'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["수탁","공동출하수탁","물량(톤)"], 	ref: 'slsCprtnTrstVlm',   	type:'input',  width:'50px',    style:'text-align:right'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["수탁","공동출하수탁","금액(천원)"], 	ref: 'slsCprtnTrstAmt',   	type:'input',  width:'80px',    style:'text-align:right'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["수탁","단순수탁","물량(톤)"], 		ref: 'slsSmplTrstVlm',   	type:'input',  width:'50px',    style:'text-align:right'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["수탁","단순수탁","금액(천원)"], 	ref: 'slsSmplTrstAmt',   	type:'input',  width:'80px',    style:'text-align:right'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["수탁","수탁 소계","물량(톤)"], 		ref: 'slsTrstVlm',   	type:'input',  width:'50px',    style:'text-align:right; background-color: lightgray'
        , calc : 'fn_slsTrstVlm'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["수탁","수탁 소계","금액(천원)"], 	ref: 'slsTrstAmt',   	type:'input',  width:'80px',    style:'text-align:right; background-color: lightgray; border-right-color: black !important;'
        , calc : 'fn_slsTrstAmt'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      /*매취*/
      {caption: ["매취","공동선별매취","물량(톤)"], 	ref: 'slsCprtnSortEmspapVlm',   	type:'input',  width:'50px',    style:'text-align:right'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["매취","공동선별매취","금액(천원)"], 	ref: 'slsCprtnSortEmspapAmt',   	type:'input',  width:'80px',    style:'text-align:right'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["매취","단순매취","물량(톤)"], 		ref: 'slsSmplEmspapVlm',   	type:'input',  width:'50px',    style:'text-align:right'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["매취","단순매취","금액(천원)"], 	ref: 'slsSmplEmspapAmt',   	type:'input',  width:'80px',    style:'text-align:right'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["매취","매취 소계","물량(톤)"], 		ref: 'slsEmspapVlm',   	type:'input',  width:'50px',    style:'text-align:right; background-color: lightgray'
        , calc : 'fn_slsEmspapVlm'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["매취","매취 소계","금액(천원)"], 	ref: 'slsEmspapAmt',   	type:'input',  width:'80px',    style:'text-align:right; background-color: lightgray; border-right-color: black !important;'
      ...
      {caption: ["합계","합계","물량(톤)"], 	ref: 'slsTotVlm',   	type:'output',  width:'50px',    style:'text-align:right; background-color: lightgray'
        , calc : 'fn_slsVlmSum'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["합계","합계","차이"], 	ref: 'slsTotVlmDiff',   	type:'output',  width:'50px',    style:'text-align:right; background-color: lightgray'
        , calc : 'fn_slsVlmDiff'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["합계","합계","금액(천원)"], 	ref: 'slsTotAmt',   	type:'output',  width:'80px',    style:'text-align:right; background-color: lightgray'
        , calc : 'fn_slsAmtSum'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["합계","합계","차이"], 	ref: 'slsTotAmtDiff',   	type:'output',  width:'80px',    style:'text-align:right; background-color: lightgray; border-right-color: black !important;'
        , calc : 'fn_slsAmtDiff'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      /*자체공판장 매출*/
      {caption: ["자체공판장 매출액","자체공판장 매출액","물량(톤)"], 	ref: 'ddcVlm',   	type:'input',  width:'50px',    style:'text-align:right'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["자체공판장 매출액","자체공판장 매출액","금액(천원)"], 	ref: 'ddcAmt',   	type:'input',  width:'80px',    style:'text-align:right'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : false}}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
  ...
  function fn_slsVlmDiff(objGrid, nRow, nCol){
    ...
    let sumVal = 0;
    if (objGrid.getRowStatus(Number(nRow)) < 2) {
      sumVal = Number(gfn_nvl(rowData.slsTotVlm)) - Number(gfn_nvl(rowData.slsTrstVlm)) - Number(gfn_nvl(rowData.slsEmspapVlm));
    } else {
      //금액의 경우 기타인 경우만 합산 처리
      if (rowData.sttgUpbrItemSe != '3' && rowData.delYn == 'N') {
  ...
  function fn_slsAmtDiff(objGrid, nRow, nCol){
    ...
    let sumVal = 0;
    if (objGrid.getRowStatus(Number(nRow)) < 2) {
      sumVal = Number(gfn_nvl(rowData.slsTotAmt)) - Number(gfn_nvl(rowData.slsTrstAmt)) - Number(gfn_nvl(rowData.slsEmspapAmt));
    } else {
      //금액의 경우 기타인 경우만 합산 처리
      if (rowData.sttgUpbrItemSe != '3' && rowData.delYn == 'N') {
  ...
  const fn_fcltMngCreateGrid03 = async function() {
    ...
    SBGridProperties.columns = [
      ...
      {caption: ["공영\n도매시장","물량(톤)"], 		ref: 'pblcWhlslMrktVlm',   	type:'input',  width:'50px',    style:'text-align:right' 				,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["공영\n도매시장","금액(천원)"], 		ref: 'pblcWhlslMrktAmt',   	type:'input',  width:'80px',    style:'text-align:right' 				,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      ...
      {caption: ["대형\n유통업체","물량(톤)"], 		ref: 'lgszRtlVlm',   	type:'input',  width:'50px',    style:'text-align:right' 					,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["대형\n유통업체","금액(천원)"], 		ref: 'lgszRtlAmt',   	type:'input',  width:'80px',    style:'text-align:right' 					,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["군납","물량(톤)"], 		ref: 'armyDlvgdsVlm',   	type:'input',  width:'50px',    style:'text-align:right' 						,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["군납","금액(천원)"], 		ref: 'armyDlvgdsAmt',   	type:'input',  width:'80px',    style:'text-align:right' 					,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["외식, 식자재\n및 단체급식","물량(톤)"], 		ref: 'eatoutMtrlMlsrVlm',   	type:'input',  width:'50px',    style:'text-align:right' 	,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["외식, 식자재\n및 단체급식","금액(천원)"], 		ref: 'eatoutMtrlMlsrAmt',   	type:'input',  width:'80px',    style:'text-align:right' 	,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["가공업체","물량(톤)"], 		ref: 'mnfcRtlVlm',   	type:'input',  width:'50px',    style:'text-align:right' 						,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["가공업체","금액(천원)"], 		ref: 'mnfcRtlAmt',   	type:'input',  width:'80px',    style:'text-align:right' 						,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["수출","물량(톤)"], 		ref: 'exprtVlm',   	type:'input',  width:'50px',    style:'text-align:right' 								,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["수출","금액(천원)"], 		ref: 'exprtAmt',   	type:'input',  width:'80px',    style:'text-align:right' 							,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["온라인 거래실적\n(e커머스, TV홈쇼핑 등 포함)","물량(톤)"], 		ref: 'onlnDlngPrfmncVlm',   	type:'input',  width:'50px',    style:'text-align:right' 		,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["온라인 거래실적\n(e커머스, TV홈쇼핑 등 포함)","금액(천원)"], 		ref: 'onlnDlngPrfmncAmt',   	type:'input',  width:'80px',    style:'text-align:right; border-right-color: black !important;' 	,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      //기타값은 합계 - 기타를 제외 한 값의 합
      //구분 기타의 경우는 계산하지 않음
      {caption: ["기타","물량(톤)"], 		ref: 'etcVlm',   	type:'input',  width:'50px',    style:'text-align:right'
        , calc : 'fn_etcVlm'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
      {caption: ["기타","금액(천원)"], 		ref: 'etcAmt',   	type:'input',  width:'80px',    style:'text-align:right'
        , calc : 'fn_etcAmt'
        ,typeinfo : {mask : {alias : 'numeric', unmaskvalue : true}, maxlength : 10}, format : {type:'number', rule:'#,###'}, datatype : 'number'},
  ...
  const fn_yrChkVal = function() {
    const yrChk = SBUxMethod.get("srch-input-yrChk");//
    const keys = Object.getOwnPropertyNames(yrChk);
    let yrChkVal = null;
    for ( let i=0; i<keys.length; i++) {
      if (yrChk[keys[i]]){
        yrChkVal = yrChk[keys[i]];
  ...
  const fn_setGrdFcltList = async function(pageSize, pageNo){
    ...
    param.brno = SBUxMethod.get("srch-input-brno");
    param.cmptnInst = SBUxMethod.get("srch-input-cmptnInst");
    param.ctpv = SBUxMethod.get("srch-input-ctpv");
    param.corpSeCd = SBUxMethod.get("srch-input-corpSeCd");
    param.corpDtlSeCd = SBUxMethod.get("srch-input-corpDtlSeCd");
    param.corpNm = SBUxMethod.get("srch-input-corpNm");
    param.aprv = SBUxMethod.get("srch-input-aprv");
    param.yrChk = fn_yrChkVal();
    param.stbltHldYn = SBUxMethod.get("srch-input-stbltHldYn");//
    ...
    try {
      const postJsonPromise = gfn_postJSON("/pd/aom/selectUoIsoOgnzList.do", param);
  ...
  const fn_listSave01 = async function(_tmprStrgYn){
    ...
    const postJsonPromise = gfn_postJSON("/pd/aom/insertPruoTotalPrch.do", saveList);
    let data = await postJsonPromise;
    try {
  ...
  const fn_listSave02 = async function(_tmprStrgYn){
    ...
    const postJsonPromise = gfn_postJSON("/pd/aom/insertPruoTotalSls.do", saveList);
    let data = await postJsonPromise;
    try {
      if (_.isEqual("S", data.resultStatus)) {
        gfn_comAlert("I0001") 			// I0001 	처리 되었습니다.
  ...
  /** 통합조직 총 매입매출 출하실적 저장 **/
  const fn_listSave03 = async function(_tmprStrgYn){
    ...
    for(var i = captionRow; i < gridData03.length + captionRow; i++ ){
      ...
      if(delYn == 'N'){
        ...
        if(_tmprStrgYn == 'Y'){
          rowData03.tmprStrgYn = _tmprStrgYn;
        }
    ...
    const postJsonPromise = gfn_postJSON("/pd/aom/insertPruoTotalSpmtPrfmnc.do", saveList);
    let data = await postJsonPromise;
    try {
      if (_.isEqual("S", data.resultStatus)) {
        gfn_comAlert("I0001") 			// I0001 	처리 되었습니다.
  ...
  */
  ...
  async function fn_dtlGridSearch() {
    ...
    const pruoPrgrsSttsMaster = await gfn_chkPruoPrgrsStts(yr, apoCd, "PD_006_001");
    fn_pruoPrgrsStts(pruoPrgrsSttsMaster, "pruoPrgrsSttsMasterArea", "dtl-inp-pruoPrgrsStts");

    const pruoPrgrsSttsTotalPrch = await gfn_chkPruoPrgrsStts(yr, apoCd, "PD_006_001_01");
    fn_pruoPrgrsStts(pruoPrgrsSttsTotalPrch, "pruoPrgrsSttsPrchArea", "dtl-inp-pruoPrgrsSttsPrch");

    const pruoPrgrsSttsTotalSls = await gfn_chkPruoPrgrsStts(yr, apoCd, "PD_006_001_02");
    fn_pruoPrgrsStts(pruoPrgrsSttsTotalSls, "pruoPrgrsSttsSlsArea", "dtl-inp-pruoPrgrsSttsSls");

    const pruoPrgrsSttsTotalSpmtPfmnc = await gfn_chkPruoPrgrsStts(yr, apoCd, "PD_006_001_03");
    fn_pruoPrgrsStts(pruoPrgrsSttsTotalSpmtPfmnc, "pruoPrgrsSttsSpmtPrfmncArea", "dtl-inp-pruoPrgrsSttsSpmtPrfmnc");
    ...
    try {
      ...
      const defaultUrl = "/pd/pcom/selectPrdcrCrclOgnPurSalMngPrchsSlsList.do";
      const upbrToAprvUrl = "/pd/pcom/selectUoTotalListForUpbrToAprv.do";
  ...
  const fn_hiddenGrd = async function() {
    ...
    SBGridProperties.columns = [
      ...
      /*수탁*/
      {caption: ["매입 공동선별수탁 물량(톤)"], 	ref: 'prchsSortTrstVlm',   	type:'input',  width:'50px',    style:'text-align:center', datatype:'number'},
      {caption: ["매입 공동선별수탁 금액(천원)"], 	ref: 'prchsSortTrstAmt',   	type:'input',  width:'80px',    style:'text-align:center', datatype:'number'},
      {caption: ["매입 공동출하수탁 물량(톤)"], 	ref: 'prchsSpmtTrstVlm',   type:'input',  width:'50px',    style:'text-align:center', datatype:'number'},
      {caption: ["매입 공동출하수탁 금액(천원)"], 	ref: 'prchsSpmtTrstAmt',   type:'input',  width:'80px',    style:'text-align:center', datatype:'number'},
      {caption: ["매입 단순수탁 물량(톤)"], 		ref: 'prchsSmplTrstVlm',   type:'input',  width:'50px',    style:'text-align:center', datatype:'number'},
      {caption: ["매입 단순수탁 금액(천원)"], 	ref: 'prchsSmplTrstAmt',   type:'input',  width:'80px',    style:'text-align:center', datatype:'number'},
      {caption: ["매입 수탁소계 물량(톤)"], 		ref: 'prchsTrstVlm',   type:'output',  width:'50px',    style:'text-align:center;', datatype:'number'},
      {caption: ["매입 수탁소계 금액(천원)"], 	ref: 'prchsTrstAmt',   type:'output',  width:'80px',    style:'text-align:center;', datatype:'number'},
      /*매취*/
      {caption: ["매입 공동선별매취 물량(톤)"], 	ref: 'prchsSortEmspapVlm',   type:'input',  width:'50px',    style:'text-align:center', datatype:'number'},
      {caption: ["매입 공동선별매취 금액(천원)"], 	ref: 'prchsSortEmspapAmt',   type:'input',  width:'80px',    style:'text-align:center', datatype:'number'},
      {caption: ["매입 단순매취 물량(톤)"], 		ref: 'prchsSmplEmspapVlm',   type:'input',  width:'50px',    style:'text-align:center', datatype:'number'},
      {caption: ["매입 단순매취 금액(천원)"], 	ref: 'prchsSmplEmspapAmt',   type:'input',  width:'80px',    style:'text-align:center', datatype:'number'},
      {caption: ["매입 매취 소계 물량(톤)"], 		ref: 'prchsEmspapVlm',   type:'output',  width:'50px',    style:'text-align:center;', datatype:'number'},
      {caption: ["매입 매취 소계 금액(천원)"], 	ref: 'prchsEmspapAmt',   type:'output',  width:'80px',    style:'text-align:center;', datatype:'number'},
      /*합계*/
      {caption: ["매입 합계 물량(톤)"], 		ref: 'prchsTotVlm',   		type:'output',  width:'50px',    style:'text-align:center;', datatype:'number'},
      {caption: ["매입 합계 금액(천원)"], 		ref: 'prchsTotAmt',   		type:'output',  width:'80px',    style:'text-align:center;', datatype:'number'},
      ...
      /*수탁*/
      {caption: ["매출 공동선별수탁 물량(톤)"], 	ref: 'slsCprtnSortTrstVlm',   	type:'input',  width:'50px',    style:'text-align:right', datatype:'number'},
      {caption: ["매출 공동선별수탁 금액(천원)"], 	ref: 'slsCprtnSortTrstAmt',   	type:'input',  width:'80px',    style:'text-align:right', datatype:'number'},
      {caption: ["매출 공동출하수탁 물량(톤)"], 	ref: 'slsCprtnTrstVlm',   	type:'input',  width:'50px',    style:'text-align:right', datatype:'number'},
      {caption: ["매출 공동출하수탁 금액(천원)"], 	ref: 'slsCprtnTrstAmt',   	type:'input',  width:'80px',    style:'text-align:right', datatype:'number'},
      {caption: ["매출 단순수탁 물량(톤)"], 		ref: 'slsSmplTrstVlm',   	type:'input',  width:'50px',    style:'text-align:right', datatype:'number'},
      {caption: ["매출 단순수탁 금액(천원)"], 	ref: 'slsSmplTrstAmt',   	type:'input',  width:'80px',    style:'text-align:right', datatype:'number'},
      {caption: ["매출 수탁 소계 물량(톤)"], 		ref: 'slsTrstVlm',   	type:'input',  width:'50px',    style:'text-align:right;', datatype:'number'},
      {caption: ["매출 수탁 소계 금액(천원)"], 	ref: 'slsTrstAmt',   	type:'input',  width:'80px',    style:'text-align:right;', datatype:'number'},
      /*매취*/
      {caption: ["매출 공동선별매취 물량(톤)"], 	ref: 'slsCprtnSortEmspapVlm',   	type:'input',  width:'50px',    style:'text-align:right', datatype:'number'},
      {caption: ["매출 공동선별매취 금액(천원)"], 	ref: 'slsCprtnSortEmspapAmt',   	type:'input',  width:'80px',    style:'text-align:right', datatype:'number'},
      {caption: ["매출 단순매취 물량(톤)"], 		ref: 'slsSmplEmspapVlm',   	type:'input',  width:'50px',    style:'text-align:right', datatype:'number'},
      {caption: ["매출 단순매취 금액(천원)"], 	ref: 'slsSmplEmspapAmt',   	type:'input',  width:'80px',    style:'text-align:right', datatype:'number'},
      {caption: ["매출 매취 소계 물량(톤)"], 		ref: 'slsEmspapVlm',   	type:'input',  width:'50px',    style:'text-align:right;', datatype:'number'},
      {caption: ["매출 매취 소계 금액(천원)"], 	ref: 'slsEmspapAmt',   	type:'input',  width:'80px',    style:'text-align:right;', datatype:'number'},
      //합계
      {caption: ["매출 합계 물량(톤)"], 	ref: 'slsTotVlm',   	type:'output',  width:'50px',    style:'text-align:right;', datatype:'number'},
      {caption: ["매출 합계 금액(천원)"], 	ref: 'slsTotAmt',   	type:'output',  width:'80px',    style:'text-align:right;', datatype:'number'},
      /*자체공판장 매출*/
      {caption: ["자체공판장 매출액 물량(톤)"], 	ref: 'ddcVlm',   	type:'input',  width:'50px',    style:'text-align:right', datatype:'number'},
      {caption: ["자체공판장 매출액 금액(천원)"], 	ref: 'ddcAmt',   	type:'input',  width:'80px',    style:'text-align:right', datatype:'number'},
      ...
      {caption: ["공영 도매시장 물량(톤)"],    ref: 'pblcWhlslMrktVlm',  type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
      {caption: ["공영 도매시장 금액(천원)"],    ref: 'pblcWhlslMrktAmt',  type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
      ...
      {caption: ["대형 유통업체 물량(톤)"],    ref: 'lgszRtlVlm',        type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
      {caption: ["대형 유통업체 금액(천원)"],    ref: 'lgszRtlAmt',        type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
      {caption: ["군납 물량(톤)"],    ref: 'armyDlvgdsVlm',     type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
      {caption: ["군납 금액(천원)"],    ref: 'armyDlvgdsAmt',     type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
      {caption: ["외식, 식자재 및 단체급식 물량(톤)"],    ref: 'eatoutMtrlMlsrVlm', type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
      {caption: ["외식, 식자재 및 단체급식 금액(천원)"],    ref: 'eatoutMtrlMlsrAmt', type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
      {caption: ["가공업체 물량(톤)"],    ref: 'mnfcRtlVlm',        type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
      {caption: ["가공업체 금액(천원)"],    ref: 'mnfcRtlAmt',        type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
      {caption: ["수출 물량(톤)"],    ref: 'exprtVlm',          type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
      {caption: ["수출 금액(천원)"],    ref: 'exprtAmt',          type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
      {caption: ["온라인 거래실적 (e커머스, TV홈쇼핑 등 포함) 물량(톤)"],    ref: 'onlnDlngPrfmncVlm', type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
      {caption: ["온라인 거래실적 (e커머스, TV홈쇼핑 등 포함) 금액(천원)"],    ref: 'onlnDlngPrfmncAmt', type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
      {caption: ["기타 물량(톤)"],    ref: 'etcVlm',            type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
      {caption: ["기타 금액(천원)"],    ref: 'etcAmt',            type:'output',  width:'70px',    style:'text-align:center', datatype:'number'},
    ];
  ...
  function fn_pruoPrgrsStts(pruoPrgrsStts, areaId, labelId) {
    // pruoPrgrsSttsPrchArea dtl-inp-pruoPrgrsSttsPrch
    // pruoPrgrsSttsSlsArea dtl-inp-pruoPrgrsSttsSls
    // pruoPrgrsSttsSpmtPrfmncArea dtl-inp-pruoPrgrsSttsSpmtPrfmnc
  ...
  function fn_clearPruoPrgrsStts() {
    ...
    document.getElementById('pruoPrgrsSttsMasterArea').style.display = 'none';
    document.getElementById('pruoPrgrsSttsPrchArea').style.display = 'none';
    document.getElementById('pruoPrgrsSttsSlsArea').style.display = 'none';
    document.getElementById('pruoPrgrsSttsSpmtPrfmncArea').style.display = 'none';

    SBUxMethod.set('dtl-inp-pruoPrgrsStts',"");
    SBUxMethod.set('dtl-inp-pruoPrgrsSttsPrch',"");
    SBUxMethod.set('dtl-inp-pruoPrgrsSttsSls',"");
    SBUxMethod.set('dtl-inp-pruoPrgrsSttsSpmtPrfmnc',"");
  }

  /** 진행상태만 저장 **/
  const fn_savePruoPrgrs = async function () {
    const crtrYr = SBUxMethod.get("dtl-input-yr");
    const apoCd = SBUxMethod.get("dtl-input-apoCd");

    if (gfn_isEmpty(apoCd)) {
      gfn_comAlert("W0001", "진행상태 저장할 조직"); // {0}을/를 선택하세요.
      return;
    }

    if (!gfn_comConfirm("Q0001", "진행상태 저장")) {	//	Q0001	{0} 하시겠습니까?
      return;
    }

    const postJsonPromise = gfn_postJSON("/pd/aom/insertPruoPrgrsApoDtl.do", {
      crtrYr : crtrYr
      , apoCd : apoCd
      , wrtPrgrmId : "PD_006_001"
    });
    const data = await postJsonPromise;
