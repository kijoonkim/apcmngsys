		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdPrdcrCrclOgnPurSalMng" style="height:350px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonPrdcrCrclOgnPurSalMng = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdPrdcrCrclOgnPurSalMng', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdPrdcrCrclOgnPurSalMng';
	    SBGridProperties.id = 'grdPrdcrCrclOgnPurSalMng';
	    SBGridProperties.jsonref = 'jsonPrdcrCrclOgnPurSalMng';
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdPrdcrCrclOgnPurSalMng\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdPrdcrCrclOgnPurSalMng\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdPrdcrCrclOgnPurSalMng\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	        ...
	        */
	        {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdPrdcrCrclOgnPurSalMng\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdPrdcrCrclOgnPurSalMng\", " + nRow + ")'>삭제</button>";
	        	}
	        }}
	    ];

	    grdPrdcrCrclOgnPurSalMng = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdPrdcrCrclOgnPurSalMng.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdPrdcrCrclOgnPurSalMng.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/pd/pcorm/selectPrdcrCrclOgnPurSalMng.do", {apcCd : apcCd});
    	let postJsonPromise = gfn_postJSON("/pd/pcorm/selectPrdcrCrclOgnPurSalMngList.do", {
    		ccCode : ccCode
    		,iiCode : iiCode
		});
        let data = await postJsonPromise;
        try{
        	jsonPrdcrCrclOgnPurSalMng.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let PrdcrCrclOgnPurSalMngVO = {
					testNo 		: item.testNo
				  , aa 		: item.aa
				  , bb 		: item.bb
				  , cc 	: item.cc
				...
				jsonPrdcrCrclOgnPurSalMng.push(PrdcrCrclOgnPurSalMngVO);
			});



        	grdPrdcrCrclOgnPurSalMng.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdPrdcrCrclOgnPurSalMng.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdPrdcrCrclOgnPurSalMng.getGridDataAll();
		let saveList = [];

		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdPrdcrCrclOgnPurSalMng.getRowData(i);
			let rowSts = grdPrdcrCrclOgnPurSalMng.getRowStatus(i);
			let bb = rowData.bb;
			let aa = rowData.aa;
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/pd/pcorm/multiSavePrdcrCrclOgnPurSalMngList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdPrdcrCrclOgnPurSalMng") {
            	grdPrdcrCrclOgnPurSalMng.setCellData(nRow, nCol, "N", true);
            	//grdPrdcrCrclOgnPurSalMng.setCellData(nRow, 5, gv_apcCd, true);
            	grdPrdcrCrclOgnPurSalMng.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdPrdcrCrclOgnPurSalMng") {
            	if(grdPrdcrCrclOgnPurSalMng.getRowStatus(nRow) == 0 || grdPrdcrCrclOgnPurSalMng.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var PrdcrCrclOgnPurSalMngVO = grdPrdcrCrclOgnPurSalMng.getRowData(nRow);
            			fn_deleteRsrc(PrdcrCrclOgnPurSalMngVO);
            			grdPrdcrCrclOgnPurSalMng.deleteRow(nRow);
            		}
            	}else{
            		grdPrdcrCrclOgnPurSalMng.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(PrdcrCrclOgnPurSalMngVO){
		let postJsonPromise = gfn_postJSON("/pd/pcorm/deletePrdcrCrclOgnPurSalMng.do", PrdcrCrclOgnPurSalMngVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		...
		//grdPrdcrCrclOgnPurSalMng 그리드 객체
        selGridRow = grdPrdcrCrclOgnPurSalMng.getRow();
        selGridCol = grdPrdcrCrclOgnPurSalMng.getCol();


        let delYnCol = grdPrdcrCrclOgnPurSalMng.getColRef('delYn');
        let delYnValue = grdPrdcrCrclOgnPurSalMng.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdPrdcrCrclOgnPurSalMng.getColRef('cc');
        let ddCol = grdPrdcrCrclOgnPurSalMng.getColRef('dd');
        let iiCol = grdPrdcrCrclOgnPurSalMng.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdPrdcrCrclOgnPurSalMng.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdPrdcrCrclOgnPurSalMng.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdPrdcrCrclOgnPurSalMng.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdPrdcrCrclOgnPurSalMng.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdPrdcrCrclOgnPurSalMng.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdPrdcrCrclOgnPurSalMng.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdPrdcrCrclOgnPurSalMng.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdPrdcrCrclOgnPurSalMng.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdPrdcrCrclOgnPurSalMng.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
