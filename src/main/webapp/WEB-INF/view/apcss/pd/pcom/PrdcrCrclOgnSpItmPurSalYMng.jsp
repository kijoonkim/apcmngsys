		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdPrdcrCrclOgnSpItmPurSalYMng" style="height:350px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonPrdcrCrclOgnSpItmPurSalYMng = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdPrdcrCrclOgnSpItmPurSalYMng', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdPrdcrCrclOgnSpItmPurSalYMng';
	    SBGridProperties.id = 'grdPrdcrCrclOgnSpItmPurSalYMng';
	    SBGridProperties.jsonref = 'jsonPrdcrCrclOgnSpItmPurSalYMng';
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdPrdcrCrclOgnSpItmPurSalYMng\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdPrdcrCrclOgnSpItmPurSalYMng\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdPrdcrCrclOgnSpItmPurSalYMng\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	        ...
	        */
	        {caption: ["처리","처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdPrdcrCrclOgnSpItmPurSalYMng\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdPrdcrCrclOgnSpItmPurSalYMng\", " + nRow + ")'>삭제</button>";
	        	}
	        }}
	    ];

	    grdPrdcrCrclOgnSpItmPurSalYMng = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdPrdcrCrclOgnSpItmPurSalYMng.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdPrdcrCrclOgnSpItmPurSalYMng.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/pd/pcom/selectPrdcrCrclOgnSpItmPurSalYMng.do", {apcCd : apcCd});
        ...
        try{
        	jsonPrdcrCrclOgnSpItmPurSalYMng.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let PrdcrCrclOgnSpItmPurSalYMngVO = {
					testNo 		: item.testNo
				  , aa 		: item.aa
				  , bb 		: item.bb
				  , cc 	: item.cc
				...
				jsonPrdcrCrclOgnSpItmPurSalYMng.push(PrdcrCrclOgnSpItmPurSalYMngVO);
			});



        	grdPrdcrCrclOgnSpItmPurSalYMng.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdPrdcrCrclOgnSpItmPurSalYMng.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdPrdcrCrclOgnSpItmPurSalYMng.getGridDataAll();
		let saveList = [];

		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdPrdcrCrclOgnSpItmPurSalYMng.getRowData(i);
			let rowSts = grdPrdcrCrclOgnSpItmPurSalYMng.getRowStatus(i);
			let bb = rowData.bb;
			let aa = rowData.aa;
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdPrdcrCrclOgnSpItmPurSalYMng") {
            	grdPrdcrCrclOgnSpItmPurSalYMng.setCellData(nRow, nCol, "N", true);
            	//grdPrdcrCrclOgnSpItmPurSalYMng.setCellData(nRow, 5, gv_apcCd, true);
            	grdPrdcrCrclOgnSpItmPurSalYMng.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdPrdcrCrclOgnSpItmPurSalYMng") {
            	if(grdPrdcrCrclOgnSpItmPurSalYMng.getRowStatus(nRow) == 0 || grdPrdcrCrclOgnSpItmPurSalYMng.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var PrdcrCrclOgnSpItmPurSalYMngVO = grdPrdcrCrclOgnSpItmPurSalYMng.getRowData(nRow);
            			fn_deleteRsrc(PrdcrCrclOgnSpItmPurSalYMngVO);
            			grdPrdcrCrclOgnSpItmPurSalYMng.deleteRow(nRow);
            		}
            	}else{
            		grdPrdcrCrclOgnSpItmPurSalYMng.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(PrdcrCrclOgnSpItmPurSalYMngVO){
		let postJsonPromise = gfn_postJSON("/pd/pcom/deletePrdcrCrclOgnSpItmPurSalYMng.do", PrdcrCrclOgnSpItmPurSalYMngVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		...
		//grdPrdcrCrclOgnSpItmPurSalYMng 그리드 객체
        selGridRow = grdPrdcrCrclOgnSpItmPurSalYMng.getRow();
        selGridCol = grdPrdcrCrclOgnSpItmPurSalYMng.getCol();


        let delYnCol = grdPrdcrCrclOgnSpItmPurSalYMng.getColRef('delYn');
        let delYnValue = grdPrdcrCrclOgnSpItmPurSalYMng.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdPrdcrCrclOgnSpItmPurSalYMng.getColRef('cc');
        let ddCol = grdPrdcrCrclOgnSpItmPurSalYMng.getColRef('dd');
        let iiCol = grdPrdcrCrclOgnSpItmPurSalYMng.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdPrdcrCrclOgnSpItmPurSalYMng.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdPrdcrCrclOgnSpItmPurSalYMng.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdPrdcrCrclOgnSpItmPurSalYMng.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdPrdcrCrclOgnSpItmPurSalYMng.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdPrdcrCrclOgnSpItmPurSalYMng.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdPrdcrCrclOgnSpItmPurSalYMng.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdPrdcrCrclOgnSpItmPurSalYMng.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdPrdcrCrclOgnSpItmPurSalYMng.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdPrdcrCrclOgnSpItmPurSalYMng.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
