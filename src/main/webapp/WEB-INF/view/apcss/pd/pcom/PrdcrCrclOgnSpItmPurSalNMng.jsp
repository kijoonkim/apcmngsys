		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdPrdcrCrclOgnSpItmPurSalNMng" style="height:350px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonPrdcrCrclOgnSpItmPurSalNMng = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdPrdcrCrclOgnSpItmPurSalNMng', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdPrdcrCrclOgnSpItmPurSalNMng';
	    SBGridProperties.id = 'grdPrdcrCrclOgnSpItmPurSalNMng';
	    SBGridProperties.jsonref = 'jsonPrdcrCrclOgnSpItmPurSalNMng';
...
// 	        {caption: ["사업자등록번호"], 	ref: 'bb',   type:'input',  width:'250px',    style:'text-align:center', validate : gfn_chkByte.bind({byteLimit: 100})},
	        {caption: ["판매위임(매입) 금액(천원)","매입처"], 	ref: 'ii',   	type:'output',  width:'200px',    style:'text-align:center'},
	        {caption: ["판매위임(매입) 금액(천원)","금액"], 	ref: 'iiCode',   	type:'input',  width:'200px',    style:'text-align:center' ,  editable : false},
	        {caption: ["판매(매출)금액(천원)","판매(매출)금액(천원)"], 		ref: 'cc',   type:'output',  width:'200px',    style:'text-align:center', validate : gfn_chkByte.bind({byteLimit: 1000})},
            {caption: ["비고","비고"], 	ref: 'ccCode',   	type:'input', width:'80px', style:'text-align:center' ,  editable : false},
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdPrdcrCrclOgnSpItmPurSalNMng\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdPrdcrCrclOgnSpItmPurSalNMng\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdPrdcrCrclOgnSpItmPurSalNMng\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	        ...
	        */
	        {caption: ["처리","처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdPrdcrCrclOgnSpItmPurSalNMng\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdPrdcrCrclOgnSpItmPurSalNMng\", " + nRow + ")'>삭제</button>";
	        	}
	        }}
	    ];

	    grdPrdcrCrclOgnSpItmPurSalNMng = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdPrdcrCrclOgnSpItmPurSalNMng.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdPrdcrCrclOgnSpItmPurSalNMng.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/pd/pcom/selectPrdcrCrclOgnSpItmPurSalNMng.do", {apcCd : apcCd});
        ...
        try{
        	jsonPrdcrCrclOgnSpItmPurSalNMng.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let PrdcrCrclOgnSpItmPurSalNMngVO = {
					testNo 		: item.testNo
				  , aa 		: item.aa
				  , bb 		: item.bb
				  , cc 	: item.cc
				...
				jsonPrdcrCrclOgnSpItmPurSalNMng.push(PrdcrCrclOgnSpItmPurSalNMngVO);
			});



        	grdPrdcrCrclOgnSpItmPurSalNMng.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdPrdcrCrclOgnSpItmPurSalNMng.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdPrdcrCrclOgnSpItmPurSalNMng.getGridDataAll();
		let saveList = [];

		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdPrdcrCrclOgnSpItmPurSalNMng.getRowData(i);
			let rowSts = grdPrdcrCrclOgnSpItmPurSalNMng.getRowStatus(i);
			let bb = rowData.bb;
			let aa = rowData.aa;
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdPrdcrCrclOgnSpItmPurSalNMng") {
            	grdPrdcrCrclOgnSpItmPurSalNMng.setCellData(nRow, nCol, "N", true);
            	//grdPrdcrCrclOgnSpItmPurSalNMng.setCellData(nRow, 5, gv_apcCd, true);
            	grdPrdcrCrclOgnSpItmPurSalNMng.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdPrdcrCrclOgnSpItmPurSalNMng") {
            	if(grdPrdcrCrclOgnSpItmPurSalNMng.getRowStatus(nRow) == 0 || grdPrdcrCrclOgnSpItmPurSalNMng.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var PrdcrCrclOgnSpItmPurSalNMngVO = grdPrdcrCrclOgnSpItmPurSalNMng.getRowData(nRow);
            			fn_deleteRsrc(PrdcrCrclOgnSpItmPurSalNMngVO);
            			grdPrdcrCrclOgnSpItmPurSalNMng.deleteRow(nRow);
            		}
            	}else{
            		grdPrdcrCrclOgnSpItmPurSalNMng.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(PrdcrCrclOgnSpItmPurSalNMngVO){
		let postJsonPromise = gfn_postJSON("/pd/pcom/deletePrdcrCrclOgnSpItmPurSalNMng.do", PrdcrCrclOgnSpItmPurSalNMngVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		...
		//grdPrdcrCrclOgnSpItmPurSalNMng 그리드 객체
        selGridRow = grdPrdcrCrclOgnSpItmPurSalNMng.getRow();
        selGridCol = grdPrdcrCrclOgnSpItmPurSalNMng.getCol();


        let delYnCol = grdPrdcrCrclOgnSpItmPurSalNMng.getColRef('delYn');
        let delYnValue = grdPrdcrCrclOgnSpItmPurSalNMng.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdPrdcrCrclOgnSpItmPurSalNMng.getColRef('cc');
        let ddCol = grdPrdcrCrclOgnSpItmPurSalNMng.getColRef('dd');
        let iiCol = grdPrdcrCrclOgnSpItmPurSalNMng.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdPrdcrCrclOgnSpItmPurSalNMng.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdPrdcrCrclOgnSpItmPurSalNMng.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdPrdcrCrclOgnSpItmPurSalNMng.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdPrdcrCrclOgnSpItmPurSalNMng.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdPrdcrCrclOgnSpItmPurSalNMng.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdPrdcrCrclOgnSpItmPurSalNMng.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdPrdcrCrclOgnSpItmPurSalNMng.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdPrdcrCrclOgnSpItmPurSalNMng.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdPrdcrCrclOgnSpItmPurSalNMng.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
