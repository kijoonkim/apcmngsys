...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/pd/pcorm/selectPrdcrCrclOgnSpItmPurSalNMng.do", {apcCd : apcCd});
    	let postJsonPromise = gfn_postJSON("/pd/pcorm/selectPrdcrCrclOgnSpItmPurSalNMngList.do", {
    		ccCode : ccCode
    		,iiCode : iiCode
		});
	...
	const fn_saveFmList = async function(){
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/pd/pcorm/multiSavePrdcrCrclOgnSpItmPurSalNMngList.do", saveList);
	        let data = await postJsonPromise;
	        try {
	...
	async function fn_deleteRsrc(PrdcrCrclOgnSpItmPurSalNMngVO){
		let postJsonPromise = gfn_postJSON("/pd/pcorm/deletePrdcrCrclOgnSpItmPurSalNMng.do", PrdcrCrclOgnSpItmPurSalNMngVO);
        let data = await postJsonPromise;
