		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdPrdcrCrclOgnGenalTblMng" style="height:350px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonPrdcrCrclOgnGenalTblMng = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdPrdcrCrclOgnGenalTblMng', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdPrdcrCrclOgnGenalTblMng';
	    SBGridProperties.id = 'grdPrdcrCrclOgnGenalTblMng';
	    SBGridProperties.jsonref = 'jsonPrdcrCrclOgnGenalTblMng';
	    SBGridProperties.emptyrecords = '데이터가 없습니다.';
	    SBGridProperties.selectmode = 'byrow';
	    SBGridProperties.extendlastcol = 'scroll';
	    SBGridProperties.oneclickedit = true;
	    SBGridProperties.columns = [
	    	{caption: ["구분"], 		ref: 'testNo',   	type:'input',  hidden : false},
	    	{caption: ["통합조직 총취급액(A)"], 	ref: 'aa',   	type:'combo',  width:'100px',    style:'text-align:center;',
...
// 	        {caption: ["사업자등록번호"], 	ref: 'bb',   type:'input',  width:'250px',    style:'text-align:center', validate : gfn_chkByte.bind({byteLimit: 100})},
	        {caption: ["생산자조직 전속출하액(B)"], 	ref: 'ii',   	type:'output',  width:'80px',    style:'text-align:center'},
	        {caption: ["전속취급률(B/A)"], 	ref: 'iiCode',   	type:'input',  width:'80px',    style:'text-align:center' ,  editable : false},
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdPrdcrCrclOgnGenalTblMng\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdPrdcrCrclOgnGenalTblMng\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdPrdcrCrclOgnGenalTblMng\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	        ...
	        */
	        {caption: ["처리","처리"], 		ref: 'delYn',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdPrdcrCrclOgnGenalTblMng\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdPrdcrCrclOgnGenalTblMng\", " + nRow + ")'>삭제</button>";
	        	}
	        }}
	    ];

	    grdPrdcrCrclOgnGenalTblMng = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdPrdcrCrclOgnGenalTblMng.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdPrdcrCrclOgnGenalTblMng.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/pd/pcom/selectPrdcrCrclOgnGenalTblMng.do", {apcCd : apcCd});
        ...
        try{
        	jsonPrdcrCrclOgnGenalTblMng.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let PrdcrCrclOgnGenalTblMngVO = {
					testNo 		: item.testNo
				  , aa 		: item.aa
				  , bb 		: item.bb
				  , cc 	: item.cc
				...
				jsonPrdcrCrclOgnGenalTblMng.push(PrdcrCrclOgnGenalTblMngVO);
			});



        	grdPrdcrCrclOgnGenalTblMng.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdPrdcrCrclOgnGenalTblMng.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdPrdcrCrclOgnGenalTblMng.getGridDataAll();
		let saveList = [];

		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdPrdcrCrclOgnGenalTblMng.getRowData(i);
			let rowSts = grdPrdcrCrclOgnGenalTblMng.getRowStatus(i);
			let bb = rowData.bb;
			let aa = rowData.aa;
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/pd/pcom/multiSavePrdcrCrclOgnGenalTblMngList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdPrdcrCrclOgnGenalTblMng") {
            	grdPrdcrCrclOgnGenalTblMng.setCellData(nRow, nCol, "N", true);
            	//grdPrdcrCrclOgnGenalTblMng.setCellData(nRow, 5, gv_apcCd, true);
            	grdPrdcrCrclOgnGenalTblMng.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdPrdcrCrclOgnGenalTblMng") {
            	if(grdPrdcrCrclOgnGenalTblMng.getRowStatus(nRow) == 0 || grdPrdcrCrclOgnGenalTblMng.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var PrdcrCrclOgnGenalTblMngVO = grdPrdcrCrclOgnGenalTblMng.getRowData(nRow);
            			fn_deleteRsrc(PrdcrCrclOgnGenalTblMngVO);
            			grdPrdcrCrclOgnGenalTblMng.deleteRow(nRow);
            		}
            	}else{
            		grdPrdcrCrclOgnGenalTblMng.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(PrdcrCrclOgnGenalTblMngVO){
		let postJsonPromise = gfn_postJSON("/pd/pcom/deletePrdcrCrclOgnGenalTblMng.do", PrdcrCrclOgnGenalTblMngVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		...
		//grdPrdcrCrclOgnGenalTblMng 그리드 객체
        selGridRow = grdPrdcrCrclOgnGenalTblMng.getRow();
        selGridCol = grdPrdcrCrclOgnGenalTblMng.getCol();


        let delYnCol = grdPrdcrCrclOgnGenalTblMng.getColRef('delYn');
        let delYnValue = grdPrdcrCrclOgnGenalTblMng.getCellData(selGridRow,delYnCol);
        ...
        //cc 전문 품목 dd 육성 품목
        let ccCol = grdPrdcrCrclOgnGenalTblMng.getColRef('cc');
        let ddCol = grdPrdcrCrclOgnGenalTblMng.getColRef('dd');
        let iiCol = grdPrdcrCrclOgnGenalTblMng.getColRef('ii');
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdPrdcrCrclOgnGenalTblMng.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdPrdcrCrclOgnGenalTblMng.getColRef('iiCode');//ii 통합조직 코드 인덱스
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdPrdcrCrclOgnGenalTblMng.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdPrdcrCrclOgnGenalTblMng.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdPrdcrCrclOgnGenalTblMng.getColRef(selRef+"Code");//품목코드 인덱스
