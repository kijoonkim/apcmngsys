		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdApcFarmOgnzInfo" style="height:650px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonapcFarmOgnzInfo = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdApcFarmOgnzInfo', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdApcFarmOgnzInfo';
	    SBGridProperties.id = 'grdApcFarmOgnzInfo';
	    SBGridProperties.jsonref = 'jsonapcFarmOgnzInfo';
	    ...
	    SBGridProperties.columns = [
	        ...
	        {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'10%',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdApcFarmOgnzInfo\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdApcFarmOgnzInfo\", " + nRow + ")'>삭제</button>";
	        	}
	        }}
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdApcFarmOgnzInfo\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdApcFarmOgnzInfo\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdApcFarmOgnzInfo\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	    ...
	    grdApcFarmOgnzInfo = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdApcFarmOgnzInfo.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdApcFarmOgnzInfo.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/pd/bsm/selectapcFarmOgnzInfo.do", {apcCd : apcCd});
    	let postJsonPromise = gfn_postJSON("/pd/bsm/selectApcFarmOgnzInfoList.do", {
    		ognzNm : ognzNm
    		,ognzSeNm : ognzSeNm
    		,typeNm : typeNm
		});
        ...
        try{
        	jsonapcFarmOgnzInfo.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let apcFarmOgnzInfoVO = {
					ognzCd : item.ognzCd
				  , ognzNm 		: item.ognzNm
				...
				jsonapcFarmOgnzInfo.push(apcFarmOgnzInfoVO);
			});



        	grdApcFarmOgnzInfo.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdApcFarmOgnzInfo.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdApcFarmOgnzInfo.getGridDataAll();
		let saveList = [];
		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdApcFarmOgnzInfo.getRowData(i);
			let rowSts = grdApcFarmOgnzInfo.getRowStatus(i);
...
// 		  		}
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/pd/bsm/multiSaveApcFarmOgnzInfoList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdApcFarmOgnzInfo") {
            	grdApcFarmOgnzInfo.setCellData(nRow, nCol, "N", true);
            	//grdApcFarmOgnzInfo.setCellData(nRow, 5, gv_apcCd, true);
            	grdApcFarmOgnzInfo.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdApcFarmOgnzInfo") {
            	if(grdApcFarmOgnzInfo.getRowStatus(nRow) == 0 || grdApcFarmOgnzInfo.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var apcFarmOgnzInfoVO = grdApcFarmOgnzInfo.getRowData(nRow);
            			fn_deleteRsrc(apcFarmOgnzInfoVO);
            			grdApcFarmOgnzInfo.deleteRow(nRow);
            		}
            	}else{
            		grdApcFarmOgnzInfo.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(apcFarmOgnzInfoVO){
		let postJsonPromise = gfn_postJSON("/pd/bsm/deleteApcFarmOgnzInfo.do", apcFarmOgnzInfoVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		console.log("================gridClick================");
		//grdApcFarmOgnzInfo 그리드 객체
        selGridRow = grdApcFarmOgnzInfo.getRow();
        selGridCol = grdApcFarmOgnzInfo.getCol();


        let delYnCol = grdApcFarmOgnzInfo.getColRef('delYn');
        let delYnValue = grdApcFarmOgnzInfo.getCellData(selGridRow,delYnCol);
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdApcFarmOgnzInfo.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdApcFarmOgnzInfo.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdApcFarmOgnzInfo.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdApcFarmOgnzInfo.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdApcFarmOgnzInfo.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdApcFarmOgnzInfo.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdApcFarmOgnzInfo.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdApcFarmOgnzInfo.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdApcFarmOgnzInfo.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
	...
	const fn_saveFrmerSn= async function(){
		...
		if(confirm(regMsg)){
			...
			//let postJsonPromise = gfn_postJSON("/co/cd/multiSaveComCdDtlList.do", saveList);
			// let postJsonPromise = gfn_postJSON("/pd/bsm/multiSaveReleyapcFarmOgnzInfoList.do", {
			let postJsonPromise = gfn_postJSON("/pd/bsm/multiSaveReleyApcFarmOgnzInfoJsoneList.do", {
	    		 frmerSn : frmerSn
	 		});
	        let data = await postJsonPromise;
	        try {
