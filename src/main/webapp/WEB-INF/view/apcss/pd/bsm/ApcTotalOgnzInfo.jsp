		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdApcTotalOgnzInfo" style="height:650px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonapcTotalOgnzInfo = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdApcTotalOgnzInfo', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdApcTotalOgnzInfo';
	    SBGridProperties.id = 'grdApcTotalOgnzInfo';
	    SBGridProperties.jsonref = 'jsonapcTotalOgnzInfo';
	    SBGridProperties.emptyrecords = '데이터가 없습니다.';
	    SBGridProperties.selectmode = 'byrow';
	    SBGridProperties.extendlastcol = 'scroll';
	    SBGridProperties.oneclickedit = true;
	    SBGridProperties.columns = [
	    	{caption: ["조직코드"], 		ref: 'ognzCd',   	type:'input',  width:'10%', hidden : false},
	        {caption: ["조직명"], 	ref: 'ognzNm',   	type:'input',   width:'20%',   style:'text-align:center'},
	        ...
	        {caption: ["사업자번호"], 	ref: 'brno',   	type:'input',   width:'12%',  style:'text-align:center'},
	        {caption: ["유형코드"], 	ref: 'typeCd',   	type:'input',  width:'10%',   style:'text-align:center'},
	        {caption: ["유형명"], 	ref: 'typeNm',   	type:'input',  width:'10%',   style:'text-align:center'},
	        {caption: ["APC보유수"], 	ref: 'apcHldCnt',   	type:'input',   width:'10%',  style:'text-align:center'},
	        {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'8%',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
	        		return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"ADD\", \"grdApcTotalOgnzInfo\", " + nRow + ", " + nCol + ")'>추가</button>";
	        	}else{
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdApcTotalOgnzInfo\", " + nRow + ")'>삭제</button>";
	        	}
	        }}
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdApcTotalOgnzInfo\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdApcTotalOgnzInfo\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdApcTotalOgnzInfo\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	    ...
	    grdApcTotalOgnzInfo = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdApcTotalOgnzInfo.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdApcTotalOgnzInfo.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
    	...
    	//let postJsonPromise = gfn_postJSON("/pd/bsm/selectapcTotalOgnzInfo.do", {apcCd : apcCd});
    	let postJsonPromise = gfn_postJSON("/pd/bsm/selectApcTotalOgnzInfoList.do", {
    		ognzNm : ognzNm
    		,ognzSeNm : ognzSeNm
    		,typeNm : typeNm
		});
        ...
        try{
        	jsonapcTotalOgnzInfo.length = 0;
        	console.log("data==="+data);
        	data.resultList.forEach((item, index) => {
				let apcTotalOgnzInfoVO = {
					ognzCd : item.ognzCd
				  , ognzNm 		: item.ognzNm
				...
				jsonapcTotalOgnzInfo.push(apcTotalOgnzInfoVO);
			});



        	grdApcTotalOgnzInfo.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdApcTotalOgnzInfo.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdApcTotalOgnzInfo.getGridDataAll();
		let saveList = [];
		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdApcTotalOgnzInfo.getRowData(i);
			let rowSts = grdApcTotalOgnzInfo.getRowStatus(i);
...
// 		  		}
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/pd/bsm/multiSaveApcTotalOgnzInfoList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdApcTotalOgnzInfo") {
            	grdApcTotalOgnzInfo.setCellData(nRow, nCol, "N", true);
            	//grdApcTotalOgnzInfo.setCellData(nRow, 5, gv_apcCd, true);
            	grdApcTotalOgnzInfo.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdApcTotalOgnzInfo") {
            	if(grdApcTotalOgnzInfo.getRowStatus(nRow) == 0 || grdApcTotalOgnzInfo.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var apcTotalOgnzInfoVO = grdApcTotalOgnzInfo.getRowData(nRow);
            			fn_deleteRsrc(apcTotalOgnzInfoVO);
            			grdApcTotalOgnzInfo.deleteRow(nRow);
            		}
            	}else{
            		grdApcTotalOgnzInfo.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(apcTotalOgnzInfoVO){
		let postJsonPromise = gfn_postJSON("/pd/bsm/deleteApcTotalOgnzInfo.do", apcTotalOgnzInfoVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		console.log("================gridClick================");
		//grdApcTotalOgnzInfo 그리드 객체
        selGridRow = grdApcTotalOgnzInfo.getRow();
        selGridCol = grdApcTotalOgnzInfo.getCol();


        let delYnCol = grdApcTotalOgnzInfo.getColRef('delYn');
        let delYnValue = grdApcTotalOgnzInfo.getCellData(selGridRow,delYnCol);
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdApcTotalOgnzInfo.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdApcTotalOgnzInfo.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdApcTotalOgnzInfo.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdApcTotalOgnzInfo.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdApcTotalOgnzInfo.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdApcTotalOgnzInfo.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdApcTotalOgnzInfo.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdApcTotalOgnzInfo.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdApcTotalOgnzInfo.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
	...
	const fn_saveFrmerSn= async function(){
		...
		if(confirm(regMsg)){
			...
			//let postJsonPromise = gfn_postJSON("/co/cd/multiSaveComCdDtlList.do", saveList);
			// let postJsonPromise = gfn_postJSON("/pd/bsm/multiSaveReleyapcTotalOgnzInfoList.do", {
			let postJsonPromise = gfn_postJSON("/pd/bsm/multiSaveReleyApcTotalOgnzInfoJsoneList.do", {
	    		 frmerSn : frmerSn
	 		});
	        let data = await postJsonPromise;
	        try {
