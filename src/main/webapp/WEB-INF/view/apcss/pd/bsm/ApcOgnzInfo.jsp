		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<div class="ad_section_top">
					<!-- SBGrid를 호출합니다. -->
					<div id="sb-area-grdApcOgnzInfo" style="height:650px; width: 100%;"></div>
				</div>
			</div>
		</div>
	</section>
...
<script type="text/javascript">
	...
	//설비 등록
	var jsonapcOgnzInfo = []; // 그리드의 참조 데이터 주소 선언
	var jsonComFcltGubun = [];

	const fn_initSBSelectFclt = async function() {

		let rst = await Promise.all([
			gfn_setComCdSBSelect('grdApcOgnzInfo', 		jsonComFcltGubun, 	'FCLT_GUBUN') 		// 설비구분
		])
	...
	const fn_fcltMngCreateGrid = async function() {
	    ...
	    SBGridProperties.parentid = 'sb-area-grdApcOgnzInfo';
	    SBGridProperties.id = 'grdApcOgnzInfo';
	    SBGridProperties.jsonref = 'jsonapcOgnzInfo';
	    ...
	    SBGridProperties.columns = [
	        ...
	        {caption: ["처리"], 		ref: 'delYn',   	type:'button', width:'10%',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	if(strValue== null || strValue == ""){
			        ...
			        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_procRow(\"DEL\", \"grdApcOgnzInfo\", " + nRow + ")'>삭제</button>";
	        	}
	        }}
...
// 	        {caption: ["통합조직 코드"], 	ref: 'iiCode',   	type:'input', hidden:true},
	        ...
	        {caption: ["통합조직"], 		ref: 'ii',   	type:'button', width:'80px',    style:'text-align:center', renderer: function(objGrid, nRow, nCol, strValue, objRowData){
	        	...
	        	if(strValue== null || strValue == ""){
	        		...
	        		return "<sbux-button type='button' uitype='modal' target-id='modal-itemSelect' class='btn btn-xs btn-outline-danger' text='찾기'  onClick='fn_GridPop(\"pop\", \"grdApcOgnzInfo\", " + nRow + ", " + nCol + ")'></sbux-button>"
	        		//return " <button type='button' class='btn btn-xs btn-outline-danger'   onClick='fn_GridPop(\"pop\", \"grdApcOgnzInfo\", " + nRow + ", " + nCol + ")'>찾기</button>";
	        		//return " <sbux-button type='button' uitype='modal' target-id='modal-itemSelect'  text='찾기'  onClick='fn_GridPop(\"pop\", \"grdApcOgnzInfo\", " + nRow + ", " + nCol + ")'></sbux-button>";
	        	}else{
	    ...
	    grdApcOgnzInfo = _SBGrid.create(SBGridProperties);
	    let rst = await Promise.all([
	    	fn_initSBSelectFclt(),
		    fn_searchFcltList()
		])
		grdApcOgnzInfo.refresh({"combo":true});
	  	//클릭 이벤트 바인드
	    grdApcOgnzInfo.bind('click','gridClick');
	...
	const fn_searchFcltList = async function(){
        ...
        try{
        	...
        	grdApcOgnzInfo.rebuild();

        	//비어 있는 마지막 줄 추가용도?
        	grdApcOgnzInfo.addRow();
        }catch (e) {
	...
	const fn_saveFmList = async function(){
		let gridData = grdApcOgnzInfo.getGridDataAll();
		let saveList = [];
		for(var i=1; i<=gridData.length; i++ ){

			let rowData = grdApcOgnzInfo.getRowData(i);
			let rowSts = grdApcOgnzInfo.getRowStatus(i);
...
// 		  		}
		...
		if(confirm(regMsg)){
			...
			let postJsonPromise = gfn_postJSON("/pd/bsm/multiSaveApcOgnzInfoList.do", saveList);
	        let data = await postJsonPromise;
	        try {
    ...
    function fn_procRow(gubun, grid, nRow, nCol) {
        if (gubun === "ADD") {
            if (grid === "grdApcOgnzInfo") {
            	grdApcOgnzInfo.setCellData(nRow, nCol, "N", true);
            	//grdApcOgnzInfo.setCellData(nRow, 5, gv_apcCd, true);
            	grdApcOgnzInfo.addRow(true);
            }
        }
        else if (gubun === "DEL") {
            if (grid === "grdApcOgnzInfo") {
            	if(grdApcOgnzInfo.getRowStatus(nRow) == 0 || grdApcOgnzInfo.getRowStatus(nRow) == 2){
            		var delMsg = "등록 된 행 입니다. 삭제 하시겠습니까?";
            		if(confirm(delMsg)){
            			var apcOgnzInfoVO = grdApcOgnzInfo.getRowData(nRow);
            			fn_deleteRsrc(apcOgnzInfoVO);
            			grdApcOgnzInfo.deleteRow(nRow);
            		}
            	}else{
            		grdApcOgnzInfo.deleteRow(nRow);
            	}
	...
	async function fn_deleteRsrc(apcOgnzInfoVO){
		let postJsonPromise = gfn_postJSON("/pd/bsm/deleteApcOgnzInfo.do", apcOgnzInfoVO);
        let data = await postJsonPromise;
    ...
    function gridClick(){
		console.log("================gridClick================");
		//grdApcOgnzInfo 그리드 객체
        selGridRow = grdApcOgnzInfo.getRow();
        selGridCol = grdApcOgnzInfo.getCol();


        let delYnCol = grdApcOgnzInfo.getColRef('delYn');
        let delYnValue = grdApcOgnzInfo.getCellData(selGridRow,delYnCol);
	...
	const fn_setGridInvstmntSpmt = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			let colRefIdx1 = grdApcOgnzInfo.getColRef('ii');//ii 통합조직 인덱스
			let colRefIdx2 = grdApcOgnzInfo.getColRef('iiCode');//ii 통합조직 코드 인덱스

			//그리드 값 세팅
			grdApcOgnzInfo.setCellData(selGridRow,colRefIdx1,rowData.mainCodeNm,true);
			grdApcOgnzInfo.setCellData(selGridRow,colRefIdx2,rowData.mainCode,true);
		}
	}
	...
	const fn_setGridItem = function(rowData) {
		...
		if (!gfn_isEmpty(rowData)) {
			...
			//getColRef(ref) ref의 인덱스 값 가져오기
			let selRef = grdApcOgnzInfo.getRefOfCol(selGridCol);

			//구분하기 편하기 위해 ref 값이 cc라면 코드값은 ccCode 로 설정
			let colRefIdx1 = grdApcOgnzInfo.getColRef(selRef);//품목명 인덱스
			let colRefIdx2 = grdApcOgnzInfo.getColRef(selRef+"Code");//품목코드 인덱스

			//그리드 값 세팅
			grdApcOgnzInfo.setCellData(selGridRow,colRefIdx1,rowData.itemNm,true);
			grdApcOgnzInfo.setCellData(selGridRow,colRefIdx2,rowData.itemCd,true);
		}
	}
	...
	const fn_saveFrmerSn= async function(){
		...
		if(confirm(regMsg)){
			...
			//let postJsonPromise = gfn_postJSON("/co/cd/multiSaveComCdDtlList.do", saveList);
			// let postJsonPromise = gfn_postJSON("/pd/bsm/multiSaveReleyapcOgnzInfoList.do", {
			let postJsonPromise = gfn_postJSON("/pd/bsm/multiSaveReleyApcOgnzInfoJsoneList.do", {
	    		 frmerSn : frmerSn
	 		});
	        let data = await postJsonPromise;
	        try {
