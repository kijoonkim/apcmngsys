		...
		<div class="box box-solid">
			...
			<c:if test="${loginVO.userType eq '01' || loginVO.userType eq '00' ||  loginVO.userType eq '02'}">
				...
				<table class="table table-bordered tbl_fixed">
					...
					<tbody>
						<tr>
							...
							<th scope="row" class="th_bg" >시도/시군구</th>
				...
				<table class="table table-bordered tbl_fixed">
					...
					<c:if test="${loginVO.userType eq '01' || loginVO.userType eq '00' ||  loginVO.userType eq '02'}">
						<tr>
							<th colspan="2" scope="row" class="th_bg"></span>통합조직 법인명</th>
							<td colspan="13" class="td_input" style="border-right:hidden;">
								<sbux-input id="dtl-input-uoCorpNm" name="dtl-input-uoCorpNm" class="form-control input-sm" autocomplete="off" uitype="text" readonly></sbux-input>
							</td>
						</tr>
					</c:if>
						<tr>
					...
					<c:if test="${loginVO.userType eq '01' || loginVO.userType eq '00' ||  loginVO.userType eq '02'}">
						<tr>
							<th colspan="2" scope="row" class="th_bg"><span class="data_required" ></span>본번</th>
							<td colspan="3" class="td_input" style="border-right: hidden;">
								<sbux-spinner
									id="dtl-input-mno"
									name="dtl-input-mno"
							...
							<th colspan="2" scope="row" class="th_bg"><span class="data_required" ></span>부번</th>
							<td colspan="3" class="td_input" style="border-right:hidden;" >
								<sbux-spinner
									id="dtl-input-sno"
									name="dtl-input-sno"
...
*/
	...
	const fn_initSBSelect = async function() {
		...
		let rst = await Promise.all([
			//검색조건
			//시도 시군 공통코드 변경 CMPTN_INST_CTPV, CMPTN_INST_SIGUN- > UNTY_CTPV , UNTY_SGG
			<c:if test="${loginVO.userType eq '01' || loginVO.userType eq '00' || loginVO.userType eq '02'}">
			...
			gfn_setComCdSBSelect('grdPrdcrCrclOgnMng', 		jsonGrdCtpv, 		'UNTY_CTPV'), 	//시도
			gfn_setComCdSBSelect('grdPrdcrCrclOgnMng', 		jsonGrdSgg, 		'UNTY_SGG'),	 //시군
			...
			gfn_setComCdSBSelect('dtl-input-ctpv', 			jsonComDtlCtpv, 	'UNTY_CTPV'), 	//시도
			gfn_setComCdSBSelect('dtl-input-sgg', 			jsonComDtlSgg, 		'UNTY_SGG'),	//시군
			...
			SBUxMethod.set("srch-inp-crtrYr", 	gfn_dateToYear(new Date())) // 기준연도
		]);
	}
	...
	const fn_fcltMngCreateGrid = async function() {
		...
		SBGridProperties.columns = [
			...
			{caption: ["상세내역"], 	ref: 'mno',   	hidden : true},
			{caption: ["상세내역"], 	ref: 'sno',   	hidden : true}
		];
	...
	const fn_setGrdFcltList = async function(pageSize, pageNo){
		...
		let corpNm = SBUxMethod.get("srch-input-corpNm");//
		let apoSe = SBUxMethod.get("srch-input-apoSe");//
		const crtrYr = SBUxMethod.get("srch-inp-crtrYr"); // 기준연도
	...
	const fn_dtlSearch = async function(){
		...
		try{
			...
			data.resultList.forEach((item, index) => {
				...
				SBUxMethod.set('dtl-input-ctpv',gfn_nvl(item.ctpv).trim(), { isTriggerExt : true })
				SBUxMethod.set('dtl-input-sgg',gfn_nvl(item.sgg).trim());
			});
	...
	*/
	...
	const fn_view = async function (){
		...
		SBUxMethod.set('dtl-input-mno',gfn_nvl(rowData.mno))//
		SBUxMethod.set('dtl-input-sno',gfn_nvl(rowData.sno))//
		</c:if>

		SBUxMethod.set('dtl-input-ctpv',gfn_nvl(rowData.ctpv).trim(), { isTriggerExt : true })
		SBUxMethod.set('dtl-input-sgg',gfn_nvl(rowData.sgg).trim());

		SBUxMethod.set('dtl-input-uoCorpNm',gfn_nvl(rowData.uoCorpNm)); //통합조직 법인명
		SBUxMethod.set('dtl-input-uoBrno',gfn_nvl(rowData.uoBrno)); //통합조직 사업자번호
		// SBUxMethod.set('dtl-input-uoApoCd',gfn_nvl(rowData.uoApoCd)); //통합조직 조직코드
	...
	function fn_create() {
		...
		<c:if test="${loginVO.userType eq '01' || loginVO.userType eq '00' ||  loginVO.userType eq '02'}">
		SBUxMethod.set('dtl-input-slctnYr',null)//
		SBUxMethod.set('dtl-input-mno',null)//
		SBUxMethod.set('dtl-input-sno',null)//
		</c:if>
	}
	...
	const fn_clearForm = function() {
		...
		SBUxMethod.set('dtl-input-uoCorpNm',null); // 통합조직 법인명
		SBUxMethod.set('dtl-input-uoApoCd',null); // 통합조직 조직코드
	}
	...
	const fn_hiddenGrdSelect = async function(){
		await fn_hiddenGrd();
		//콤보박스 설정
		//공통코드 변경 CMPTN_INST_CTPV,CMPTN_INST_SIGUN -> UNTY_CTPV,UNTY_SGG
		await gfn_setComCdSBSelect('hiddenGrd', 	jsonComCmptnInstH, 	'CMPTNC_INST'); 	//관할기관
		await gfn_setComCdSBSelect('hiddenGrd', 	jsonComCtpvH, 		'UNTY_CTPV'); 	//시도
		await gfn_setComCdSBSelect('hiddenGrd', 	jsonComSggH, 		'UNTY_SGG');	 //시군
