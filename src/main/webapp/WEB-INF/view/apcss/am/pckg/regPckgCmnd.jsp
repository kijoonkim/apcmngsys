<%
 /**
  * @Class Name : regPckgCmnd.jsp
  * @Description : 포장지시등록 화면
  * @author SI개발부
  * @since 2023.08.31
  * @version 1.0
				...
				<table class="table table-bordered tbl_fixed">
					...
					<tbody>
						<tr>
							...
							<td class="td_input" style="border-right: hidden;">
								<sbux-select unselected-text="전체" uitype="single" id="srch-slt-itemCd" name="srch-slt-itemCd" class="form-control input-sm" jsondata-ref="jsonComItem" onchange="fn_onChangeSrchItemCd(this)"></sbux-select>
							</td>
							<td class="td_input" style="border-right: hidden;">
								<sbux-select unselected-text="선택" uitype="single" id="srch-slt-vrtyCd" name="srch-slt-vrtyCd" class="form-control input-sm input-sm-ast inpt_data_reqed" jsondata-ref="jsonComVrty" onchange="fn_onChangeSrchVrtyCd(this)"></sbux-select>
							</td>
							<td>&nbsp;</td>
...
<script type="text/javascript">
	...
	function fn_createSortInvntrGrid() {
	    ...
	    SBGridProperties.columns = [
	    	{
	    		caption : ["전체","<input type='checkbox' id='allCheckBox' onchange='fn_checkAllSortInvntr(grdSortInvntr, this);'>"],
	        ...
	        {caption: ["선별","수량"],  		ref: 'invntrQntt',    	type:'output',  width:'100px',    style:'text-align:right',
...
// 				typeinfo : {ref:'jsonComSpcfct', 	displayui : false,	itemcount: 10, label:'label', value:'value', unselect: {label : '선택', value: ''}}},
    ...
    const fn_setValue = async function(){
    	...
    	if(nCol == 0){
			...
			if((pckgQntt == 0 && pckgWght == 0) || (gfn_isEmpty(pckgQntt) && gfn_isEmpty(pckgWght))){
				grdSortInvntr.setCellData(nRow, pckgQnttCol, invntrQntt);
				grdSortInvntr.setCellData(nRow, pckgWghtCol, invntrWght);
			}
    	}
    }
	...
	function fn_createPckgCmndGrid() {
	    ...
	    SBGridProperties.columns = [
	        ...
	        {caption: ["납기일자"],		ref: 'dudtYmd',     type:'output',  	width:'100px',  style:'text-align:center',
	        	format : {type:'date', rule:'yyyy-mm-dd', origin:'yyyymmdd'}},
	        ...
	        {caption: ["지시수량"],		ref: 'cmndQntt',    type:'output',  	width:'60px',   style:'text-align:right',
	        	format : {type:'number', rule:'#,###'}},
	        {caption: ["지시중량 (Kg)"],	ref: 'cmndWght',    type:'output',  	width:'80px',   style:'text-align:right',
	        	format : {type:'number', rule:'#,###'}},
    ...
    function fn_personalSave(){
    	grdPckgCmnd.savePersonalInfo("apcCd");
   	}
    // 개인화 호출
    function fn_personalLoad(){
    	grdPckgCmnd.loadPersonalInfo("apcCd");
   	}
	// 열 숨기기
    function fn_colHidden(){
    	grdPckgCmnd.setColHidden(grdPckgCmnd.getCol(), true);
   	}
	// 열 보이기
    function fn_colShow(){
    	for(let i = grdPckgCmnd.getFixedCols(); i < grdPckgCmnd.getCols()-1; i++) {
   			grdPckgCmnd.setColHidden(i, false);
    	}
   	}
