...
<script type="text/javascript">
	...
	function fn_createGridPckgPrfmnc() {
        ...
        SBGridProperties.columns = [
            ...
            {caption: ["상품명"],		ref: 'spmtPckgUnitCd',	type:'combo',  	width:'200px', style: 'text-align:center;background-color:#FFF8DC;',
            ...
            /*
            {caption: ["포장규격"],		ref: 'spcfctCd',	type:'combo',  	width:'100px', style: 'text-align:center',
        ...
        const columns2 = [
            ...
            {caption: ["포장중량"], 			ref: 'pckgWght',  		type:'input',  width:'80px', style: 'text-align:right;',
	        ...
	        {caption: ["규격"], ref: 'spcfctCd', 	type:'output',	hidden: true},
	...
	const fn_save = async function() {
		...
		for ( let i=0; i<allPckgData.length-1; i++ ) {
			...
			if (gfn_isEmpty(spmtPckgUnitCd) || spmtPckgUnitCd === "notSelect") {
				gfn_comAlert("W0005", "포장단위");		//	W0005	{0}이/가 없습니다.
				return;
			}
		...
		// comConfirm
		if (!gfn_comConfirm("Q0001", "포장실적 등록")) {	//	Q0001	{0} 하시겠습니까?
	    	return;
	    }
	...
	const fn_grdSortInvntrValueChanged = function(event) {
 	...
 	const fn_setPckgWghtInfo = function() {
 		let totPckgWght = 0;
		const allPckgData = grdPckgPrfmnc.getGridDataAll();
		allPckgData.forEach((item, index) => {
			if (!gfn_isEmpty(item.pckgYmd)) {
				totPckgWght += parseFloat(item.pckgWght) || 0;
			}
		});

		SBUxMethod.set("dtl-inp-pckgWght", gfn_apcEstmtWght(totPckgWght));
 	}
	...
	const fn_getExpColumns = function() {
		...
		_columns.push(
			...
			{caption: ["포장구분"], 	ref: 'spmtPckgUnitCd',   		type:'combo',  width:'120px',    style:'text-align:center; background:#FFF8DC;',
				typeinfo : {
	...
	const fn_dwnld = async function(){
		...
		let fileName = "포장실적(샘플)_" + itemNm + ".xlsx"
...
*/
	...
	const fn_importPckgPrfmnc = async function(_grdImp) {
    	...
    	let postUrl = "/am/pckg/insertImportPckgRslt.do";
	...
	/**
	 * @name fn_setImportCellAbnormal
	 * @description
	 * @param {grid} _grdImp
	 * @param {number} _nRow
	 * @param {number} _nCol
	 */
	const fn_setImportCellAbnormal = function (_grdImp, _nRow, _nCol) {
		const usrAttr = _grdImp.getColUserAttr(_nCol);
		if (!gfn_isEmpty(usrAttr) && usrAttr.hasOwnProperty('abnormalStyles')) {
			_grdImp.setCellStyleEx(_nRow, _nCol, usrAttr.abnormalStyles);
		}
	}
	/**
	 * @name fn_setImportCellNormal
	 * @description
	 * @param {grid} _grdImp
	 * @param {number} _nRow
	 * @param {number} _nCol
	 */
	const fn_setImportCellNormal = function (_grdImp, _nRow, _nCol) {		
		const usrAttr = _grdImp.getColUserAttr(_nCol);
		if (!gfn_isEmpty(usrAttr) && usrAttr.hasOwnProperty('normalStyles')) {
			_grdImp.setCellStyleEx(_nRow, _nCol, usrAttr.normalStyles);
		}
	}
	...
	const fn_setDataAfterImport = function(_grdImp) {
		...
		if (abnormalList.length > 0) {
			abnormalList.forEach((item) => {
				fn_setImportCellAbnormal(_grdImp, item.row, item.col);
			});
		}
		*/
     }
							...
							if (!gfn_isEmpty(chkInfo)) {
								...
								chkInfo = _.find(
										...
										{	
											...
											vrtyCd: rowData.vrtyCd,
											spcfctCd: rowData.spcfctCd,
											spmtPckgUnitNm: chkInfo.spmtPckgUnitNm										
										}
									);
								if (!gfn_isEmpty(chkInfo)) {
									rowData.spmtPckgUnitCd = chkInfo.spmtPckgUnitCd;
...
/*
		if (hasError) {
			fn_setImportCellAbnormal(_grdImp, nRow, nCol);
		} else {
