<%
    /**
     * @Class Name : gdsCrynReg.jsp
     * @Description : 출하상품 반입 등록
     * @author SI개발부
     * @since 2024.11.25
     * @version 1.0
    ...
    <div class="box box-solid">
        ...
        <div class="box-body">
            ...
            <div class="table-responsive tbl_scroll_sm">
                <div id="sb-area-grdSpmt" style="height:230px;"></div>
            </div>
            <table class="table table-bordered tbl_fixed" style="margin-top: 5px;margin-bottom: 5px;">
                ...
                <tbody>
                    <tr>
                        <th scope="row" class="th_bg"><span class="data_required" ></span>반품일자</th>
                        <td colspan="2" class="td_input" style="border-right: hidden;">
                            <sbux-datepicker
                                    uitype="popup"
                    ...
                    <tr>
                        <th scope="row" class="th_bg">비고사항</th>
            ...
            <div class="ad_tbl_top">
                ...
                <div style="display:flex;vertical-align:middle;float:right;margin-right:auto;padding-right: 10px;">
                    <sbux-button
                            id="btn-addTrgt"
                            name="btn-addTrgt"
                            uitype="normal"
...
<script type="application/javascript">
    ...
    const fn_init = async function(){
        ...
        SBUxMethod.set("srch-dtp-spmtYmd", firstYmd + "," + lastYmd);
        ...
        let result = await Promise.all([
            ...
            gfn_setTrsprtsSBSelect('dtl-slt-trsprtCoCd', jsonTrsprtCoCd, gv_selectedApcCd),
    ...
    /**
     * @name fn_createGridSpmt
     * @description 출하실적 그리드 생성
     * @function
     */
    const fn_createGridSpmt = function() {
        ...
        grdSpmt.bind('valuechanged' , fn_grdSpmtValueChanged);
    }

    /**
     * @name fn_grdSpmtValueChanged
     * @description 출하그리드 변경 event 처리
     * @function
     */
    const fn_grdSpmtValueChanged = function() {

        var nRow = grdSpmt.getRow();
        var nCol = grdSpmt.getCol();

        const usrAttr = grdSpmt.getColUserAttr(nCol);

        if (!gfn_isEmpty(usrAttr) && usrAttr.hasOwnProperty('colNm')) {

            const rowData = grdSpmt.getRowData(nRow, false);	// deep copy

            let rmnSpmtQntt = parseInt(rowData.rmnSpmtQntt) || 0;
            let rmnSpmtWght = parseInt(rowData.rmnSpmtWght) || 0;

            let qntt = parseInt(rowData.shpgotQntt) || 0;
            let wght = parseInt(rowData.shpgotWght) || 0;

            let spmtQntt = parseInt(rowData.spmtQntt) || 0;
            let spmtWght = parseInt(rowData.spmtWght) || 0;


            switch (usrAttr.colNm) {
                case "checkedYn":
                    if (_.isEqual("Y", rowData.checkedYn)) {
                        rowData.shpgotQntt = rmnSpmtQntt;
                        rowData.shpgotWght = rmnSpmtWght;
                    } else {
                        rowData.shpgotQntt = 0;
                        rowData.shpgotWght = 0;
                    }

                    grdSpmt.refresh({"focus":false});
                ...
                case "shpgotQntt":
                    ...
                    if (qntt <= 0) {
                        ...
                        if (qntt > rmnSpmtQntt) {
                            ...
                            if (spmtQntt > 0) {
                                rowData.shpgotWght = gfn_apcEstmtWght(spmtWght * qntt / spmtQntt, gv_selectedApcCd);
                            } else {
                                rowData.shpgotWght = gfn_apcEstmtWght(rmnSpmtWght * qntt / rmnSpmtQntt, gv_selectedApcCd);
                            }
     ...
     * @function
     */
    const fn_setGrdSpmt = async function() {

        jsonSpmt.length = 0;

        // 출하 조회
        const spmtYmdFrom = SBUxMethod.get('srch-dtp-spmtYmd_from');    // 출하일자 from
        const spmtYmdTo = SBUxMethod.get('srch-dtp-spmtYmd_to');        // 출하일자 to
        const cnptCd = SBUxMethod.get("srch-slt-cnptCd");               // 거래처
        const itemCd = SBUxMethod.get("srch-slt-itemCd");				    // 품목
        let vrtyCd = SBUxMethod.get("srch-slt-vrtyCd");                 // 품종
        if (!gfn_isEmpty(vrtyCd)) {
            vrtyCd = vrtyCd.substring(4);
        }
        const spmtno = gfn_nvl(SBUxMethod.get("srch-inp-spmtno"));

        try {
            const postJsonPromise = gfn_postJSON("/am/spmt/selectSpmtDtlPrfmncListForCryn.do", {
                apcCd: gv_selectedApcCd,
            ...
            if (_.isEqual("S", data.resultStatus)) {
                data.resultList.forEach((item, index) => {
                    item.shpgotQntt = 0;
                    item.shpgotWght = 0;
                    const idxNo = "" + item.spmtno + item.pckgno + item.pckgsn + item.spmtSn;
                    item.idxNo = idxNo;
                    jsonSpmt.push(item);
                });
        ...
        document.querySelector('#cnt-spmt').innerText = jsonSpmt.length;
    }
    ...
    const fn_addTrgt = async function() {
        ...
        const allData = grdSpmt.getGridDataAll();
        const rowsForAdd = [];

        for ( let i=0; i<allData.length; i++ ) {
            const spmt = allData[i];
            if (_.isEqual("Y", spmt.checkedYn)) {

                const qntt = parseFloat(spmt.shpgotQntt) || 0;
                const wght = parseFloat(spmt.shpgotWght) || 0;
                ...
                for ( let iCheck=0; iCheck<jsonShpgot.length; iCheck++ ) {
                    ...
                    if (_.isEqual(spmt.idxNo, shpgot.idxNo)) {
                        isExist = true;
                        break;
                    }
                }
                ...
                spmt.checkedYn = "N";
                spmt.warehouseSeCd = "";
                spmt.bfrSpmtno = spmt.spmtno;
                spmt.bfrPckgno = spmt.pckgno;
                spmt.bfrPckgSn = spmt.pckgSn;
                spmt.bfrSpmtSn = spmt.spmtSn;

                rowsForAdd.push(spmt);
            }
        }
     ...
     * @function
     */
    const fn_delTrgt = async function() {
        const nCol = grdShpgot.getColRef("checkedYn");
        const checkedRows = grdShpgot.getCheckedRows(nCol);

        for ( let i=checkedRows.length-1; i>=0; i-- ) {
            const nRow = checkedRows[i];
            grdShpgot.deleteRow(nRow);
        }
    }
    ...
    const fn_save = async function(){
        ...
        const trsprtCoCd = SBUxMethod.get("dtl-slt-trsprtCoCd");
        const vhclno = gfn_nvl(SBUxMethod.get("dtl-inp-vhclno"));
        const rmrk = gfn_nvl(SBUxMethod.get("dtl-inp-rmrk"));
        ...
        for ( let i=0; i<allData.length; i++ ) {
            ...
            if (gfn_isEmpty(shpgot.warehouseSeCd)) {
                gfn_comAlert("W0005", "반품창고");
                return;
            }
        ...
        console.log(param);
        try {
            const postJsonPromise = gfn_postJSON("/am/shpgot/insertShpgotRsltGds.do", param);
            const data = await postJsonPromise;
