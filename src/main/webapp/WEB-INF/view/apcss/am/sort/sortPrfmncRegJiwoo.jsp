<%
 /**
  * @Class Name : sortPrfmncRegJiwoo.jsp
  * @Description : 선별실적등록 (지우)
  * @author SI개발부
  * @since 2024.10.28
  * @version 1.0
...
<%--							<td colspan="9" style="border-right: hidden;">&nbsp;</td>--%>
    ...
    <sbux-modal id="modal_normal" name="modal_normal" uitype="middle" header-title="선별일지 발행"
...
<script type="text/javascript">
	...
	const fn_createGrid = function() {
	    ...
	    SBGridProperties.columns = [
	        ...
	        {caption: ["규격"],		ref: 'spcfctCd',    type:'combo',  width : '100px', typeinfo : {ref:'jsonApcSpcfct', label:'label', value:'value', oneclickedit: true},   style:'text-align:center', sortable: false},
	        {caption: ["재고수량"],		ref: 'invntrQntt',      type:'output',  width:'100px',    style:'text-align:center', sortable: false,
	        	dataType : 'number' ,format : {type:'number', rule:'#,###'}
	        },
	        {caption: ["재고중량"],		ref: 'invntrWght',      type:'output',  width:'100px',    style:'text-align:center', sortable: false,
	        	dataType : 'number' ,format : {type:'number', rule:'#,###'}
	        }];

	    jsonApcGrd.forEach(item => {
	    	let column = {   type:'input',  width:'50px',    style:'text-align:center', sortable: false,
	            	format : {type:'number', rule:'#,###'}
	        }
	    	column['caption'] = item.grdNm;
	    	column['ref'] = item.grdCd;
	    	SBGridProperties.columns.push(column);
	    })
	    SBGridProperties.columns.push({caption: ["선별수량"],	ref: 'sortQntt',    type:'output',  width:'50px',    style:'text-align:center', sortable: false,
        	format : {type:'number', rule:'#,###'}
        });
	    SBGridProperties.columns.push({caption: ["선별중량"],	ref: 'sortWght',    type:'output',  width:'50px',    style:'text-align:center', sortable: false,
        	format : {type:'number', rule:'#,###'}
        });

	    SBGridProperties.columns.push({caption: ["단가"],	ref: 'untPrc',    type:'input',  width:'50px',    style:'text-align:center', sortable: false,
        	format : {type:'number', rule:'#,###'}
        });
	    SBGridProperties.columns.push({caption: ["공급가"],	ref: 'splyPrc',    type:'input',  width:'50px',    style:'text-align:center', sortable: false,
        	format : {type:'number', rule:'#,###'}
        });
	    SBGridProperties.columns.push({caption: ["비고"],		ref: 'rmrk',      type:'output',  width:'50px',    style:'text-align:center', sortable: false});
		...
		//"9901","9902", "9901","9906" 품종에 대해서 조회한다음에 그냥 넣으면 될듯
	...
	const fn_grdValueChanged = function(){
		...
		let rowNum = grdRawMtrWrhs.getRow();
		let rowData = grdRawMtrWrhs.getRowData(rowNum);
		let sum = 0;
		for (let i = 1; i <= 10; i++) {
		   let key = i.toString().padStart(2, '0'); // 01, 02, ..., 10 형식의 키 생성
 			sum += parseInt(rowData[key], 10) || 0; // 값이 없거나 null일 경우 0으로 처리
		}
		let spc = jsonApcSpcfct.find(data => data.spcfctCd === rowData['spcfctCd']);
		rowData['sortQntt'] = sum;
		rowData['sortWght'] = sum * parseFloat(spc.wght);
		grdRawMtrWrhs.setRowData(rowNum, rowData, true);
	}
    ...
    const fn_save = async function() {
		...
		allData.pop();
 		allData.forEach(item => {
			...
			item['apcCd'] = gv_apcCd
 			...
 			item['itemCd'] = '1326';
 			item['prdcrCd'] = prdcrCd;
 			item['sortQntt'] = (item['bxQntt'] === "" || item['bxQntt'] === undefined) ? 0 : item['bxQntt'];
 			item['sortWght'] = parseInt(item['bxQntt']) * parseFloat(spc.wght);
 			//item['vhclno'] = vhclno;
    	...
    	allData.forEach(item => {
    		...
    		/* item.forEach((item2,index) => {
    			...
    			sum += gfn_nvl(item2[index]) == "" ? 0 : parseInt(item2[index])
    		}) */

    		for (let i = 1; i <= 10; i++) {
 			   let key = i.toString().padStart(2, '0'); // 01, 02, ..., 10 형식의 키 생성
    			sum += parseInt(item[key], 10) || 0; // 값이 없거나 null일 경우 0으로 처리
			}

    		if(item.invntrQntt * 11  < sum * 10){
    			console.log("재고 보다 크다!");
    		}else{
    			console.log("좋다!");
    			sumData[item.wrhsno] = sum;
    		}
    	})
	...
	/**
     * @name fn_sortDoc
     * @description 선별일지 발행 버튼
     */
	const fn_sortDoc = async function(pltno) {
