...
<body oncontextmenu="return false">
		...
		<div class="box box-solid">
			...
			<div class="box-body">
				...
				<table class="table table-bordered tbl_fixed">
					...
					<tbody>
						...
						<th scope="row" class="th_bg"><span class="data_required" ></span>입고일자</th>
							...
							<th scope="row" class="th_bg"><span ></span>품목/품종</th>
							<td class="td_input" style="border-right: hidden;">
								<sbux-select
									unselected-text="전체"
									uitype="single"
						...
						<tr>
							...
							<td colspan="2">
								<sbux-checkbox
										id="srch-chk-prcsType"
										name="srch-chk-prcsType"
										uitype="normal"
										text="재처리"
				...
				<div class="table-responsive tbl_scroll_sm">
					<div id="sb-area-grdRawMtrInvntr" style="height:308px;"></div>
				</div>
						...
						<tr>
							...
							<td colspan="2" class="td_input" style="border-right: hidden;">
								<sbux-select
									...
									onchange="fn_fcltChange(dtl-slt-fcltCd)"
								></sbux-select>
							</td>
							<td></td>
				...
				<div class="table-responsive tbl_scroll_sm">
					<div id="sb-area-grdSortPrfmnc" style="height:282px;"></div>
				</div>
				<div class="exp-div-excel" style="display: none;width: 1000px;">
					...
					<div id="sbexp-area-grdExpPckgGrd" style="height:1px; width: 100%;"></div>
	...
	<div>
		<sbux-modal
				id="modal-rawMtrPop"
				name="modal-rawMtrPop"
				uitype="middle"
				header-title="생산자 선택"
				body-html-id="body-modal-rawMtrPop"
				header-is-close-button="false"
				footer-is-close-button="false"
				style="width:1000px"
		></sbux-modal>
	</div>
	<div id="body-modal-rawMtrPop">
		<jsp:include page="../../am/popup/rawMtrInvntrRegPopup.jsp"></jsp:include>
	</div>
    <!-- IMPORT 엑셀 등록 Modal -->
    <div>
        <sbux-modal
...
<script type="text/javascript">
	...
	const _REPRCS_CHECK_ 	= "REPRCS_CHECK";
	...
	var jsonInptType = [{text: '수량', value: 'qntt'}, {text: '중량', value: 'wght'}];
	...
	const fn_init = async function() {
		...
		SBUxMethod.set("slt-inptType", "qntt");
		let result = await Promise.all([
		...
		for ( let codeObj of jsonApcAtrb ) {
			if (_.isEqual(codeObj.cdVl, _REPRCS_CHECK_)) {
				if (_.isEqual("Y", codeObj.cdChrVl)) {
					SBUxMethod.set('srch-chk-prcsType', true);
				}
			}
		}
...
*/
	...
	const fn_fcltChange = function(obj){
	...
	const fn_createGridRawMtrInvntr = function() {
		...
		SBGridProperties.columns = [
            ...
            {caption: ["컨테이너<br>팔레트","컨테이너<br>팔레트"],	ref: 'pltno',     		type: 'output',	width:'120px', 	style: 'text-align:center'},
            ...
            {caption: ["투입지시","중량 (Kg)"],	ref: 'cmndWght', 		type: 'output', width:'70px', 	style: 'text-align:right', format : {type:'number', rule:'#,###'}},
            {
				caption: ["선별투입","수량"],
				ref: 'inptQntt',
				type: 'input',
            ...
            {caption: ["선별투입","중량 (Kg)"],		ref: 'inptWght',  		type:'input',  width:'70px', 	style: 'text-align:right;background-color:#FFF8DC;',
	        ...
	        {caption: ["상품구분코드"],	ref: 'gdsSeCd',   		hidden: true},
	        {caption: ["입고구분코드"],	ref: 'wrhsSeCd',   		hidden: true},
	        {caption: ["운송구분코드"],	ref: 'trsprtSeCd', 		hidden: true},
	        {caption: ["설비"],		ref: 'fcltCd', 			hidden: true},
	        {caption: ["등급코드"],		ref: 'grdCd',   		hidden: true},
	        {caption: ["박스종류코드"],	ref: 'bxKnd',   		hidden: true},
	        ...
	        {caption: ["등록필요여부"],	ref: 'needInsertYn', 	hidden: true},
	...
	const fn_createGridSortPrfmncByGrd = function() {
		var SBGridProperties = {};
		SBGridProperties.parentid = 'sb-area-grdSortPrfmnc';
		SBGridProperties.id = 'grdSortPrfmnc';
		SBGridProperties.jsonref = 'jsonSortPrfmnc';
		...
		if (gjsonStdGrdObjKnd.length == 1) {
			const grdKnd = gjsonStdGrdObjKnd[0].grdKnd;
			// 등급별 수량 등록
			const objGrd = gStdGrdObj.getGrdJson(gStdGrdObj.idList[0]);
			objGrd.forEach((grdDtl, iGrd) => {
				const grdQntt = {
					caption: [grdDtl.grdNm],
					ref: gStdGrdObj.colPrfxQntt + grdKnd + grdDtl.grdCd,
					datatype: 'number',
					type:'input',
					width:'60px',
					style: 'text-align:right;background-color:#FFF8DC',
					userattr: {colNm: "stdGrdQntt", colCd: grdDtl.grdCd, grpCd: grdKnd},
					typeinfo: {
		                mask : {alias : '#', repeat: '*', unmaskvalue : true},
		                maxlength: 9,
		                oneclickedit: true
				...
				columnsStdGrd.push(grdQntt);

				const grdWght = {
						caption: ["등급중량(Kg)"],
						ref: gStdGrdObj.colPrfxWght + grdKnd + grdDtl.grdCd,
						datatype: 'number',
						type:'input',
						userattr: {colNm: "stdGrdWght", colCd: grdDtl.grdCd, grpCd: grdKnd},
		                hidden: true
					}

				columnsStdGrd.push(grdWght)
			});
		}
	...
	const fn_setGrdRawMtrInvntr = async function() {
		...
		const chkPrcsType = SBUxMethod.get("srch-chk-prcsType");
		const prcsType = gfn_isEmpty(chkPrcsType) ? "" : (chkPrcsType['srch-chk-prcsType'] ? "RR" : "");
...
//          	await fn_getSpmtPckgUnit(itemCd, vrtyCd);
	...
	const fn_save = async function() {
		...
		if (gjsonStdGrdObjKnd.length > 1) {
  			gfn_comAlert("W0009", "둘 이상의 등급종류");		//	W0009	{0}이/가 있습니다.
            return;
		}
		...
		allInvntrData.forEach((item, index) => {
			if (item.checkedYn === "Y") {
				...
				let needInsertYn = item['needInsertYn'];

				if (_.isEqual("Y", needInsertYn)) {
					rawMtr.needInsertYn = item.needInsertYn;
					rawMtr.wrhsYmd = item.wrhsYmd;
					rawMtr.prdcrCd = item.prdcrCd;
					rawMtr.itemCd = item.itemCd;
					rawMtr.vrtyCd = item.vrtyCd;
					rawMtr.warehouseSeCd = item.warehouseSeCd;
					rawMtr.bxKnd = item.bxKnd;
					rawMtr.pltno = item.pltno;
					rawMtr.gdsSeCd = item.gdsSeCd;
					rawMtr.wrhsSeCd = item.wrhsSeCd;
					rawMtr.trsprtSeCd = item.trsprtSeCd;
					rawMtr.invntrQntt = item.invntrQntt;
					rawMtr.invntrWght = item.invntrWght;
					rawMtr.grdCd = item.grdCd;
					rawMtr.stdGrdList = item.stdGrdList;
				}

				rawMtrInvntrList.push(rawMtr);
		...
		for ( let i=0; i<allSortData.length-1; i++ ) {
			...
			const rowData = allSortData[i];

			const inptYmd= rowData.inptYmd;
			const fcltCd = rowData.fcltCd;
		...
		allSortData.every((item, index) => {
			if (!gfn_isEmpty(item.inptYmd)) {
				...
				const spmtPckgUnitCd = item.spmtPckgUnitCd;

				const objGrd = gStdGrdObj.getGrdJson(gStdGrdObj.idList[0]);
				if (gjsonStdGrdObjKnd.length == 1 && objGrd.length > 0) {

					const grdKnd = gjsonStdGrdObjKnd[0].grdKnd;

					const objGrd = gStdGrdObj.getGrdJson(gStdGrdObj.idList[0]);

					objGrd.every((grdDtl, iGrd) => {
						const jgmtGrdCd = grdDtl.grdCd;
						...
						const qnttColId = gStdGrdObj.colPrfxQntt + grdKnd + grdDtl.grdCd;
						const wghtColId = gStdGrdObj.colPrfxWght + grdKnd + grdDtl.grdCd;
						const qntt = parseInt(item[qnttColId]) || 0;
						const wght = parseFloat(item[wghtColId]) || 0;
	...
	const fn_addRow = async function(nRow, nCol) {
		...
		grdSortPrfmnc.setCellDisabled(nRow, 0, nRow, 5, true);
		...
		if (!gfn_isEmpty(usrAttr) && usrAttr.hasOwnProperty('colNm')) {
			if (nCol == grdRawMtrInvntr.getColRef("checkedYn")) {
				...
				if (inptQntt === 0 && inptWght === 0) {
					...
					const chkCol = grdRawMtrInvntr.getColRef("checkedYn");
					const checkedRows = grdRawMtrInvntr.getCheckedRows(chkCol, true);

			    	const allData = grdRawMtrInvntr.getGridDataAll();
			    	if(checkedRows.length == allData.length){
						allData.checked = true;
			    	}
				}
			}
			grdRawMtrInvntr.refresh({"focus":false});
			fn_setInptInfo();
		}
 	}
			...
			if (usrAttr.colNm == "inptQntt") {
				...
				if (tmpInptQntt <= 0) {
					...
					if (tmpInptQntt > cmndQntt && cmndQntt > 0) {
						...
						if (wrhsQntt > 0) {
							rowData.inptWght = gfn_apcEstmtWght(wrhsWght * tmpInptQntt / wrhsQntt, gv_selectedApcCd);
						} else {
							rowData.inptWght = gfn_apcEstmtWght(invntrWght * tmpInptQntt / invntrQntt, gv_selectedApcCd);
						}
		...
		if (!gfn_isEmpty(usrAttr) && usrAttr.hasOwnProperty('colNm')) {
			...
			case "spmtPckgUnitCd":
				...
				if (!gfn_isEmpty(rowData.spmtPckgUnitCd)) {
					...
					let chkInfo = _.find(jsonSpmtPckgUnit, {spmtPckgUnitCd: rowData.spmtPckgUnitCd});
					...
					if (gfn_isEmpty(rowData.itemCd)) {
						gfn_comAlert("W0001", "품목");		//	W0002	{0}을/를 선택하세요.
					} else if (gfn_isEmpty(rowData.vrtyCd)) {
						gfn_comAlert("W0001", "품종");		//	W0002	{0}을/를 선택하세요.
					} else {
						hasError = false;
					}
					...
					grdSortPrfmnc.refresh({"focus":false});
					/*
					if (	_.isEqual(rowData.itemCd, chkInfo.itemCd)
							&& _.isEqual(rowData.vrtyCd, chkInfo.vrtyCd)
							&& _.isEqual(rowData.spcfctCd, chkInfo.spcfctCd)
							) {
						// ok
					} else {
						rowData.spmtPckgUnitCd = "";
						const spmt = _.filter(
								jsonSpmtPckgUnit,
								{itemCd: rowData.itemCd, vrtyCd: rowData.vrtyCd, spcfctCd: rowData.spcfctCd,}
							);
						if (gfn_isEmpty(spmt)) {
							gfn_comAlert("W0004", "등록가능");
						} else {
							let msg = "가능한 포장단위: \n";
							spmt.forEach((spmtUnit, idx) => {
								if (idx === 0) {
									msg += "\n [ ";
								} else {

								}
								msg += spmtUnit.spmtPckgUnitNm + " ]";
							});
							gfn_comAlert("E0000", msg);
							return;
						}
					}
					 */
				...
				case "stdGrdQntt":	// 등급별 수량
					spcfctWght = parseFloat(rowData.spcfctWght) || 0;
					const objGrd = gStdGrdObj.getGrdJson(gStdGrdObj.idList[0]);
					if (gjsonStdGrdObjKnd.length > 0 && objGrd.length > 0) {
						const grdKnd = gjsonStdGrdObjKnd[0].grdKnd;
						let totQntt = 0;
						//let totWght = 0;
						objGrd.forEach((grdDtl, iGrd) => {
							const qnttColId = gStdGrdObj.colPrfxQntt + grdKnd + grdDtl.grdCd;
							const wghtColId = gStdGrdObj.colPrfxWght + grdKnd + grdDtl.grdCd;
							const qntt = parseInt(rowData[qnttColId]) || 0;
							const wght = qntt * spcfctWght;
							totQntt += qntt;
							//totWght += wght;
							rowData[wghtColId] = wght;
						});
...
// 	}
	...
	var jsonExpSltSpmtPckgUnit		= [];	// 출하포장단위
	...
	var jsonExpSltSpmtPckgUnit		= [];	// 출하포장단위
	var jsonExpPckgGrd 				= [];	// 엑셀 포장등급Json
	...
	const fn_getExpColumns = function() {
		...
		if (_.isEqual("wght", SBUxMethod.get("slt-inptType"))) {
			_inpt.caption = ["총 투입중량(Kg)"];
			_inpt.ref = "inptWght";
		}
	...
	const fn_setSltJson = async function() {
		...
		jsonExpSltSpmtPckgUnit 		= gfn_cloneJson(jsonExeclComSpmtPckgUnit);	// 엑셀 포장구분Json

		jsonExpSltItem				= gfn_getJsonFilter(jsonExpSltItem, "itemCd", itemCd);
		jsonExpSltVrty				= gfn_getJsonFilter(jsonExpSltVrty, "itemCd", itemCd);
		jsonExpSltSpcfct			= gfn_getJsonFilter(jsonExpSltSpcfct, "itemCd", itemCd);
		jsonExpSltSpmtPckgUnit		= gfn_getJsonFilter(jsonExpSltSpmtPckgUnit, "itemCd", itemCd);
	}
	...
	const fn_setExpGrdJson = function() {
	    ...
	    jsonExpPckgGrd.length = 0;
	    gjsonExtGrdObjKnd.forEach((item, index) => {
	    	const jsonTemp = gStdGrdObj.getExtJson(gStdGrdObj.idList[index]);
	    	jsonTemp.forEach((stdGrd) => {
	    		jsonExpPckgGrd.push({
					grdKnd: item.grdKnd,
		    		grdKndNm: item.grdKndNm,
	    ...
	    if (gjsonExtGrdObjKnd.length > 1 && gjsonExtGrdObjJgmt.length > 0) {
	    	...
	    	gjsonExtGrdObjJgmt.forEach((stdGrd) => {
	    		jsonExpPckgGrd.push({
					grdKnd: "jgmt",
		    		grdKndNm: "판정등급",
		    		grdCd: stdGrd.grdCd,
	...
	const fn_dwnld = async function(){
		...
		const expObjList = [
			{
				...
				columns: [
			    	...
			    	{caption: ['대표품목명'],	ref: 'rprsItemNm', 	type: 'output', width: '100px', style: 'text-align:center'},
			        {caption: ['대표품종'], 	ref: 'rprsVrtyCd', 	type: 'output', width: '80px', style: 'text-align:center'},
			    	{caption: ['대표품종명'], 	ref: 'rprsVrtyNm', 	type: 'output', width: '100px', style: 'text-align:center'}
				],
		        sheetName: "생산자",
		        title: "",
		        unit: ""
		        ...
		        sbGrid: grdExpGdsGrd,
		        parentid: "sbexp-area-grdExpPckgGrd",
		        id: "grdExpPckgGrd",
		        jsonref: "jsonExpPckgGrd",
				columns: [
					{caption: ["등급종류"],	ref: 'grdKndNm',  type:'output',  width:'100px',    style:'text-align:center'},
					{caption: ["등급코드"],	ref: 'grdCd',  type:'output',  width:'100px',    style:'text-align:center'},
					{caption: ["등급명"],	ref: 'grdNm',  type:'output',  width:'100px',    style:'text-align:center'},
		...
		let fileName = "선별실적(샘플)_" + itemNm + ".xlsx"
	...
	const fn_importSortPrfmnc = async function(_grdImp) {
		...
		if (impData.length == 0) {
 		...
 		for ( let iRow = 1; iRow <= impData.length; iRow++ ) {
 	    	...
 	    	*/
 	    	...
 	    	let sortQntt = parseFloat(rowData.sortQntt) || 0;
 	    	let sortWght = parseFloat(rowData.sortWght) || 0;
 	    	let inptQntt = parseFloat(rowData.inptQntt) || 0;
 	    	let inptWght = parseFloat(rowData.inptWght) || 0;

 	    	if (inptWght > 0 && sortWght > inptWght) {
 	    		gfn_comAlert("W0015", "투입중량", "선별중량");		//	W0015	{0}이/가 {1} 보다 작습니다.
 	            return;
 	    	}
 	    	...
 	    	if (_.isEqual(rowData.checkedYn, "Y")) {
 	    		if (gfn_isEmpty(rowData.spmtPckgUnitCd)) {
 	 	    		gfn_comAlert("W0001", "포장상품명");		//	//	W0002	{0}을/를 입력하세요.
 	 	            return;
 	 	    	}
 	    	}
	    	...
	    	if (_.isEqual(rowData.checkedYn, "Y")) {
				...
				gjsonExtGrdObjKnd.forEach((knd, idx) => {

					const gdsColId = gStdGrdObj.colExtPrfx + knd.grdKnd;
					let stdGrd = {
						grdSeCd: _GRD_SE_CD_GDS,
						itemCd: rowData.itemCd,
						grdKnd: knd.grdKnd,
						grdCd: rowData[gdsColId]
					}

					if (idx === 0) {
						gdsJgmtGrdCd = rowData[gdsColId];
					}

					gdsStdGrdList.push(stdGrd);
				});

				if (gjsonExtGrdObjKnd.length > 1 && gjsonExtGrdObjJgmt.length > 0) {
					gdsJgmtGrdCd = rowData['gdsJgmtGrdCd'];
				}
			...
			if (hasError) {
 	    	...
 	    	const sortPrfmnc = {
				...
				inptYmd: rowData.sortYmd,
   				...
   				inptWght: inptWght,
				stdGrdList: stdGrdList,
				autoPckgInptYn: rowData.checkedYn,
    	...
    	let postUrl = "/am/sort/insertImportSortRslt.do";
		...
		for ( let iRow = 1; iRow <= impData.length; iRow++ ) {
			...
			if (!gfn_isEmpty(rowData.warehouseSeCdFrom)) {
				...
				if (gfn_isEmpty(chkInfo)) {
					...
					if (!gfn_isEmpty(chkInfo)) {
						...
						// 없는 원물창고 이므로 원물창고 clear
						rowData.warehouseSeCdFrom = "";
					}
				}
			}
			...
			if (!gfn_isEmpty(rowData.sortFcltCd)) {
				...
				if (gfn_isEmpty(chkInfo)) {
					...
					if (!gfn_isEmpty(chkInfo)) {
						rowData.sortFcltCd = chkInfo.value;
					} else {
						// 없는 선별기 이므로 선별기 clear
						rowData.sortFcltCd = "";
					}
				}
			}
			...
			if (!gfn_isEmpty(rowData.spmtPckgUnitCd)) {
				...
				if (gfn_isEmpty(chkInfo)) {
					...
					if (!gfn_isEmpty(chkInfo)) {
						if (
								!_.isEqual(rowData.itemCd, chkInfo.itemCd)
								||
								!_.isEqual(rowData.vrtyCd, chkInfo.vrtyCd)
							) {
							rowData.spmtPckgUnitCd = "";
			...
			gjsonExtGrdObjKnd.forEach((item, index) => {
				...
				gdsGrdList.push(rowData[colNm]);
			});
		}

		_grdImp.refresh({"focus":false});
     }
	...
	/**
	 * @name fn_AddRawMtr
	 * @description 원물재고 행추가
	 * @function
	 */
	const fn_AddRawMtr = async function() {

		const itemCd = SBUxMethod.get("srch-slt-itemCd");
		const itemVrtyCd = SBUxMethod.get("srch-slt-vrtyCd");
		...
		const param = {
			...
			itemCd: itemCd,
			itemVrtyCd: itemVrtyCd,
		}
		popRawMtr.init(param, fn_setAddedRow);
	}

	/**
	 * @name fn_setAddedRow
	 * @description 원물재고 추가 데이터 선택 후 행추가
	 * @param _addedRowDatas {Array}
	 */
	const fn_setAddedRow = function(_addedRowDataArray) {

		if (!Array.isArray(_addedRowDataArray) || _addedRowDataArray.length == 0) {
			return;
		}

		_addedRowDataArray.forEach((_rowData) => {
			let qntt = _rowData['qntt'];
			let wght = _rowData['wght'];
			_rowData['needInsertYn'] = "Y";
			_rowData['invntrQntt'] = qntt;
			_rowData['invntrWght'] = wght;
			_rowData['inptQntt'] = qntt;
			_rowData['inptWght'] = wght;

			grdRawMtrInvntr.addRow(false, _rowData);
		});
	...
	/**
	 * @name fn_delRawMtr
	 * @description 원물재고 행 삭제
	 * @function
	 */
	const fn_delRawMtr = function() {
		...
		const rows = grdRawMtrInvntr.getCheckedRows(chkCol);
		rows.reverse().forEach((row) => {
			const rowSts = grdRawMtrInvntr.getRowStatus(row);
			if (_.isEqual(1, rowSts)) {
				grdRawMtrInvntr.deleteRow(row);
			}
		});
	}
