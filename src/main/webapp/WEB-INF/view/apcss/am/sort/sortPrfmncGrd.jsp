		...
		<div class="box box-solid">
			<div class="box-header" style="display:flex; justify-content: flex-start;" >
				<div>
					...
					<h3 class="box-title"> ▶ <c:out value='${menuNm}'></c:out></h3><!-- 선별실적조회(등급별) -->
				</div>
...
<script type="text/javascript">
    ...
    const fn_createGrid = function() {
        ...
        SBGridProperties.columns = [
            ...
            {caption: ["등급1","등급명"],  			ref: 'grdNm1', 				type:'output',  	width:'80px',   style:'text-align:center'},
            {caption: ["등급1","수량"],  			ref: 'sortQntt1', 			type:'output',  	width:'80px',   style:'text-align:right',
            ...
            {caption: ["총 선별","수량"],  			ref: 'sortQntt', 			type:'output',  	width:'60px',   style:'text-align:right',
            	format : {type:'number', rule:'#,###'}
            },
	...
	const fn_setGrdSortPrfmnc = async function(pageSize, pageNo) {
		...
		const postJsonPromise = gfn_postJSON("/am/sort/selectSortPrfmncGrdList.do", {
  				...
  				for(var i=1; i<=11; i++){
  	          	...
  	          	data.resultList.forEach((item, index) => {
  	          		const sortPrfmnc = {
  	 					  ...
  	 					  , grdCd9			: item.grdCd9
  	 					  , grdNm9			: item.grdNm9
  	 					  , grdCd10			: item.grdCd10
  	 					  , grdNm10			: item.grdNm10
  	 					  , grdCd11			: item.grdCd11
  	 					  , grdNm11			: item.grdNm11
    ...
    const fn_grdItemCdReCreate = async function () {
	    ...
	    jsonApcGrdDtl.forEach((item, index) => {
	    	var captionId = item.grdNm;
	    	var refSortQnttId = "sortQntt" + i;
	    	var refSortWghtId = "sortWght" + i;

	    	columnsList.push(
	    			{caption: [captionId,"수량"],  				ref: refSortQnttId, 			type:'output',  	width:'80px',   style:'text-align:right',
	                	format : {type:'number', rule:'#,###'}
	                },
	                {caption: [captionId,"중량 (KG)"],  		ref: refSortWghtId, 			type:'output',  	width:'80px',  style:'text-align:right',
	                	format : {type:'number', rule:'#,###'}
	                },
	    	)
	    	i++;
    	...
    	SBGridProperties.columns = columnsList;
        grdSortPrfmnc = _SBGrid.create(SBGridProperties);
        grdSortPrfmnc.bind('click', fn_viewSortInpt);
        grdSortPrfmnc.bind('beforepagechanged', fn_pagingSortPrfmnc);
    }
	...
	const fn_selectItem = async function (obj){
		...
		if (gfn_isEmpty(itemCd)) {
			...
			let result = await Promise.all([
				...
				gfn_setApcGdsGrdSBSelect('srch-slt-grdDtl', 	jsonApcGrdDtl, 	gv_selectedApcCd, itemCd, "02"),	// 등급상세
				gfn_setApcVrtySBSelect('srch-slt-vrtyCd', 		jsonApcVrty, 	gv_selectedApcCd, itemCd),			// 품종
			]);
		}

		fn_grdItemCdReCreate();
	}
	...
	const fn_onChangeSrchVrtyCd = async function(obj){
		...
		if(!gfn_isEmpty(vrtyCd)){
			...
			if (itemCd != prvItemCd) {
				...
				await fn_selectItem({value: itemCd});
