				...
				<table class="table table-bordered tbl_fixed">
					...
					<tbody>
						...
						<tr>
							...
							<td colspan="7" class="td_input" style="border-right: hidden;border-bottom: hidden">
...
*/
	...
	const fn_createGridRawMtrInvntr = function() {
		...
		SBGridProperties.columns = [
            ...
            {caption: ["선별투입","수량"],			ref: 'inptQntt',  		type: 'input',  width:'70px', 	style: 'text-align:right;background-color:#FFF8DC;',
		...
		const columns2 = [
            ...
            },
            {
            	...
            	typeinfo: {
					...
					filtering: {usemode : true, uppercol : 5, attrname : 'spcfctCd', listall:true},
            		displayui : false,
            		oneclickedit: true,
            	}
            },
	...
	const fn_save = async function() {
        ...
        allSortData.forEach((item, index) => {
            if (!gfn_isEmpty(item.inptYmd)) {
                ...
                if (_.isEqual(autoPckgInptYn, "Y")) {
                    ...
                    if (gjsonExtGrdObjKnd.length > 1 && gjsonExtGrdObjJgmt.length > 0) {
                        gdsJgmtGrdCd = item['gdsJgmtGrdCd'];
                    }
        ...
        }else if(!gfn_comConfirm("Q0001", "선별실적 저장")){
            return;
        }
	...
	const fn_addRow = async function(nRow, nCol) {
		...
		let idx = grdSortPrfmnc.getColRef("checkedYn");
		grdSortPrfmnc.setCellData(nRow, idx, 'Y');
		...
		grdSortPrfmnc.setColDisabled(idx,true);
		grdSortPrfmnc.setCellDisabled(nRow, 0, nRow, 5, true);
		grdSortPrfmnc.setCellDisabled(nRow, 5, nRow, grdSortPrfmnc.getCols() - 1, false);
		// grdSortPrfmnc.setCellDisabled(nRow,idx,nRow,idx,true);
		nRow++;
		grdSortPrfmnc.addRow(true);
		grdSortPrfmnc.setCellDisabled(nRow, 0, nRow, grdSortPrfmnc.getCols() - 1, true);

		// grdSortPrfmnc.setCellDisabled(nRow-1,idx,nRow-1,idx,true);
	...
	const fn_grdSortPrfmncValueChanged = function() {
		...
		if (!gfn_isEmpty(usrAttr) && usrAttr.hasOwnProperty('colNm')) {
			...
			case "spmtPckgUnitCd":
					...
					/** 상품명에 연관된 규격코드 세팅부분 주석처리 [신미네 요구사항 ] 2024.08.08_MS **/
				...
				case "spcfctCd":	// 규격
					...
					let spcfVal = grdSortPrfmnc.getCellData(nRow, nCol);
					let spcfIdx = grdSortPrfmnc.getColRef('spmtPckgUnitCd');

					let spcfArr = grdSortPrfmnc.getColumnComboData(spcfIdx).filter((item) => item.spcfctCd == spcfVal);
					if(spcfArr.length == 0 || spcfArr == null){
						gfn_comAlert("Q0000","해당 규격에 맞는 상품명을 등록해주세요.");
						grdSortPrfmnc.setCellData(nRow, nCol,'');
						break;
					}
				...
				case "sortQntt":
					...
					break;
				case "stdGrd":
					/** 지금 바뀐 등급에 ref를 찾아서 ex) 01,02
					 *  뒤쪽에 상품등급 앞에 prefix 찾아서 ref붙여서 그값으로
					 *  그걸로 index찾아서 setCellData해야함. **/
					let val = grdSortPrfmnc.getCellData(nRow, nCol);
					let refWord = grdSortPrfmnc.getRefOfCol(nCol);

					let refNm = gStdGrdObj.colExtPrfx + refWord;
					let idx = grdSortPrfmnc.getColRef(refNm);

					grdSortPrfmnc.setCellDisabled(nRow,idx,nRow,idx,true);

					let ref = grdSortPrfmnc.getColumnTypeInfo(idx).ref;
					let arr = window[ref].filter((item) => item.grdCd == val);

					grdSortPrfmnc.setCellData(nRow,idx,arr[0].aftrGrdCd);
					if(gfn_isEmpty(arr[0].aftrGrdCd)){
						grdSortPrfmnc.setCellDisabled(nRow,idx,nRow,idx,false);
					}
