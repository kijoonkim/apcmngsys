			...
			<div class="box-body">
				...
				<div class="tab-content">
					...
					<div id="tab_sortTot">
						...
						<div id="sb-area-sortTot" style="height: 500px;"></div>
					</div>
...
<script type="text/javascript">
		...
		if(!gfn_isEmpty(colList)) {
			colList.forEach(item => {
				let col = {
					...
					ref : gStdGrdObj.colPrfxQntt + item.vrtyCd, // qntt_vrtyCd
					type : 'input',
					width : `${'${calc}%'}`,
					style: 'text-align:center; font-size:15px; background-color: #FFF8DC;',
					fixedstyle: 'font-size:20px;font-weight:bold',
	...
	/**
	 * @name fn_createTotGird
	 * @description 집계탭 그리드
	 */
	const fn_createTotGird = function() {
		var SBGridProperties = {};
		...
		SBGridProperties.emptyrecords = '데이터가 없습니다.';
		SBGridProperties.columns = [
			 {caption: ["등급/품종","등급/품종"],	ref: 'grdVrty',	type:'output',  width:'10%', style: 'text-align:center; font-size:15px;', fixedstyle: 'font-size:20px;font-weight:bold'},
		];
		...
		SBGridProperties.columns.push(
				{caption: ["합계","수량"],	ref: 'qntt',	type:'output',  width:'5%', style: 'text-align:center; font-size:15px;', fixedstyle: 'font-size:20px;font-weight:bold'},
				{caption: ["합계","중량"],	ref: 'wght',	type:'output',  width:'5%', style: 'text-align:center; font-size:15px;', fixedstyle: 'font-size:20px;font-weight:bold'},
		);
	...
	const fn_clickTab = function() {
		...
		if (_.isEqual(tabModel,"tab_sortTot")) { // tabModel이 집계라면
	...
	function fn_calcQnttSum (objGrid,nRow, nCol) {
		let rowData = objGrid.getRowData(Number(nRow));

		let sumList =  [];
		for (let key in rowData) {
			if (key !== "qntt") {
				sumList.push(rowData[key]);
			}
		}
		let sum = 0;
		for(let i =0; i<sumList.length ; i++){
			sum += Number(sumList[i]);
		}
		return sum.toString();
	}

	function fn_calcSumQnttForSort (objGrid,nRow, nCol) {
		let rowData = objGrid.getRowData(Number(nRow));
		let sumList = [];
		for (let key in rowData) {
			if (key.startsWith('qntt__')) {
				sumList.push(rowData[key]);
			}
		}
		let sum = 0;
	...
	const fn_save = async function () {
		...
		for (let i = 0; i < getGrids.length; i++){
			...
			if (!_.isEqual(gridId,"gridTot") && !_.isEqual(gridId,"gridSpcfct")) {
				let getGridDataAll = window[getGrids[i]].getGridDataAll();
								...
								Object.keys(rowData).forEach(key =>{
									if (key.startsWith(gStdGrdObj.colPrfxQntt)) { // "qntt__"
										...
										for (let i = 0; i < spcfctGridDataAll.length; i++) {
											if (spcfctGridDataAll[i].grdCd === grdCd) {
												wght = parseFloat(spcfctGridDataAll[i].spcfctNm);
												break;
											}
										}
										if (qntt > 0) {
		...
		console.log("저장리스트",sortPrfmncList);
		...
		try {
			const postJsonPromise = gfn_postJSON("", sortMng);
