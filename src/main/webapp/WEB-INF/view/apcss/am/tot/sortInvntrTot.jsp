		...
		<div class="box box-solid">
			<div class="box-header" style="display:flex; justify-content: flex-start;" >
				<div>
					...
					<h3 class="box-title"> ▶ <c:out value='${menuNm}'></c:out></h3><!-- 선별재고집계 -->
				</div>
...
<script type="text/javascript">
	...
	const fn_initSBSelect = async function() {
		let result = await Promise.all([
			...
			await fn_totCrtrInfoList("SI","TOT_TERM_KND")
        ]);
    ...
    const fn_search = async function() {
		...
		let vrtyCd = SBUxMethod.get("srch-slt-vrtyCd");
		 ...
		 const postJsonPromise = gfn_postJSON("/am/tot/selectSortInvntrTotInfo.do", {
	...
	const fn_doc = async function() {
		...
		conn.push({data:data})
 		...
 		gfn_popClipReportPost("선별재고집계", "am/rawMtrInvntrTot.crf",null, conn);
 	}
