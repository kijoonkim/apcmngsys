...
<script>
    ...
    const fn_createAsstCrtrList = function(){
        ...
        SBGridProperties.columns = [
                ...
                {caption: ['입고수량'], ref: 'wrhsQntt', width: '50px', type: 'input', style:'text-align:center'},
                {caption: ['입고일자'], ref: 'wrhsYmd', width: '100px', type : 'datepicker', typeinfo : {displayui: true}, style:'text-align:center'},
    ...
    const fn_asstSave = async function(){
        ...
        try{
			...
			filterData.forEach(item => {
				let qntt = item.wrhsQntt;
				let wrhsYmd = item.wrhsYmd;

				if(gfn_nvl(qntt) === "" || gfn_nvl(wrhsYmd) === "" ){
					chk = false;
					return;
				}
				...
				item['wrhsWght'] = parseInt(item['wrhsQntt']) * parseInt(item['unitWght']);
				item['wrhsYmd'] = item['wrhsYmd'].replaceAll("-","");
				item['wrhsYmdFrom'] = SBUxMethod.get("srch-dtp-wrhsYmd_from");
				item['wrhsYmdTo'] = SBUxMethod.get("srch-dtp-wrhsYmd_to");
			})
