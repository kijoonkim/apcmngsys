...
<%
/**
 * @Class Name : asstMtrWrhsReg.jsp
 * @Description : 부자재 입고등록
 * @author SI개발부
 * @since 2024.09.03
 * @version 1.0
					...
					<table class="table table-bordered tbl_fixed">
						...
						<tbody>
							<tr>
								...
								<th scope="row" class="th_bg">사용공정</th>
								<td colspan="2" class="td_input" style="border-right: hidden;">
									<sbux-checkbox id="srch-chk-useProcs" name="srch-chk-useProcs" uitype="normal" jsondata-ref="jsonUseProcs" ></sbux-checkbox>
								</td>
							</tr>
							<tr>
								<th scope="row" class="th_bg">규격/모델</th>
								<td class="td_input" style="border-right: hidden;"><sbux-select
							...
							<tr>
								<th scope="row" class="th_bg">공급처</th>
								<td class="td_input" style="border-right: hidden;"><sbux-select
										id="srch-slt-splr" name="srch-slt-splr"
										uitype="single" class="form-control input-sm"
										unselected-text="전체" jsondata-ref="jsonSplr"></sbux-select>
								</td>
								<th scope="row" class="th_bg">단위</th>
								<td colspan="2" class="td_input" style="border-right: hidden;"><sbux-select
										id="srch-slt-unit" name="srch-slt-unit"
										uitype="single" class="form-control input-sm"
										unselected-text="전체" jsondata-ref="jsonUnit"></sbux-select>
								</td>
							</tr>
							<tr>
								...
								<th scope="row" class="th_bg">유통기한</th>
								<td colspan="2" class="td_input" style="border-right: hidden;">
								<sbux-datepicker
									id="srch-dtp-rtlTerm"
									name="srch-dtp-rtlTerm"
									uitype="popup"
									date-format="yyyy-mm-dd"
									class="form-control input-sm sbux-pik-group-apc input-sm-ast inpt_data_reqed"
								></sbux-datepicker>
...
<script>
	...
	/** 사용공정 chk **/
	var jsonUseProcs =  [
			{ text : "입고", truevalue:"Y", falsevalue:"N" },
			{ text : "선별", truevalue:"Y", falsevalue:"N" },
			{ text : "포장", truevalue:"Y", falsevalue:"N" },
			{ text : "출하", truevalue:"Y", falsevalue:"N" },
			{ text : "저장", truevalue:"Y", falsevalue:"N" }
   		];
    ...
    const fn_create2 = function(){
        var SBGridProperties = {};
        SBGridProperties.parentid = 'sb-area-grdAsstMtrWrhs';
        SBGridProperties.id = 'grdAsstMtrWrhs';
        SBGridProperties.jsonref = 'jsonAsstMtrWrhs';
        SBGridProperties.emptyrecords = '데이터가 없습니다.';
        SBGridProperties.columns = [
            {caption: [""], 		ref: 'itemCd', 		type:'button', width:'5%', style: 'text-align:center',
                renderer: function(objGrid, nRow, nCol, strValue, objRowData) {
                    if (gfn_isEmpty(objRowData.delYn)){
                        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_addRow(grdAsstMtrWrhs," + nRow + ", " + nCol + ")'>추가</button>";
                    } else {
                        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_delRow(grdAsstMtrWrhs," + nRow + ")'>삭제</button>";
                    }
                }},
                {caption: ['부자재유형'], ref: 'asstMtrCrtrType', width: '20%', type: 'input', style:'text-align:center'},
                {caption: ['수량'], ref: 'asstMtrCrtrTypeNm', width: '20%', type: 'input', format : {type:'number', rule:'#,###'}, style:'text-align:center'},
        ...
        grdAsstMtrWrhs = _SBGrid.create(SBGridProperties);
    }

    /** 부자재 출고목록 grid create **/
    const fn_create3 = function(){
        var SBGridProperties = {};
        SBGridProperties.parentid = 'sb-area-grdAsstMtrSpmt';
        SBGridProperties.id = 'grdAsstMtrSpmt';
        SBGridProperties.jsonref = 'jsonAsstMtrSpmt';
        SBGridProperties.emptyrecords = '데이터가 없습니다.';
        SBGridProperties.columns = [
            {caption: [""], 		ref: 'itemCd', 		type:'button', width:'5%', style: 'text-align:center',
                renderer: function(objGrid, nRow, nCol, strValue, objRowData) {
                    if (gfn_isEmpty(objRowData.delYn)){
                        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_addRow(grdAsstMtrSpmt," + nRow + ", " + nCol + ")'>추가</button>";
                    } else {
                        return "<button type='button' class='btn btn-xs btn-outline-danger' onClick='fn_delRow(grdAsstMtrSpmt," + nRow + ")'>삭제</button>";
                    }
                }},
                {caption: ['부자재유형'], ref: 'asstMtrCrtrType', width: '20%', type: 'input', style:'text-align:center'},
                {caption: ['수량'], ref: 'asstMtrCrtrTypeNm', width: '20%', type: 'input', format : {type:'number', rule:'#,###'}, style:'text-align:center'},
        ...
        grdAsstMtrSpmt = _SBGrid.create(SBGridProperties);
    }
