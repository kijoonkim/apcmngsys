<%
 /**
  * @Class Name : regSpmtCmnd.jsp
  * @Description : 출하지시등록 화면
  * @author SI개발부
  * @since 2023.08.31
  * @version 1.0
		...
		<div class="box box-solid">
			<div class="box-header" style="display:flex; justify-content: flex-start;" >
				<div>
					...
					<h3 class="box-title"> ▶ <c:out value='${menuNm}'></c:out></h3><!-- 출하지시등록 -->
				</div>
					...
					<tbody>
						<tr>
							...
							<td class="td_input"  style="border-right: hidden;">
								<sbux-datepicker id="srch-dtp-outordrYmdFrom" name="srch-dtp-outordrYmdFrom" uitype="popup" date-format="yyyy-mm-dd" class="form-control input-sm sbux-pik-group-apc input-sm-ast inpt_data_reqed" onchange="fn_dtpChange(srch-dtp-outordrYmdFrom)"></sbux-datepicker>
							</td>
							<td class="td_input"  style="border-right: hidden;">
								<sbux-datepicker id="srch-dtp-outordrYmdTo" name="srch-dtp-outordrYmdTo" uitype="popup" date-format="yyyy-mm-dd" class="form-control input-sm sbux-pik-group-apc input-sm-ast inpt_data_reqed" onchange="fn_dtpChange(srch-dtp-outordrYmdTo)"></sbux-datepicker>
							</td>
						...
						<tr>
							...
							<td class="td_input" style="border-right: hidden;">
								<sbux-button id="btn-srch-cnpt" name="btn-srch-cnpt" uitype="modal" target-id="modal-cnpt" onclick="fn_modalCnpt" text="찾기" class="btn btn-xs btn-outline-dark"></sbux-button>
							</td>
...
<script type="text/javascript">
	...
	const fn_onChangeSrchItemCd = async function(obj) {
		...
		if (gfn_isEmpty(itemCd)) {
			gfn_setApcSpcfctsSBSelect('srch-slt-spcfctCd', jsonComSpcfct, gv_selectedApcCd, "");
		} else {
	...
	const fn_onChangeSrchVrtyCd = async function(obj) {
		...
		if(!gfn_isEmpty(itemCd)){			
			let rst = await Promise.all([
				await gfn_setSpmtPckgUnitSBSelect('grdSpmtCmndTrg', 	jsonSpmtPckgUnit, 	gv_selectedApcCd, itemCd, vrtyCd.substring(4,8)),	// 포장구분	(그리드)
	 			await gfn_setApcGdsGrdSBSelect('grdSpmtCmndTrg', 		jsonComGdsGrd, 		gv_selectedApcCd, itemCd, '03')		// 상품등급(재고그리드)
	 		]);
			console.log('jsonSpmtPckgUnit', jsonSpmtPckgUnit);
 			grdSpmtCmndTrg.refresh({"combo":true, "focus":false});
		}else{
			let rst = await Promise.all([
				await gfn_setSpmtPckgUnitSBSelect('grdSpmtCmndTrg', 	jsonSpmtPckgUnit, 	gv_selectedApcCd, "", ""),	// 포장구분	(그리드)
	 			await gfn_setApcGdsGrdSBSelect('grdSpmtCmndTrg', 		jsonComGdsGrd, 		gv_selectedApcCd, "", "")		// 상품등급(재고그리드)
	 		]);
			console.log('jsonSpmtPckgUnit', jsonSpmtPckgUnit);
			grdSpmtCmndTrg.refresh({"combo":true, "focus":false});
		}
