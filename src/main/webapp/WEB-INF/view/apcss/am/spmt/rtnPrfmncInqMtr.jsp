                ...
                <tr>
                    ...
                    <td class="td_input" colspan="3" style="border-right: hidden;border-top: hidden">
                        <div style="display: flex;justify-content: center;align-items: center">
                            <sbux-datepicker
                                    uitype="popup"
                                    id="srch-dtp-shpgotYmdFrom"
                                    name="srch-dtp-shpgotYmdFrom"
                            ...
                            <sbux-datepicker
                                    uitype="popup"
                                    id="srch-dtp-shpgotYmdTo"
                                    name="srch-dtp-shpgotYmdTo"
                                    date-format="yyyy-mm-dd"
                                    class="form-control pull-right input-sm inpt_data_reqed input-sm-ast"
...
<script type="application/javascript">
    ...
    window.addEventListener("DOMContentLoaded",function(){
        fn_init();
        SBUxMethod.set("srch-dtp-shpgotYmdTo",gfn_dateToYmd(new Date()));
        SBUxMethod.set("srch-dtp-shpgotYmdFrom",gfn_dateFirstYmd(new Date()));
    });
    ...
    const fn_create_rtnCrtr = async function(){
        ...
        SBGridProperties.columns = [
            ...
            {caption: ["생산자"],	ref: 'prdcrNm',	type:'output',  width:'12%', style: 'text-align:center;'},
            {caption: ["반품번호"],	ref: 'shpgotSn',		type:'output',  width:'11%', style: 'text-align:center;'},
            {caption: ["수량"],	ref: 'shpgotQntt',		type:'output',  width:'10%', style: 'text-align:center;'},
            {caption: ["중량"],	ref: 'shpgotWght',		type:'output',  width:'10%', style: 'text-align:center;'},
            {caption: ["상세코드"],	ref: 'shpgotCd',		type:'output',  width:'10%', style: 'text-align:center;'},
            {caption: ["반품사유"],	ref: 'shpgotRmrk',		type:'output',  width:'25%', style: 'text-align:center;'},
        ]
    ...
    const fn_search = async function(){
        ...
        searchParam.apcCd = gv_selectedApcCd;
        const postJsonPromise = gfn_postJSON("/am/spmt/selectShpgotRawMtr.do",searchParam);
        ...
        data.resultList.forEach(function(item){
            let shpgotCd;
          if(item.shpgotBadCd){
            item.shpgotCd = jsonShpgot.filter(inner => inner.crtrCd === item.shpgotBadCd)[0].crtrIndctNm;
          }else if(item.shpgotCsCd){
              item.shpgotCd = jsonShpgot.filter(inner => inner.crtrCd === item.shpgotCsCd)[0].crtrIndctNm;
          }
        });
