<%
 /**
  * @Class Name : rawMtrWghNPrcsPrfmncMng.jsp
  * @Description : 원물계량미처리실적관리 화면
  * @author SI개발부
  * @since 2023.08.31
  * @version 1.0
		...
		<div class="box box-solid">
			<div class="box-header" style="display:flex; justify-content: flex-start;" >
				<div>
					...
					<h3 class="box-title"> ▶ <c:out value='${menuNm}'></c:out></h3><!-- 원물계량미처리실적관리 -->
...
<script type="text/javascript">
	...
	const fn_createWghPrfmncGrid = function() {
        ...
        SBGridProperties.columns = [
            ...
            {caption: ['전체중량 (Kg)'], ref: 'wholWght', width: '80px', type: 'output', style:'text-align:right', format : {type:'number', rule:'#,### '}},
            ...
            {caption: ['입고중량 (Kg)'], ref: 'wrhsWght', width: '80px', type: 'output', style:'text-align:right',
            	format : {type:'number', rule:'#,### '}
            },
            {caption: ['팔레트중량 (Kg)'], ref: 'pltWght', width: '90px', type: 'output', style:'text-align:right',
            	format : {type:'number', rule:'#,### '},
            	merge : false
            },
            {caption: ['박스중량 (Kg)'], ref: 'bxWght', width: '80px', type: 'output', style:'text-align:right',
            	format : {type:'number', rule:'#,### '},
            	merge : false
            },
            {caption: ['재고중량 (Kg)'], ref: 'invntrWght', width: '80px', type: 'output', style:'text-align:right',
            	format : {type:'number', rule:'#,### '},
            	merge : false
            },
    ...
    function fn_excelDwnld() {
    	grdWghPrfmnc.exportLocalExcel("원물계량", {bSaveLabelData: true, bNullToBlank: true, bSaveSubtotalValue: true, bCaptionConvertBr: true, arrSaveConvertText: true});
    }

    // 개인화 저장
    function fn_personalSave(){
    	grdWghPrfmnc.savePersonalInfo("apcCd");
   	}
    // 개인화 호출
    function fn_personalLoad(){
    	grdWghPrfmnc.loadPersonalInfo("apcCd");
   	}
	// 열 숨기기
    function fn_colHidden(){
    	grdWghPrfmnc.setColHidden(grdWghPrfmnc.getCol(), true);
   	}
	// 열 보이기
    function fn_colShow(){
    	for(let i = grdWghPrfmnc.getFixedCols(); i < grdWghPrfmnc.getCols()-1; i++) {
   			grdWghPrfmnc.setColHidden(i, false);
    	}
   	}
	...
	const fn_save = async function() {
    	...
    	let rowData = grdWghPrfmnc.getRowData(grdWghPrfmnc.getRow());

    	let postUrl = gfn_isEmpty(rowData.itemCd) ? "/am/wgh/insertWghVhclPrfmnc.do" : "/am/wgh/updateWghVhclPrfmnc.do";
    ...
    const fn_setGrdWghPrfmnc = async function() {
		...
		const postJsonPromise = gfn_postJSON("/am/wgh/selectWghEntrVhclList.do", {
			apcCd: gv_selectedApcCd,
			wghYmd: wghYmd,
  		});
  		...
  		try {
	        ...
	        if (!_.isEqual("S", data.resultStatus)) {
          	...
          	data.resultList.forEach((item, index) => {
          		...
          		if(gfn_nvl(item.wholWght) ==="" ){
          			if (item.entrWght > item.outWght){
              			wholWght = item.entrWght;
              			emptVhclWght = item.outWght;
              		}else if (item.entrWght < item.outWght){
              			wholWght = item.outWght;
              			emptVhclWght = item.entrWght;
              		}
          		}else{
  				...
  				const wghPrfmnc = {
						...
						entrTm : item.entrTm,
						outTm : item.outTm,
						actlWght : item.wholWght - item.emptVhclWght
  				}
	...
	const fn_view = async function() {
		...
		try {
			...
			// 계량일자
			SBUxMethod.set("dtl-dtp-wghYmd", rowData.entrTm);
