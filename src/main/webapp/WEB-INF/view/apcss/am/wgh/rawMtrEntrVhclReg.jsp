...
<script>
	...
	const fn_click = async function(){
		...
		SBUxMethod.set("srch-inp-entrVhclWght",rowData.wghWght); //입차중량
		const timePart = rowData.entrTm.split(' ')[1].replace(/:/g, '').slice(0, 6);
		SBUxMethod.set("srch-dtp-entrVhclTm",timePart); //입차시간
		SBUxMethod.set("srch-inp-rmrk",rowData.wghRmrk);
		SBUxMethod.set("srch-dtp-wghYmd",rowData.wghDt);

	}


	const fn_search = async function(){
    	let postJsonPromise = gfn_postJSON("/am/wgh/selectWghVhclList.do", {apcCd : gv_selectedApcCd});
  			...
  			if (_.isEqual("S", data.resultStatus)) {
  	        	...
  	        	data.resultList.forEach((item, index) => {
  					let fcltVO = {
 							...
 							, wghWght : item.wghWght
 							, delYn : item.delYn
  					}
  					if(item.wghSeq === "1"){
  						fcltVO['entrTm'] = item.wghDt;
  					}
	...
	const fn_save = async function(){
		...
		try{
            ...
            let postJsonPromise = gfn_postJSON("/am/wgh/insertWghVhcl.do",obj);
    ...
    const fn_delete = async function(){
        ...
        let postJsonPromise = gfn_postJSON("/am/wgh/deleteWghVhcl.do",rowData);
        let data = await postJsonPromise;
