...
// 		await gfn_setComCdGridSelect('grdUserAprv', 	jsonUserType,    'USER_TYPE');	// 사용유무
	...
	const fn_setGrdUserAprv = async function(recordCountPerPage, currentPageNo) {
		...
		const postJsonPromise = gfn_postJSON("/co/user/selectLocgovUserAprvList.do", {
			userStts: userStts,
			ctpv: untyCtpv,
			sgg: untySgg,
			userNm: userNm,
	...
	const fn_insert = async function() {
    	...
    	const postJsonPromise = gfn_postJSON("/co/user/insertLocgovAprvList.do", aprvList);
		const data = await postJsonPromise;
        try {
        	if (_.isEqual("S", data.resultStatus)) {
	...
	const fn_cancel = async function() {
    	...
    	const postJsonPromise = gfn_postJSON("/co/user/deleteLocgovAprvList.do", userList);
		const data = await postJsonPromise;
        try {
        	if (_.isEqual("S", data.resultStatus)) {
        		gfn_comAlert("I0001");	// I0001	처리 되었습니다.
