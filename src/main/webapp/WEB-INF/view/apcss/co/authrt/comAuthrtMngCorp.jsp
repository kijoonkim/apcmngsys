<%
 /**
  * @Class Name : comAuthrtUntyMng.jsp
...
<script type="text/javascript">
    ...
    const fn_initSBSelect = async function() {
    	...
    	let rst = await Promise.all([
    		// 조회 SB select
    	 	...
    	 	// 상세 SB select
    	 	gfn_setComCdSBSelect('dtl-slt-authrtType', jsonComAuthrtType, 'UNTY_AUTHRT_TYPE'),	// 권한유형
    	 	fn_initComCd()
    	]);
    	...
    	switch (gv_userType) {
    		...
    		case "27":
    			//jsonComAuthrtType = gfn_getJsonFilter(jsonComAuthrtType, 'value', ["20", "21"]);
    			jsonComSysId = gfn_getJsonFilter(jsonComSysId, 'value', ["AM", "CS", "FM", "PD", "PT", "CO"]);
    			break;
    ...
    const fn_createGrdComAuth = function() {
        ...
        SBGridProperties.columns = [
            ...
            {caption: ["통합조직명"],		ref: 'untyOgnzNm',  type:'output',  width:'30%',    style:'text-align:left'},
    ...
    const fn_setGrdComAuth = async function(pageSize, pageNo) {
        ...
        const postJsonPromise = gfn_postJSON("/co/authrt/selectUntyAuthrtList.do",{
        		sysId: sysId,
            	authrtNm: authrtNm,
            	// pagination
    	  		pagingYn : 'Y',
    ...
    const fn_setGrdComAuthMenu = async function(sysId, authrtId, authrtType) {
		...
		const postJsonPromise = gfn_postJSON("/co/authrt/selectUntyAuthrtMenuTreeList.do", {
         	sysId: sysId,
         	authrtId: authrtId,
    ...
    const fn_insertComAuthrt = async function(isConfirmed) {
    	...
    	const postJsonPromise = gfn_postJSON("/co/authrt/insertUntyAuthrt.do", {
			authrtId 	: authrtId,
			authrtNm 	: authrtNm,
			authrtType 	: authrtType,
    ...
    const fn_view = function () {
        ...
        fn_setGrdComAuthMenu(rowData.sysId, rowData.authrtId, rowData.authrtType);
    }
    ...
    const fn_saveComAuthMenu = async function(isConfirmed) {
    	...
    	let authrtType = SBUxMethod.get("dtl-slt-authrtType");
		let sysId = "";
        ...
        try {
        	if (_.isEqual("S", data.resultStatus)) {
        		gfn_comAlert("I0001");	// I0001	처리 되었습니다.
        		fn_setGrdComAuthMenu(sysId, authrtId, authrtType);
        	} else {
        		gfn_comAlert(data.resultCode, data.resultMessage);
        	}
        } catch (e) {
