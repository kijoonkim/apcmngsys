...
//			scaleQusArry += ',어떤 경로를 통해 참가하시게 됐습니까?';
			...
			choiceAnsQusArry +=  '1^융합적사고역량';
			choiceAnsQusArry += ',1^문제해결역량';
			choiceAnsQusArry += ',1^자기관리역량';
			choiceAnsQusArry += ',1^도전성취역량';
			choiceAnsQusArry += ',1^대인관계역량';
			choiceAnsQusArry += ',1^협업실천역량';
...
function fnOnClickSave(viewType){
	...
	if( viewType == 'MODIFY'){
		...
		if('${programInfoMap.NPI_STAT_DB_CD}' == 'NCR_T05_P06' ||'${programInfoMap.NPI_STAT_DB_CD}' == 'NCR_T05_P07'||'${programInfoMap.NPI_STAT_DB_CD}' == 'NCR_T05_P08'){
			if('${sessionScope.loginVO.roleId}' == 'RLE0000024' || '${sessionScope.loginVO.roleId}' == 'SUPERADMIN_01'){
...
/**
 *********************************************************
@ function : 하위역량 입력값의 합을 상위역량 퍼센트에 넣어줌(위덕 VER)
@ comment  :
@ param    : _cdDiv = 전공/핵심 역량 구분
@ history  : 2017-11-07 (최초작성)
 **********************************************************
	...
	**********************************************************
	...
	function fnApplyStdStateGraph(divNm, dataArry, categoriesArry) {
		var options = {
				...
				colors: ['#37aa82', '#d09fda', '#da6319', '#ffcd7f', '#faa7d2', '#5a5db4'],
				fill: {
					colors: ['#37aa82', '#d09fda', '#da6319', '#ffcd7f', '#faa7d2', '#5a5db4'],
				},
				plotOptions: {
