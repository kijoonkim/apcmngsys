...
import com.apcmngsys.apcss.am.spmt.service.SpmtMngService;
...
//            double spmtWght = 0;
    ...
    public HashMap<String, Object> insertSpmtRsltMonthly(SpmtPrfmncComVO spmtPrfmncComVO) throws Exception {
        ...
        for ( SpmtPrfmncVO spmt : spmtPrfmncList ) {
            ...
            String sortGrdCd = ComUtil.nullToDefault(spmt.getSortGrdCd(), "X0");
            String prdcrIdentno = ComUtil.nullToEmpty(spmt.getPrdcrIdentno());
            String aftrGrdCd = ComUtil.nullToEmpty(spmt.getAftrGrdCd());
...
//            double spmtWght = 0;
            ...
            if (StringUtils.hasText(pckgno) && pckgSn > 0) {    // 포장번호가 있는 경우
                ...
                gdsParam.setWarehouseSeCd(warehouseSeCd);
                gdsParam.setSpmtPckgUnitCd(spmtPckgUnitCd);
                gdsParam.setRprsPrdcrCd(prdcrCd);


                List<GdsInvntrVO> gdsInvntrList = gdsInvntrService.selectSpmtGdsInvntrList(gdsParam);
