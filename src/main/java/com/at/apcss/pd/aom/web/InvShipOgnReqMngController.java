...
public class InvShipOgnReqMngController extends BaseController{
	...
	// 로우데이터 조회
	@PostMapping(value = "/pd/aom/hiddenGrdIsoSelectList.do", consumes = {MediaType.APPLICATION_JSON_VALUE , MediaType.TEXT_HTML_VALUE})
	public ResponseEntity<HashMap<String, Object>> hiddenGrdIsoSelectList(Model model, @RequestBody InvShipOgnReqMngVO InvShipOgnReqMngVO, HttpServletRequest request) throws Exception{
		HashMap<String,Object> resultMap = new HashMap<String,Object>();
		List<InvShipOgnReqMngVO> resultList = new ArrayList<>();
		try {
			 resultList = InvShipOgnReqMngService.hiddenGrdIsoSelectList(InvShipOgnReqMngVO);
		} catch (Exception e) {
			logger.debug(e.getMessage());
			return getErrorResponseEntity(e);
		}
